...
type
  ...
  TGIS_Editor = {$IFDEF OXYGENE} public {$ENDIF}
    ...
    private // property internal values
      ...
      /// <summary>
      ///   If True, block snapping.
      /// </summary>
      FBlockSnapping : Boolean ;
    ...
    public // properties from IGIS_Editor
      ...
      property BlockSnapping      : Boolean
                                    read  fget_BlockSnapping
                                    write fset_BlockSnapping  ;
...
//============================================================================
  ...
  begin
    ...
    if ( not FUseSnapLayers or FBlockSnapping ) then exit ;
  ...
  begin
    if not InEdit then exit ;
    if ( _pos < 0 ) or ( _pos > pointState.Count ) then exit ;

    if pointState.Action = TGIS_EditorAction.Add then begin
      // decrement point position according to mouseEndInternal
...
//----------------------------------------------------------------------------
  ...
  function TGIS_Editor.fget_BlockSnapping      : Boolean ;
  begin
    Result := FBlockSnapping ;
  end ;

  procedure TGIS_Editor.fset_BlockSnapping(
    const _value : Boolean
  ) ;
  begin
    FBlockSnapping := _value ;
  end;
...
//----------------------------------------------------------------------------
  ...
  begin
    ...
    vwr.BusyPrepare( Self, GIS_BUSY_NOHOURGLASS ) ;
    try
      deletePointInternal( _pos - 1 ) ;
    finally
      vwr.BusyRelease( Self ) ;
    end ;
  end ;

  procedure TGIS_Editor.DeletePointEx( const _pos : Integer ) ;
  var
    vwr : TGIS_Viewer ;
  begin
    vwr := TGIS_Viewer(FViewer) ;
  ...
  begin
    ...
    vwr.BusyPrepare( Self, GIS_BUSY_NOHOURGLASS ) ;
    try
      insertPointInternal( _pos, _ptg, False ) ;
    finally
      vwr.BusyRelease( Self ) ;
    end ;
  end ;

  procedure TGIS_Editor.InsertPointEx(
    const _pos : Integer      ;
    const _ptg : TGIS_Point3D
  );
  var
  ...
  begin
    ...
    vwr.BusyPrepare( Self, GIS_BUSY_NOHOURGLASS ) ;
    try
      insertPointInternal( _pos, _ptg, True ) ;
    finally
      vwr.BusyRelease( Self ) ;
    end ;
  end ;
  ...
  begin
    ...
    vwr.BusyPrepare( Self, GIS_BUSY_NOHOURGLASS ) ;
    try
      movePointInternal( _pos - 1, _ptg ) ;
    finally
      vwr.BusyRelease( Self ) ;
    end ;
  end ;

  procedure TGIS_Editor.MovePointEx(
    const _pos : Integer      ;
    const _ptg : TGIS_Point3D
  ) ;
  var
