...
unit FMX.GisRenderer3D ;
{$HPPEMIT '#pragma link "FMX.GisRenderer3D"'}
...
type
  ...
  private
    ...
    pdataTEX_sel           : TTexture ;      // Selection texture
    labelInfo              : T_arLabelInfo     ;
    oMtrl                  : TColorMaterial    ;
    ambColorDX             : TD3DColor         ;
    ambColorDiffuse        : TD3DColor         ;
    ambColor               : TD3DColor        ;
  ...
  private
    ...
    /// <summary>
    ///   Make texture used to mark selected texture objects.
    /// </summary>
    procedure makeSelectionTex       ;
  ...
  protected
    ...
    /// <inheritdoc/>
    procedure setDiffuseLightColor   ( const _val  : Integer
                                     );
...
//    oContext.SetContextState( TContextState.csAlphaBlendOn ) ;
  ...
  procedure TGIS_Renderer3DFMX.setDiffuseLightColor(
    const _val  : Integer
  ) ;
  begin
    ambColorDX := color2ARGB( 255, 1- _val, 1 - _val, 1 - _val ) ;
    ambColorDiffuse := ambColorDX ;
  end;
...
//?    oContext.SetCameraMatrix( mtxView ) ;
  ...
  procedure TGIS_Renderer3DFMX.makeSelectionTex ;
  var
    bmp     : TGIS_Bitmap ;
    i,j,k,l : Integer ;
    dat     : TBitmapData ;
  ...
  begin
    ...
    try
      ...
      TBitmap( bmp.NativeBitmap ).Clear(
        TAlphaColorF.Create(
        oGis.SelectionGisColor.R/255,
        oGis.SelectionGisColor.G/255,
        oGis.SelectionGisColor.B/255,
        a/255 ).ToAlphaColor
      ) ;

      if Assigned( pdataTEX_sel ) then
        FreeObject( pdataTEX_sel ) ;
      pdataTEX_sel := create_texture( bmp ) ;
    finally
      FreeObject( bmp ) ;
    end ;
  end;
  ...
  begin
    ...
    plinelist_vb := TVertexBuffer.Create(GISVIEWER3D_D3DFVF_GRAPH3DTEXTURE, 4) ;
  ...
  var
    ...
    begin
      ...
      vertxlist_vb := TVertexBuffer.Create(GISVIEWER3D_D3DFVF_GRAPH3DTEXTURE, 6) ;
  ...
  begin
    ...
    vertxlist_vb := TVertexBuffer.Create(GISVIEWER3D_D3DFVF_GRAPH3DTEXTURE, num) ;
  ...
  begin
    ...
    linelist_vb := TVertexBuffer.Create( GISVIEWER3D_D3DFVF_GRAPH3DCOLOR,
                                          num_val
                                        ) ;
...
//     oContext.SetContextState( TContextState.csAlphaBlendOff ) ;
  ...
  begin
    ...
    // Create the vertex buffer
    vertxlist_vb := TVertexBuffer.Create(GISVIEWER3D_D3DFVF_GRAPH3DTEXTURE,4) ;
        ...
        if _mode then begin
          ...
          case bt of
            9 : begin
              ...
              arBufSelInfo[bufposi].SelIdx.Add(j1) ;
              arBufSelInfo[bufposi].SelIdx.Add(j2) ;
            end;
            10 : begin
              for j := j1 to j2 - 1 do begin
                TMesh(_mesh[i0]).tvertices[j].Color := mark_color ;
  ...
  begin
    ...
    pdataVB := TVertexBuffer.Create( GISVIEWER3D_D3DFVF_GRAPH3DTEXTURE,
     ...
     cdataVB := TVertexBuffer.Create( GISVIEWER3D_D3DFVF_GRAPH3DCOLOR,
                                     videoVertexBufferSize
                                   ) ;
  ...
  begin
    ...
    _vbuf := TVertexBuffer.Create( GISVIEWER3D_D3DFVF_GRAPH3DCOLOR,
                                     videoVertexBufferSize
                                    ) ;
...
//      if not (((_shp.Uid = 5) and (part_no = 11)) or
//           ((_shp.Uid = 6) and (part_no = 2))) then continue;
  ...
  begin
    ...
    if meshDemTilesCounter > 0 then begin
      ...
      for i := 0 to mtc do
        TMesh(meshDemTiles[i]).DrawSubset(0, -1, False) ;
    end;
    ...
    if meshWallTilesCounter > 0 then begin
      ...
      for i := 0 to mtc do
        TMesh(meshWallTiles[i]).DrawSubset(0, -1, False) ;
  ...
  begin
    ...
    if meshVectTilesCounterCR > 0 then begin
      for l := 0 to bufSelInfoCounter -1 do
        if arBufSelInfo[l].BufferType = 2 then break;
      ...
      for i := 0 to mtc do begin
        ...
        for j := 0 to k -1 do begin
          ...
          TMesh(meshVectTilesCR[i]).DrawSubset(j, l, False) ;
        end;
      end;
      blendOff ;
      current_opc := 1.0 ;
    ...
    if meshVectTilesCounterCW > 0 then begin
      for l := 0 to bufSelInfoCounter -1 do
        if arBufSelInfo[l].BufferType = 1 then break;
      ...
      for i := 0 to mtc do begin
        ...
        for j := 0 to k -1 do begin
          ...
          TMesh(meshVectTilesCW[i]).DrawSubset(j, l, False) ;
        end;
      end;
      blendOff ;
      current_opc := 1.0 ;
    ...
    if meshVectTilesCounterTR > 0 then begin
      for l := 0 to bufSelInfoCounter -1 do
        if arBufSelInfo[l].BufferType = 4 then break;
      ...
      for j := 0 to mtc do begin
        ...
        for i := 0 to k do begin
          ...
          TMesh(meshVectTilesTR[j]).DrawSubset(i, l, True) ;
        end;
      end;
      blendOff ;
      current_opc := 1.0 ;
    ...
    if meshVectTilesCounterTW > 0 then begin
      for l := 0 to bufSelInfoCounter -1 do
        if arBufSelInfo[l].BufferType = 3 then break;
      ...
      for j := 0 to mtc do begin
        ...
        for i := 0 to k do begin
          ...
          TMesh(meshVectTilesTW[j]).DrawSubset(i, l, True) ;
        end;
      end;
      blendOff ;
      current_opc := 1.0 ;
  ...
  begin
    ...
    if meshMPatchTilesCounterC > 0 then begin
      for l := 0 to bufSelInfoCounter -1 do
        if arBufSelInfo[l].BufferType = 9 then break;
      ...
      for i := 0 to meshMPatchTilesCounterC -1 do begin
        ...
        for j := 0 to k -1 do begin
          ...
          TMesh(meshMPatchTilesC[i]).DrawSubset(j, l, False) ;
        end;
      end;
      blendOff ;
      current_opc := 1.0 ;
    ...
    if meshMPatchTilesCounterT > 0 then begin
      for l := 0 to bufSelInfoCounter -1 do
        if arBufSelInfo[l].BufferType = 10 then break;
      ...
      for j := 0 to meshMPatchTilesCounterT -1 do begin
        ...
        for i := 0 to k do begin
          ...
          TMesh(meshMPatchTilesT[j]).DrawSubset(i, l, True) ;
        end ;
      end ;
      current_opc := 1.0 ;
      blendOff ;
 ...
 begin
   ...
   TMesh(meshTinTilesC[mtc]).DrawSubset(0, -1, False) ;
 end;
  ...
  begin
    ...
    TMesh(meshTinTilesT[mtc]).DrawSubset(0, -1, False) ;
  end;
  ...
  begin
    ...
    if meshTinTilesCounterC > 0 then begin
      ...
      for i := 0 to meshTinTilesCounterC -1 do begin
        ...
        if k = 0 then k := 1;
        for j := 0 to k -1 do begin
          TMesh(meshTinTilesC[i]).DrawSubset(j, -1, False) ;
        end;
        if transpInfo.TCT[i] <> 255 then begin
          blendOff ;
          current_opc := 1 ;
    ...
    if meshTinTilesCounterT > 0 then begin
      for i := 0 to meshTinTilesCounterT -1 do begin
        ...
        TMesh(meshTinTilesT[i]).DrawSubset(0, -1, False) ;
...
//?          demFrameSize  := vertexBufferSize ;
  ...
  begin
    ...
    if format = GISVIEWER3D_D3DFVF_GRAPH3DTEXTURE then
  ...
  procedure TMesh.DrawSubset(
    const _attribId : DWORD;       // subset no
    const _bufno    : Integer;     // index in TGIS_Renderer3DSelectionInfoArray
    const _mode     : Boolean      // True - modify texture, False - modify opacity
  ) ;
  var
    ...
    procedure draw1SubsNoSel;
    begin
      idataIBm.Length := l * 3 ;
      rnd.current_cntx.DrawPrimitives(TPrimitivesKind.Triangles, @tvertices[0],
          @indices[0],
    ...
    procedure drawSubsPart( const _iatr : Integer; const _num : Integer; const _opc : Single ) ;
    begin
      rnd.current_cntx.DrawPrimitives(TPrimitivesKind.Triangles, @tvertices[0],
          @indices[_iatr],
          pdataVBm.GetVertexDeclarations, pdataVBm.VertexSize, pdataVBm.Length,
          sizeOf(indices[0]), _num ,
          mtrl, _opc )
    end;

    function getRange(  var _v1 : Integer; var _v2 : Integer; var _opc : Single ) : Boolean ;
    begin
      Result := False ;
      _opc := rnd.current_opc ;
      rnd.texture_mtrl.Texture := rnd.current_tex ;
      mtrl := rnd.texture_mtrl ;

      if curidx = k then exit;

      if curidx < j1 then begin
        _v1 := curidx ;
        _v2 := j1-curidx ;
        curidx := j1 ;
        Result := True;
      ...
      if curidx = j1 then begin
        ...
        if _mode then begin
          rnd.texture_mtrl.Texture := rnd.pdataTEX_sel;
          mtrl := rnd.texture_mtrl ;
        end
        else
        _opc := rnd.oGIS.SelectionTransparency / 100 ;

        Result := True;
        exit;
      end;

      if curidx = j2 then begin
        _v1 := curidx ;
        if numsel=curnum then begin
          _v2 := k - curidx ;
          curidx := k
        end
        else begin
          j1 := rnd.arBufSelInfo[_bufno].SelIdx[curnum] ;
          Inc( curnum ) ;
          j2 := rnd.arBufSelInfo[_bufno].SelIdx[curnum] ;
          Inc( curnum ) ;
          _v2 := j1 - curidx ;
          curidx := j1 ;
        end;
    ...
    procedure draw1SubsIsSel ;
    var
      v1, v2 : Integer ;
      opc : Single ;
    begin
      if j1 <> -1 then
        while getRange(v1, v2, opc) do begin
          if rnd.current_lght then mtrl := rnd.light_mtrl ;
          drawSubsPart( v1, v2, opc ) ; // rnd.current_opc
        end
      else
        draw1SubsNoSel;
    end;

    procedure getj1j2(var _j1 : Integer; var _j2 : Integer) ;
    var
      i : Integer ;
    begin
      i := 0 ;
      while True do begin ;
        _j1 := rnd.arBufSelInfo[_bufno].SelIdx[i] ;
        inc( i ) ;
        _j2 := rnd.arBufSelInfo[_bufno].SelIdx[i] ;
        inc( i ) ;
        if (attridx >= _j1) and (attridx < j2) then exit;
        if i=numsel then break;
      end;
      _j1 := -1 ;
      _j2 := -1 ;
    end;
...
//-----------------------------------------------------------------------
  begin
    ...
    if _bufno < 0 then
      ...
      bt := rnd.arBufSelInfo[_bufno].BufferType ;

    j1 := -1;
    j2 := -1 ;
    if _bufno >= 0 then begin
      if rnd.arBufSelInfo[_bufno].SelIdx.Count > 0 then begin
        rnd.arBufSelInfo[_bufno].SelIdx.Sort ;
        numsel := rnd.arBufSelInfo[_bufno].SelIdx.Count ;
        curnum := 2 ;
        j1 := rnd.arBufSelInfo[_bufno].SelIdx[0] ;
        j2 := rnd.arBufSelInfo[_bufno].SelIdx[1] ;
        if (bt = 10) then begin
    ...
    if format = GISVIEWER3D_D3DFVF_GRAPH3DTEXTURE then
      begin
        ...
        pdataVBm.Color0[0] := $99FF0000 ;

        if rnd.current_lght then begin
          rnd.light_mtrl.Ambient  := rnd.ambColor ;
          rnd.light_mtrl.Diffuse  := rnd.ambColorDiffuse ;
        ...
        if Length( subsets ) = 0 then begin
          if j1 = -1 then   // 1 subset, no selection
            draw1SubsNoSel
          else
            draw1SubsIsSel  // 1 subset with selection
        end
        else begin
          if bt = 10 then begin  // textured mpatch
            if (attridx>=j1) and (attridx<j2) then begin
              rnd.texture_mtrl.Texture := rnd.pdataTEX_sel;
              mtrl := rnd.texture_mtrl ;
            end;
            drawNSubsNoSel
          end
          else begin  // subsets > 1
            if idataIBm.Length = k then
              draw1SubsIsSel
            else
              drawNSubsNoSel
          end;
