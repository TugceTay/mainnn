...
unit Lider.CG.GIS.FMX.GeoFramework ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.FMX.GeoFramework"'}
...
uses
  ...
  Lider.CG.GIS.FMX.GeoControlPrintPreview,
...
function TGIS_BitmapFMX.fget_NativeBitmap
  : TObject ;
begin
  Result := FBitmap ;
end;

procedure TGIS_BitmapFMX.fset_NativeBitmap(
...
begin
  ...
  vwr := TGIS_ViewerBmp.Create( FMX.Graphics.TBitmap( self.NativeBitmap ) ) ;
  try
    if _ppi > 0  then
      vwr.CustomPPI := _ppi ;
...
begin
  FreeObject( FViewer ) ;
  FViewer := TGIS_ViewerBmp.Create( T_FMXBitmap( self.NativeBitmap ) ) ;
...
begin
  ...
  BitmapHelper := TGIS_BitmapFactoryFMX.Create ;
