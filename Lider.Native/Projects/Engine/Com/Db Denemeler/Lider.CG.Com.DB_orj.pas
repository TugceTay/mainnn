...
type
  ...
  protected {My simplified methods to override}
    ...
    function Navigate(Buffer: PChar; GetMode: TGetMode; doCheck: Boolean):
...
begin
  Result := Navigate(PWideChar(Buffer), GetMode, DoCheck);
  if (Result = grOk) then
  begin
...
begin
  ...
  begin
    ...
    if (Char(RecBuffer[0]) = #0) or (Buffer = nil) then
      exit
    else
...
function TlicgBaseDataset.FilterRecord(Buffer: PChar): Boolean;
begin
  result := True;
end;
...
function TlicgTable.Navigate(Buffer: PChar; GetMode: TGetMode; doCheck: Boolean):
  TGetResult;
var
  Acceptable: Boolean;
begin
...
function TlicgTable.FilterRecord(Buffer: PChar): Boolean;
var
...
function TlicgDesignTable.Navigate(Buffer: PChar; GetMode: TGetMode; doCheck:
  Boolean): TGetResult;
begin
  if FRecordCount < 1 then
    Result := grEOF
