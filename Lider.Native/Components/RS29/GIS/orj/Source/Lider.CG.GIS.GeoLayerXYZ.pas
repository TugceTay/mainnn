...
{$IFDEF DCC}
  unit GisLayerXYZ ;
  {$HPPEMIT '#pragma link "GisLayerXYZ"'}
{$ENDIF}
{$IFDEF CLR}
...
//=============================================================================
  ...
  procedure TGIS_LayerXYZ.RevertShapes ;
  var
    i : Integer ;
  begin
    inherited ;
    ...
    for i := 0 to BUILTIN_CURSORS - 1 do
      cursorXYZ[i].curShape := nil ;
  end ;

  { Perform initialization section.
  }
  class procedure Unit_GisLayerXYZ.SelfRegisterLayer() ;
  begin
...
{$IFDEF DCC}
  initialization
    Unit_GisLayerXYZ.SelfRegisterLayer() ;
{$ENDIF}
