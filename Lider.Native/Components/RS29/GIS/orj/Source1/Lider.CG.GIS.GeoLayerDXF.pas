...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoLayerDXF ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayerDXF"'}
{$ENDIF}
{$IFDEF CLR}
...
//=============================================================================
  ...
  function TGIS_LayerDXF.getSubLayer(
    const _name : String
  ) : TGIS_LayerVector ;
  var
    obj : TObject ;
  ...
  begin
    ...
    begin
      if dxfTestLine( GIS_DXF_CADE_DATA, '' ) then begin
        ...
        try
          tkn.Execute( lineName, ['='] ) ;
          if tkn.Result.Count > 0 then begin
            try
              if FindField( tkn.Result[0] ) < 0 then
                AddFieldInternal( tkn.Result[0], TGIS_FieldType.String, 1, 0 ) ;

              for i := 0 to SubLayers.Count - 1 do begin
                lv := TGIS_LayerVector( SubLayers.Items[ i ] );
                if lv.FindField( tkn.Result[0] ) < 0 then
                  lv.AddFieldInternal( tkn.Result[0], TGIS_FieldType.String, 1, 0 ) ;
              end ;
              if tkn.Result.Count > 1 then
                currShape.SetField( tkn.Result[0], tkn.Result[1] ) ;
            except
            end ;
          end ;
        finally
    ...
    currShape.Lock( TGIS_Lock.Projection ) ;
      ...
      begin
        if      dxfTestLine( GIS_DXF_CHANDLE, '' ) then begin
                  ...
                  currShape.Layer := getSubLayer( layername ) ;
                end
        else if dxfTestLine( GIS_DXF_CCOLOR, '' ) then begin
...
//      if color <> 0 then
  ...
  begin
    ...
    currShape.Lock( TGIS_Lock.Projection ) ;
      ...
      drawArc( ptgCenter, a, b, start, swgang ) ;
 ...
 begin
  ...
  begin
    ...
    begin
        ...
        while not dxfTestLine( GIS_DXF_CNTABLES, GIS_DXF_ENTABLES ) and
          ...
          if dxfTestLine( GIS_DXF_CLLSTYLE, '' ) then begin
             if not IsStringEmpty( Trim( lineName ) ) then begin
               ...
               T_DxfLayer(la).PenStyle := getLineStyle( layername, lstyle )
             end ;
          end ;
      ...
      if not dxfTestLine( GIS_DXF_TELAYER, GIS_DXF_NLAYER ) then
        dxfFetchLine ;
    end ;
    {$IFDEF OXYGENE}
      if assigned( stab ) then
        stab := nil ;
    {$ENDIF}
  end ;
  ...
  begin
    ...
    if IsStringEmpty( lname ) or (lname = 'BYLAYER') then begin
      if FLayers.TryGetValue( _layer, obj ) then
...
//==============================================================================
  ...
  class procedure GisLayerDXF.SelfRegisterLayer() ;
  begin
