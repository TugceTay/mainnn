...
unit VCL.GisPrintLegend ;
{$HPPEMIT '#pragma link "VCL.GisPrintLegend"'}
...
//==============================================================================
  ...
  begin
    ...
    FFont := TFont.Create ;
    FFont.Assign( TBitmap(oBitmap.NativeBitmap).Canvas.Font ) ;
    FCompactView := False ;
  ...
  var
    ...
    begin
      if RightToLeft then
        rr := Rect( oBitmap.Width - _l - _b.Width, _t,
                    oBitmap.Width - _l, _t + _b.Height )
      else
        rr := Rect( _l, _t, _l + _b.Width, _t + _b.Height ) ;
      // make sure that pixels are unlocked
      _b.UnlockPixels ;
  ...
  begin
    ...
    // the most tested renderer for printing
    ...
    try
      ...
      oRenderer.CanvasFont.Name := FFont.Name ;
      oRenderer.CanvasFont.Size := RoundS( FFont.Size ) ;
      ...
      oBMPFactory.Renderer := oRenderer.CreateInstance ;
