...
type
  ...
  protected {My simplified methods to override}
    ...
    function Navigate(Buffer: TRecordBuffer; GetMode: TGetMode; doCheck: Boolean):
      TGetResult; virtual; abstract;
    //Internal isOpen property
    function FilterRecord(Buffer: TRecordBuffer): Boolean; virtual;
    function DoBookmarkValid(Bookmark: TBookmark): boolean; virtual; abstract;
    function DoCompareBookmarks(Bookmark1, Bookmark2: TBookmark): Integer;
      virtual; abstract;
  ...
  protected
    ...
    function GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode; DoCheck:
...
{$ENDIF}
  ...
  protected
    function FilterRecord(Buffer: TRecordBuffer): Boolean; override;
  ...
  public
    ...
    function CreateBlobStream(Field: TField; Mode: TBlobStreamMode): TStream; override;
    function Find(const Expression: string; Direction: TlicgDirection; Origin:
      TlicgOrigin): Boolean;
    ...
    procedure ScopeFilter(const Scope: string; ClearBefore: Boolean); //: 17.10.2007
    procedure PolygonFilter(Polygon: IlicgEntity; operator: TlicgGraphicOperator;
      const QueryExpression: string; ClearBefore: Boolean); //: 17.10.2007
    procedure RectangleFilter(const AxMin, AyMin, AxMax, AyMax: Double; operator:
      TlicgGraphicOperator; const QueryExpression: string; //: 17.10.2007
      ClearBefore: Boolean);
    procedure BufferFilter(Buffer: IlicgEntity; operator: TlicgGraphicOperator;
      const QueryExpression: string; CurvePoints: Integer; //: 17.10.2007
      const Distance: Double; ClearBefore: Boolean);
    procedure PolylineIntersects(Polyline: IlicgEntity; const QueryExpression:
      string; ClearBefore: Boolean); //: 17.10.2007
    procedure FilterFromLayer(SourceLayer: TlicgBaseLayer; const QueryExpression:
      string; operator: TlicgGraphicOperator; ClearBefore: Boolean); //: 17.10.2007
...
procedure TlicgBaseDataset.ClearCalcFields(Buffer: TRecordBuffer);
begin
  FillChar(Buffer[FStartCalculated], CalcFieldsSize, 0);
end;

function TlicgBaseDataset.GetActiveRecordBuffer: TRecordBuffer;
begin
  case State of
    dsBrowse:
      if isEmpty then
...
function TlicgBaseDataset.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode;
  DoCheck: Boolean): TGetResult;
begin
  Result := Navigate(PByte(Buffer), GetMode, DoCheck);
  if (Result = grOk) then
  begin
...
begin
  ...
  for Index := 0 to FieldCount - 1 do
    case Fields[Index].DataType of
      ftString:
        Result := Result + Fields[Index].Size + 1; //Leave space for terminating null
      ftInteger, ftSmallInt, ftDate, ftTime:
...
procedure TlicgBaseDataset.InternalInitRecord(Buffer: TRecordBuffer);
begin
  FreeRecordPointers(Buffer);
...
procedure TlicgBaseDataset.InternalAddRecord(Buffer: Pointer; Append: Boolean);
begin
  if Append then
    InternalLast;
...
procedure TlicgBaseDataset.InternalSetToRecord(Buffer: TRecordBuffer);
begin
...
procedure TlicgBaseDataset.BufferToRecord(Buffer: TRecordBuffer);
var
  TempStr: string;
  TempInt: Integer;
  TempDouble: Double;
...
procedure TlicgBaseDataset.RecordToBuffer(Buffer: TRecordBuffer);
var
  Value: Variant;
  TempStr: string;
  TempInt: Integer;
    ...
    case Fields[Index].DataType of
      ftString:
        begin
          ...
          StrLCopy(PAnsiChar(Buffer + Offset), PAnsiChar(AnsiString(TempStr)), length(TempStr));
        end;
      ftInteger, ftSmallInt, ftDate, ftTime:
        begin
          if VarIsNull(Value) then
...
begin
  ...
  begin
    ...
    if (PAnsiChar(RecBuffer[0]) = #0) or (Buffer = nil) then
      exit
    else
  ...
  begin
    ...
    case Field.DataType of
      ...
      ftString:
        begin
          StrLCopy(Buffer, PAnsiChar(RecBuffer + Offset), StrLen(PAnsiChar(RecBuffer + Offset)));
          StrPCopy(Buffer, TrimRight(StrPas(PAnsiChar(Buffer))));
        end;
      ftCurrency, ftFloat:
...
begin
  ...
  begin
    ...
    case Field.DataType of
      ...
      ftString:
        StrLCopy(PAnsiChar(RecBuffer + Offset), Buffer, StrLen(PAnsiChar(Buffer)));
      ftDateTime:
        begin
...
procedure TlicgBaseDataset.GetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  if BookMarkSize > 0 then
    AllocateBookMark(PRecordInfo(Buffer + FDataSize).RecordID, Data);
end;

function TlicgBaseDataset.GetBookmarkFlag(Buffer: TRecordBuffer): TBookmarkFlag;
begin
  Result := PRecordInfo(Buffer + FDataSize).BookMarkFlag;
end;

procedure TlicgBaseDataset.SetBookmarkData(Buffer: TRecordBuffer; Data: Pointer);
begin
  if PRecordInfo(Buffer + FDataSize)^.BookMark = nil then
...
procedure TlicgBaseDataset.SetBookmarkFlag(Buffer: TRecordBuffer; Value: TBookmarkFlag);
begin
...
{function TlicgBaseDataset.CreateBlobStream(Field: TField; Mode: TlicglobStreamMode): TStream;
begin
  Result:=TlicgBlobStream.Create(Field as TlicglobField, Mode);
end; }
...
function TlicgBaseDataset.CurrToBCD(const Curr: Currency; BCD: Pointer;
...
function TlicgBaseDataset.FilterRecord(Buffer: TRecordBuffer): Boolean;
begin
  Result := True;
end;
...
//************************** TlicgBlobStream ***************************************
{constructor TlicgBlobStream.Create(Field: TlicglobField; Mode: TlicglobStreamMode);
begin
  inherited Create;
  FField:=Field;
  FMode:=Mode;
...
var
  Stream: TMemoryStream;
  Offset: Integer;
  RecBuffer: PByte; // ilker deðiþtirme
begin
  Self.Size:=0;
...
begin
  ...
  for I := 1 to Layer.DBTable.FieldCount do
    if (AnsiCompareText(Layer.DBTable.Field(I), 'Geometry') <> 0) and (AnsiCompareText
      (Layer.DBTable.Field(I), 'Shape') <> 0) then
      with Add do
      begin
        SourceField := I;
...
begin
  ...
  begin
    ...
    //(MapField.SourceField as TlicglobField).SaveToStream(Self);
  end;
  Self.Position := 0;
end;
...
begin
  //C: 11.01.2006 GIS sorgusundan dönen sonuçlar listeye GISBeforePaintEntity de  eklendiðinden.
  //aþaðýdaki koþul eklendi.
  ...
  for I := 0 to FMapFields.Count - 1 do
    ...
    begin
      ...
      begin
        ...
        begin
          FldDef.Size:= TlicgCDField(SourceField).Size;
          FldDef.Precision:= TlicgCDField(SourceField).Precision;
        end; }
      end;
    end;
end;
...
function TlicgTable.Lookup(const KeyFields: string; const KeyValues: Variant;
...
begin
  ...
  if MapField.IsExpression then
    Exit;
  if not CharInSet(FLayer.DBTable.FieldType(MapField.SourceField), ['M', 'B', 'G']) then
  begin
    FLayer.DBTable.Edit;
...
function TlicgTable.Navigate(Buffer: TRecordBuffer; GetMode: TGetMode; doCheck: Boolean):
  TGetResult;
var
  Acceptable: Boolean;
begin
...
function TlicgTable.Find(const Expression: string; Direction: TlicgDirection;
  Origin: TlicgOrigin): Boolean;
begin
  result := false;
...
procedure TlicgTable.Notification(AComponent: TComponent; Operation: toperation);
begin
...
function TlicgTable.FilterRecord(Buffer: TRecordBuffer): Boolean;
var
...
begin
  ...
  begin
    if (FCurRec < 0) or (FCurRec >= FRecordCount) or (RecordCount = 0) then
      Exit;
...
procedure TlicgTable.OrderBy(const Expression: string; Descending: Boolean = False);
var
...
procedure TlicgTable.BufferFilter(Buffer: IlicgEntity; operator:
  TlicgGraphicOperator; const QueryExpression: string; CurvePoints: Integer;
  const Distance: Double; ClearBefore: Boolean);//: 17.10.2007
begin
  SetBaseLayer;
  if (FLayer = nil) or (FGIS = nil) then
    Exit;
  FGIS.QuerySelectTool.QueryBuffer(Buffer, FLayer.Name, QueryExpression,
    operator, CurvePoints, Distance, FGraphicFilterList, ClearBefore);
end;
...
begin
  ...
  if (FLayer = nil) or (FGIS = nil) then
    Exit;
  FGIS.QuerySelectTool.QueryExpression(FLayer.Name, Scope, FGraphicFilterList,
    ClearBefore);
  FGraphicFiltered := True;
end;

procedure TlicgTable.PolygonFilter(Polygon: IlicgEntity; operator:
  TlicgGraphicOperator; const QueryExpression: string; ClearBefore: Boolean);
begin
  SetBaseLayer;
  if (FLayer = nil) or (FGIS = nil) then
    Exit;
  FGIS.QuerySelectTool.QueryPolygon(Polygon, FLayer.Name, QueryExpression,
    operator, FGraphicFilterList, ClearBefore);
  FGraphicFiltered := True;
end;

procedure TlicgTable.RectangleFilter(const AxMin, AyMin, AxMax, AyMax: Double;
  operator: TlicgGraphicOperator; const QueryExpression: string; ClearBefore:
  Boolean);//: 17.10.2007
begin
  SetBaseLayer;
  if (FLayer = nil) or (FGIS = nil) then
    Exit;
  FGIS.QuerySelectTool.QueryRectangle(Axmin, Aymin, Axmax, Aymax, FLayer.Name,
    QueryExpression, operator, FGraphicFilterList, ClearBefore);
  FGraphicFiltered := True;
end;

procedure TlicgTable.PolylineIntersects(Polyline: IlicgEntity; const
  QueryExpression: string; ClearBefore: Boolean);
begin
  SetBaseLayer;
  if Assigned(FLayer) and Assigned(FGIS) then
  begin
    FGIS.QuerySelectTool.QueryPolygon(Polyline, FLayer.Name, QueryExpression,
      goIntersects, FGraphicFilterList, ClearBefore);
    FGraphicFiltered := True;
  end;
end;

procedure TlicgTable.FilterFromLayer(SourceLayer: TlicgBaseLayer; const
  QueryExpression: string; operator: TlicgGraphicOperator; ClearBefore: Boolean);//: 17.10.2007
var
  E: IlicgEntity;
  WasInit: Boolean;
...
begin
  ...
  begin
    ...
    try
      try
        if not (E.EntityID in [idPolyline, idPolygon, idPolyArc, idPolyEllipse,
          idArc, idEllipse, idSpline]) then
        begin
          Sourcelayer.Next;
          Continue;
        end;
        ...
        begin
          ...
          DoClearBefore := false;
        if E.IsPolygon then
          FGIS.QuerySelectTool.QueryPolygon(E, Sourcelayer.Name, QueryExpression,
            operator, FGraphicFilterList, DoClearBefore)
        else
          FGIS.QuerySelectTool.QueryPolygon(E, Sourcelayer.Name, QueryExpression,
            goIntersects, FGraphicFilterList, DoClearBefore);
      finally
        E := nil;
      end;
    finally
...
function TlicgDesignTable.Navigate(Buffer: TRecordBuffer; GetMode: TGetMode; doCheck:
  Boolean): TGetResult;
begin
  if FRecordCount < 1 then
    Result := grEOF
