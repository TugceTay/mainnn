...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoLayerWMS ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayerWMS"'}
{$ENDIF}
{$IFDEF CLR}
  ...
  begin
    ...
    if FUseLayers.Count > 0 then begin
      ...
      for i := 0 to oWMS.AllLayers.Count - 1 do begin
        ...
        lv.UseParentParams    := False                       ;
        lv.ParentLayer        := Self                        ;
        lv.Active             := oWMS.AllLayers[ i ] = oWMS.DefaultLayer ;
  ...
  begin
    ...
    try
      ...
      if assigned( SubLayers ) then begin
        slays.Clear ;
        for j := 0 to SubLayers.Count - 1 do begin
          if TGIS_Layer( SubLayers[ j ] ).Active then begin
            slays.Add( TGIS_Layer( SubLayers[ j ] ).Name ) ;
            sstyles.Add( TGIS_Layer( SubLayers[ j ] ).Params.Style ) ;
          end ;
        end ;
      end ;
...
//?        oImage.workLayerList := oLayerList ;
  ...
  begin
    ...
    if ( w > 10 ) and ( h > 10 ) then begin
      if assigned( SubLayers ) then begin
        FUseLayers.Clear ;
        for j := 0 to SubLayers.Count - 1 do begin
          if TGIS_Layer( SubLayers[ j ] ).Active then
            FUseLayers.Add( TGIS_Layer( SubLayers[ j ] ).Name ) ;
        end ;
      end ;
  ...
  class procedure GisLayerWMS.SelfRegisterLayer() ;
  begin
