...
{$IFDEF DCC}
  unit GisLayerGRD ;
  {$HPPEMIT '#pragma link "GisLayerGRD"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
     ...
     if reversedRowOrder then
      ...
      for j := 0 to FBitWidth -1 do begin
        if isBinary then begin
          ...
          if (AnsiChar(fileToken[StringFirst]) in ['-', '+'])
             and (not (AnsiChar(fileToken[StringFirst +1]) in [ '0'..'9'])) then
            val6 := FNoDataValue
          else
  ...
  class procedure Unit_GisLayerGRD.SelfRegisterLayer() ;
  begin
...
{$IFNDEF OXYGENE}
  initialization
    Unit_GisLayerGRD.SelfRegisterLayer() ;
{$ENDIF}
