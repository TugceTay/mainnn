...
{$IFDEF DCC}
  unit GisFileJSON ;
  {$HPPEMIT '#pragma link "GisFileJSON"'}
{$ENDIF}
{$IFDEF CLR}
    ...
    begin
      case stk[ lvl ].curState of
        TGIS_JSONTokenerState.tsEatws :
          begin
            if ( c = #32 ) or (c = #8) or (c=#9) or (c=#10) or (c =#12) or (c =#13) then
          ...
          case c of
            ...
            'u':
              begin
                ucsChar := 0 ;
                stPos := 0 ;
                stk[ lvl ].curState := TGIS_JSONTokenerState.tsEscapeUnicode ;
                exit ;
              end;
            else
              exit;
          end ;
        TGIS_JSONTokenerState.tsEscapeUnicode:
          begin
            if (ord(c) < 235) and
               (
                 ( ( c >= '0') and (c <'9') )
            ...
            then begin
              inc( ucsChar, (Word(hexdigit(c)) shl ((3-stPos)*4)) );
              inc( stPos ) ;
              if (stPos = 4) then begin
                writer.Append( Char(ucsChar) );
                stk[ lvl ].curState := stk[ lvl ].lastState ;
                exit ;
              end
            end ;
