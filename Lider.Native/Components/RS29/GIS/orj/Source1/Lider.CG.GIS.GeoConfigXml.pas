...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoConfigXml ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoConfigXml"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
  ...
  begin
    ...
    for i := 0 to vwr.Items.Count-1 do begin
      ...
      if assigned( ll.SubLayers ) then
        for j := 0 to ll.SubLayers.Count-1 do begin
          ls := TGIS_Layer( ll.SubLayers[ j ] ) ;
