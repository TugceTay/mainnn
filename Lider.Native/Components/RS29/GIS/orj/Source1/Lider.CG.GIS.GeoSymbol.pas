...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoSymbol ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoSymbol"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_SymbolLineEx = {$IFDEF OXYGENE} public {$ENDIF} class( TGIS_SymbolAbstract )
    ...
    {$IFDEF OXYGENE} assembly or protected {$ELSE} protected {$ENDIF}
      ...
      /// </param>
      procedure DrawLine       ( const _drawbuf : TGIS_DrawBuf ;
                                 const _cnt     : Integer
                               ) ;
...
//==============================================================================
  ...
  // line (vector). Version based on integers.
  ...
  begin
    ...
    // check for being on vector
       m2 := Sqr( 1.0 * Int( _ptB.X -_ptA.X ) ) +
             Sqr( 1.0 * Int( _ptB.Y -_ptA.Y ) ) ;
       if m2 > 0 then begin
         r := 1.0 * ( Int(_ptA.Y - _pt1.Y) * Int(_ptA.Y -_ptB.Y) -
                      Int(_ptA.X - _pt1.X) * Int(_ptB.X -_ptA.X)
                    ) / m2 ;
         ...
         if Result = 2 then begin
           r := 1.0 * ( Int(_ptA.Y - _pt2.Y) * Int(_ptA.Y -_ptB.Y) -
                        Int(_ptA.X - _pt2.X) * Int(_ptB.X -_ptA.X)
                      ) / m2 ;
           if (r < 0) or (r > 1) then begin
             dec( Result ) ;
           end ;
...
//==============================================================================
  ...
  var
    ...
    procedure set_buffer( const _pos : Integer ; const _pt : TPoint ) ;
    begin
      if _pos > size then begin
        inc( size ) ;
    ...
    begin
      ...
      cnt := 0 ;
      pt_A := _TPoint( _drawbuf[0] ) ;
      set_buffer( cnt, pt_A ) ;
      inc( cnt ) ;
      j :=1 ;
      pt_tmp := _TPoint( pt_A ) ;

      while j < _cnt do begin
        pt_B := _TPoint( _drawbuf[j] ) ;

        d := getLineCrossCircle( pt_A, pt_B, pt_tmp, wdth, tmp_a, tmp_b ) ;

        if d <= 0 then begin
          if Sqrt( Sqr( 1.0 * Int(pt_B.X - pt_tmp.X) ) +
                   Sqr( 1.0 * Int(pt_B.Y - pt_tmp.Y) )
                 ) <= wdth then
          begin
            pt_A := _TPoint( pt_B ) ;
            inc( j ) ;
        ...
        pt_tmp := _TPoint( tmp_a) ;
        set_buffer( cnt, pt_tmp ) ;
        inc( cnt ) ;

        dist_B := Sqrt( Sqr( 1.0 * Int(pt_B.X - pt_tmp.X) ) +
                        Sqr( 1.0 * Int(pt_B.Y - pt_tmp.Y) )
                      ) ;

        if wdth < dist_B then begin
          d := getLineCrossCircle( pt_tmp, pt_B, pt_tmp,
                                   TruncS(dist_B / wdth) * wdth, tmp_a, tmp_b
                                 ) ;
          if d = 0 then
            break ;
          pt_tmp := _TPoint( tmp_a ) ;
      ...
      set_buffer( cnt, _drawbuf[_cnt-1] ) ;
      inc( cnt ) ;
    end ;
  ...
  begin
    ...
    rangle := IGIS_Viewer(_viewer).RotationAngle ;

    for i:=0 to cnt - 2 do begin
      angle := ArcTan2( buf[i+1].Y - buf[i].Y,
                        buf[i+1].X - buf[i].X ) ;

      _sym.prepareRotate( angle - rangle, False ) ;
      len := Sqrt( Sqr( buf[i+1].Y - buf[i].Y ) +
                   Sqr( buf[i+1].X - buf[i].X )
                 ) ;
      ...
      dy := sa * wdth ;
      j := 0 ;
      while len >= wdth -1  do begin
        x := TruncS( j * dx ) + buf[i].X ;
        y := TruncS( j * dy ) + buf[i].Y ;

        if vistest then begin
          _sym.prepareRotate( angle + _sym.symAngle - rangle, False ) ;
          _sym.Draw( x, y ) ;
        end ;
        len := len - wdth ;
        inc( j ) ;
  ...
  var
    ...
    begin
      ...
      buf[ cnt ] := Point( RoundS( pt1.X ), RoundS( pt1.Y ) ) ;
      inc( cnt ) ;
    end ;
...
//==============================================================================
  ...
  var
    l : Integer ;
    pt : TPoint {$IFDEF GIS_NORECORDS} := new TPoint(0, 0) {$ENDIF} ;
    sa, ca : Single ;
  begin
    l := length( outBuf ) ;
  ...
  begin
    ...
    if  _dx >= 0 then begin // search forward
      for nd := curNode + 1 to length( lineBuf ) - 1 do begin
        pt_A := _TPoint( arDrawBuf[ nd-1 ] ) ;
        pt_B := _TPoint( arDrawBuf[ nd   ] ) ;
