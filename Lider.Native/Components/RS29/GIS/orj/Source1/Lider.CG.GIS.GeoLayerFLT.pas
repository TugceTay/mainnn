...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoLayerFLT ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayerFLT"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
  ...
  procedure TGIS_LayerFLT.prepareMinMaxZ( const _zoom : Double = -1 ) ;
  var
    i, k : Integer ;
    {$IFDEF OXYGENE}
      j  : Integer ;
  ...
  const
    MAX_LINES = 750 ;
  begin
    if not assigned( fileStream ) then exit ; // call Alive ?
    ...
    for i := 0 to FBitHeight -1 do begin
      ...
      if fact >= 1 then begin
        fact := fact - 1 ;
        fileStream.Seek( Int64(bytes) * i, soBeginning ) ;
        {$IFDEF OXYGENE}
          for j := 0 to FBitWidth-1 do
            fileStream.ReadSingle( lineBuffer[j], sizeOf(Single) ) ;
        {$ELSE}
          fileStream.Read( ( Addr(lineBuffer[0]))^, bytes ) ;
        {$ENDIF}
        if bigEndian then
          convertLineToLE ;
        for k := 0 to FBitWidth -1 do begin
          if lineBuffer[k] <> NoDataValue then
          begin
  ...
  class procedure GisLayerFLT.SelfRegisterLayer() ;
  begin
