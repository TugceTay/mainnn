...
unit VCL.GisRendererGdi32;
{$HPPEMIT '#pragma link "VCL.GisRendererGdi32"'}
  ...
  var
    ...
    begin
      ...
      if ( FStyle = TGIS_PenStyle.Clear ) or ( FWidth = 0 ) then
        ...
        // verify
        assert( assigned( FBitmap.GetData( NativeBitmapFactory ) ) ) ;
        FreeObject( FStretchedBitmap ) ;
        lb.lbStyle := BS_PATTERN ;
        // verify
        bmp := _rnd.prepareBitmapFill( TBitmap( FBitmap.GetData( NativeBitmapFactory ) ) ) ;
        // verify
        if bmp <> TBitmap( FBitmap.GetData( NativeBitmapFactory ) ) then
...
//=============================================================================
  ...
  begin
    ...
    if not _useBaseMap and Context.SelectionOnDemand then
      Context.AssignSelection(
        TGIS_Bitmap.Create( Width, Height ),
        True
      ) ;
    ...
    if not _useBaseMap and assigned( Context.Selection ) then begin
      ...
      oSelectionCanvas  := TGIS_CanvasInternal.Create(
                             TBitmap(TGIS_Bitmap( Context.Selection ).GetData( BitmapFactory )).Canvas
                           ) ;
    end
    else begin
  ...
  begin
    ...
    if assigned( Context.Charts ) then begin
      ...
      oChartsCanvas  := TGIS_CanvasInternal.Create(
                          TBitmap(TGIS_Bitmap( Context.Charts ).GetData( BitmapFactory )).Canvas
                        ) ;
    end
    else begin
  ...
  begin
    ...
    if assigned( Context.Labels ) then begin
      ...
      oLabelsCanvas  := TGIS_CanvasInternal.Create(
                          TBitmap(TGIS_Bitmap( Context.Labels ).GetData( BitmapFactory )).Canvas
                        ) ;
    end
    else begin
  ...
  function TGIS_RendererVclGdi32.fget_BitmapFactory
    : TGIS_BitmapFactory ;
  begin
    Result := NativeBitmapFactory ;
  end ;
  ...
  begin
    ...
    oCanvas := TGIS_CanvasInternal.Create(
                 TBitmap(TGIS_Bitmap( Context.BaseMap ).GetData( BitmapFactory )).Canvas
               ) ;
  ...
  procedure TGIS_RendererVclGdi32.PrepareDrawCharts ;
  begin
    prevCanvas := FCanvas;
    prepareChartsCanvas;
  ...
  procedure TGIS_RendererVclGdi32.AfterDrawCharts ;
  begin
    FCanvas := prevCanvas ;
  end;

  procedure TGIS_RendererVclGdi32.PrepareDrawLabels ;
  begin
    prevCanvas := FCanvas;
    prepareLabelsCanvas;
    FCanvas := oLabelsCanvas ;
  ...
  procedure TGIS_RendererVclGdi32.AfterDrawLabels ;
  begin
    FCanvas := prevCanvas ;
  end;
    ...
    if assigned( oTransparentBitmap ) then begin
      if assigned( oTransparentCanvas ) then begin
        ...
        blendBitmaps( oTransparentBitmap,
                      TGIS_Bitmap(Context.BaseMap).GetData( BitmapFactory ),
...
procedure TGIS_RendererVclGdi32.prepareFont(
  ...
  begin
    ...
    if T_Brush(_canvas.VclBrush).Style = TGIS_BrushStyle.Clear then begin
      old_bkmode := Winapi.Windows.SetBkMode( _canvas.Canvas.Handle, TRANSPARENT ) ;
      old_bkcolor := 0 ;
    end
    else begin
      ...
      begin
        ...
        repeat
          ...
          for point_no := 0 to part_size - 1 do begin
            ...
            if point_no = 0 then begin
              draw_bufF[isize] := PointF( pt_x, pt_y );
              last_pt_x := pt_x ;
              last_pt_y := pt_y ;
              inc( isize ) ;
            end
           ...
           begin
            ...
            then begin
              if assigned( params_line.Symbol ) then begin
                ...
                SetLength( draw_bufF, _shp.GetPartSize(part_no) ) ;
                cur_sizeF := prepare_drawbufpartF( iTolerance, part_no ) ;

                TGIS_SymbolLineHelper.DrawLine( Viewer, draw_buff,
                                                params_line.Symbol, cur_sizeF ) ;
  ...
  var
    ...
    var
      ...
      draw_bufF : TGIS_DrawBufF ;
      part_bufF : TGIS_IntegerArray ;
      hole_bufF : array of Byte ;
      act_part_no : Integer ;
      shp_size  : Integer ;
      max_tlrnc : Integer ;
      selwidth  : Integer ;
      ...
      begin
        ...
        SetLength( draw_bufF, _shp.GetNumPoints ) ;
        SetLength( part_bufF, _shp.GetNumParts  ) ;
        SetLength( hole_bufF, _shp.GetNumParts  ) ;
        ...
        for part_no := start to stop do begin
          ...
          repeat
            ...
            for point_no := 0 to part_size - 1 do begin
              ...
              if ( point_no = 0 ) then begin
                ...
                if tmp_tlrnc > cur_tlrnc then begin
                  if cur_size = 0 then begin
                    draw_bufF[isize] := PointF( first_pt_x, first_pt_y ) ;
                    inc( isize ) ;
                    inc( cur_size ) ;
                  end;
          ...
          if ( isize = 0 ) and ( part_size > 4 ) then begin
            // degenerated case
            draw_bufF[isize+0].X := pt_x ;
            draw_bufF[isize+0].Y := pt_y ;
            draw_bufF[isize+1].X := pt_x + 0;
            draw_bufF[isize+1].Y := pt_y + 1;
            draw_bufF[isize+2].X := pt_x + 1;
            draw_bufF[isize+2].Y := pt_y + 1;
            draw_buFf[isize+3].X := pt_x + 1;
            draw_bufF[isize+3].Y := pt_y + 0;
            draw_bufF[isize+4].X := pt_x ;
            draw_bufF[isize+4].Y := pt_y ;
            isize := isize+5 ;
            cur_size:= 5;
          end;
      ...
      try
        ...
        if not _selectionOnly then begin
          ...
          if assigned( params_area.OutlineSymbol ) then begin
            ...
            // draw parts
            k := 0 ;
            prepare_drawbufF( iTolerance ) ;
            for i := 0 to length(part_bufF)-1 do begin
              SetLength( pbuf, part_bufF[i] ) ;
              for j := 0 to part_bufF[i] - 1 do
                pbuf[j] := draw_bufF[k+j];
  ...
  begin
    ...
    rect_screen := Rect( 0, 0, Width, Height ) ;
    try
      draw_polygon ;
    except
    end ;
  ...
  procedure TGIS_RendererVclGdi32.doShapeMultiPatch(
    const _shp           : TGIS_ShapePolygon ;
    const _source        : TGIS_ShapePolygon ;
    const _selectionOnly : Boolean
  ) ;
  ...
  begin
    ...
    if Assigned( Context ) then
      // verify
      tbmp := TBitmap( TGIS_Bitmap(Context.BaseMap).GetData( BitmapFactory ) )
    else
      exit ;
  ...
  begin
    ...
    try
      // verify
      stretchBitmap( _bmp.GetData( BitmapFactory ), bmp, Rect( 0, 0, bmp.Width, bmp.Height ), 100 ) ;
    ...
    if Assigned( Viewer ) and Viewer.Editor.InEdit then begin
      if assigned( Viewer.Editor.CurrentShape ) then begin
        ...
        if assigned( TGIS_Shape( Viewer.Editor.CurrentShape ).Layer ) then begin
          ...
          if not ( _context is TGIS_Bitmap ) then exit ;

          drawEditingPoints(
            TBitmap( TGIS_Bitmap( _context ).GetData( BitmapFactory ) ).Canvas,
            TGIS_Shape( Viewer.Editor.CurrentShape )
          ) ;
          fix_bitmap( TBitmap( TGIS_Bitmap( _context ).GetData( BitmapFactory ) ) ) ;
        end ;
      end ;
    end ;
  end ;
  ...
  var
    ...
    begin
      ...
      if ( _context is TCanvas ) then begin
        ...
        oCanvasBitmap := TBitmap(TGIS_Bitmap( _context ).GetData( BitmapFactory )) ;
        FCanvas := TGIS_CanvasInternal.Create( oCanvasBitmap.Canvas ) ;
      end;
    end ;

    procedure do_extra ;
    begin
      if ( _context is TGIS_Bitmap )
         and
         assigned( TGIS_Bitmap(_context).NativeBitmap ) then begin
        fix_bitmap( oCanvasBitmap ) ;
      end ;
    end;
  ...
  function TGIS_RendererVclGdi32.FriendlyName
    : String ;
  begin
    Result := 'Gdi32' ;
  end ;

  function TGIS_RendererVclGdi32.ViewerPrepareCache(
    const _bitmap : TGIS_Bitmap
  ) : TGIS_Bitmap ;
  begin
    if TGIS_Bitmap.IsNilOrEmpty( _bitmap ) then
      ...
      Result := TGIS_Bitmap.Create( _bitmap.Width, _bitmap.Height, BitmapFactory ) ;
      TBitmap(Result.NativeBitmap).Assign( TPersistent(_bitmap.NativeBitmap) ) ;
    end ;
  end ;
  ...
  function TGIS_RendererVclGdi32.CanvasTextBaseline(
    const _text : String
  ) : Single ;
  begin
    assert( assigned( FCanvas ) ) ;
