...
{$IFDEF DCC}
  unit GisLayerIMG ;
  {$HPPEMIT '#pragma link "GisLayerIMG"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_LayerIMG = {$IFDEF OXYGENE} public {$ENDIF} class( TGIS_LayerPixel )
    ...
    private // various private values
        ...
        /// </summary>
        decompressedBlock : array [0..3] of TAAByte ;
        ...
        ///   Block in buffer
        /// </summary>
        blockInBuffer : array of Integer ;
...
//==============================================================================
      ...
      if data_type = EDSC_TABLE then begin
        if data_name = DESCRIPTOR_TABLE then begin
          ...
          while lchild.Next <> 0 do begin
            ...
            if lchild.Next >= fileStream.Size then
              break ;
          end ;
        end ;
      end
  ...
  begin
    ...
    if not assigned(decompressedBlock[0]) then begin
      for i := 0 to layersNo -1 do
        SetLength(decompressedBlock[i], blocksColumns, blocksize) ;
      SetLength(bandValidationFlag, layersNo) ;
      SetLength(blockInBuffer, blocksColumns) ;
      blockInBuffer[actualColumn] := _bidx ;
      for i := 0 to layersNo -1 do
        bandValidationFlag[i] := False ;
    end
    else begin
      if blockInBuffer[actualColumn] <> _bidx then begin
        blockInBuffer[actualColumn] := _bidx ;
        for i := 0 to layersNo -1 do
          bandValidationFlag[i] := False ;
      end
    ...
    if numruns = -1 then begin
      ...
      for pixels := 0 to blocksize -1 do begin
        ...
        begin
          decompressedBlock[_bandidx][actualColumn][pixels] := Byte(ndatavalue) ;
        end
        else
        ...
        for j := 0 to maxidx do begin
          ...
          for k := 0 to 7 - pixpos do begin
            if mask <> 0 then begin
              ...
              decompressedBlock[_bandidx][actualColumn][bytepos +j] :=
                            decompressedBlock[_bandidx][actualColumn][bytepos +j]  or mask ;
              mask := mask shr 1 ;
            end ;
            dec(repeatcount) ;
        ...
        if numbits = 16 then begin
          for k := 0 to nrepeatcount -1 do begin
            ...
            if buffer[dataidx +1] <> 0 then begin
              decompressedBlock[_bandidx][actualColumn][j] := buffer[dataidx +1] ;
            end
            else
              decompressedBlock[_bandidx][actualColumn][j] := buffer[dataidx] +mindata;
            inc(j) ;
          ...
          for k := 0 to nrepeatcount -1 do begin
            if dataidx > size then begin
              decompressedBlock[_bandidx][j] := 0 ;
              inc(j) ;
              if j >= blocksize  then
                break ;
              continue ;
...
//Bands managing
  ...
  begin
    ...
    for i := 0 to _pixCount -1 do begin
      ...
      if bitPalette[pal_idx].A <> 0 then
      ...
      begin
        ...
        if not_comressed then begin
          ...
          if bitsCountInfo = 16 then begin
            ...
            if blockInfoArray[_bandidx][bidx].Size = 0 then
            begin
              bitPalette[0] := TGIS_Color.FromARGB(00, bitPalette[0].R,
                                    bitPalette[0].G, bitPalette[0].B) ;
              for i := 0 to tilewidth -1 do
                bandsBBuffer[_bufidx][read +i] := 0 ;
            end
            else begin
            {$IFDEF OXYGENE}
          ...
          if (bitsCountInfo = 16)  then begin
            ...
            for i := 0 to tilewidth -1 do begin
              bandsBBuffer[_bufidx][read +i] :=
                  decompressedBlock[_bandidx][actualColumn][offset +i] ;
            end ;
          end ;
...
//==============================================================================
  ...
  class procedure Unit_GisLayerIMG.SelfRegisterLayer() ;
  begin
...
{$IFDEF DCC}
  initialization
    Unit_GisLayerIMG.SelfRegisterLayer() ;
{$ENDIF}
