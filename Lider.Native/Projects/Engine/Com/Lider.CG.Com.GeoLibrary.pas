...
function _IsEqual(const CoorArray1, CoorArray2: IlicgVector; const start1,
  start2, length1, length2: integer; const aEpsilon: double = _EPSILON): boolean; overload;
...
/// <summary>
///   Üçgeni, T[1] = minZ, T[3] = maxZ degerlerine gore sýralar, döndürür.
/// </summary>
procedure _SortTriangle(var T: TlicgTriangle3D);

/// <summary>
///   Fonksiyona gönderilen 3 nokta, x ve y yi içeren bir üçgenin kenarlarý
///   ise, nokta icinden bir Z degeri döndürür. <br />Eðer üçgen içinde
///   degilse, en yakýn olan deðer döndürülüyor. } <br /> DÝKKAT!! fonksiyona
///   gelen Üçgenin, sýralanmýþ olmasý gerekiyor.
/// </summary>
...
/// <summary>
///   Poligonun ilk ve son koorinatlarý eþit deðilse, ilk kordinatý son
///   koordinat olarak ekler.
/// </summary>
procedure NormalizePolygon(var AEntity: IlicgEntity);
...
begin
  Result := _Distance(Point1.X, Point1.Y, Point2.X, Point2.Y);
end;
...
function _IsEqual(const Coor1, Coor2: TlicgCoor; const aEpsilon: double = _EPSILON): boolean; overload;
begin
    Result := (_IsEqual(Coor1.x,Coor2.x,aEpsilon) and
	           _IsEqual(Coor1.y,Coor2.y,aEpsilon));
end;

function _IsEqual(const Coor1, Coor2: TlicgCoor3D; const aEpsilon: double = _EPSILON): boolean; overload;
begin
  Result := (_IsEqual(Coor1.x,Coor2.x,aEpsilon) and
             _IsEqual(Coor1.y,Coor2.y,aEpsilon) and
             _IsEqual(Coor1.z,Coor2.z,aEpsilon));
end;
...
begin
  Result := ( _IsEqual(Coor1.x,Coor2.x,aEpsilon) and
              _IsEqual(Coor1.y,Coor2.y,aEpsilon) and
              _IsEqual(Coor1.z,Coor2.z,aEpsilon) and
              _IsEqual(Coor1.N,Coor2.N,aEpsilon));
end;
...
// Poligonun ilk ve son koorinatlarý eþit deðilse, ilk kordinatý son koordinat olarak ekler.
procedure NormalizePolygon(var AEntity: IlicgEntity);
var
  I, Idx1, Idx2: Integer;
  AFound: Boolean;
begin
  if AEntity.EntityID <> idPolygon then
    Exit;
  with AEntity.Geometry.Points do
  begin
    if PartCount < 2 then
    begin
      if not Lider.CG.Com.Lib.FuzzEqualPoint2D(Coors2D[0], Coors2D[Count - 1]) then
        Add(Coors4D[0]);
    end
    else
    begin
      ...
      repeat
        ...
        if I < PartCount - 1 then
          ...
          Idx2 := Count - 1;
        repeat
          if not Lider.CG.Com.Lib.FuzzEqualPoint2D(Coors2D[Idx1], Coors2D[Idx2]) then
          begin
            if Idx2 < Count - 1 then
              Insert(Idx2 + 1, Coors2D[Idx1].X, Coors2D[Idx1].Y)
            else
              Add(Coors4D[Idx1]);
          ...
          if I < PartCount - 1 then
            ...
            Idx2 := Count - 1;
        until False;
      until (not AFound) or (I >= PartCount);
    end;
  end;
end;
...
begin
  ...
  try
    ...
    begin
      if Polyline1.PartEnd(PartIndex) = i then     //(* Oguz Ekleme *)
      begin
        inc(PartIndex);
        Continue;
      end;                                            //(* Oguz Ekleme Bitis *)
