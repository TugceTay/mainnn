...
type
  ...
  protected {My simplified methods to override}
    ...
    procedure DoGotoBookmark(Bookmark: TBookmark); virtual; abstract; //********
...
{$ENDIF}
  ...
  protected
    ...
    procedure DoGotoBookmark(Bookmark: TBookmark); override;
...
begin
  ...
  begin
    // ilker silme gerek yokFreeMem(PRecordInfo(Buffer + FDataSize)^.BookMark);
...
begin
  ...
  if (Result = grOk) then
  begin
    RecordToBuffer(TRecordBuffer(Buffer));
...
begin
  ...
  begin
    ...
    BufferToRecord(TRecordBuffer(GetActiveRecordBuffer));
...
begin
  ...
  BufferToRecord(TRecordBuffer(Buffer));
    ...
    case Fields[Index].DataType of
      ftString:
        begin
          ...
          //Application.MessageBox(PChar(TempStr), 'StrLCopy example', MB_OKCANCEL);
          StrLCopy(PChar(TRecordBuffer(Buffer + Offset)), PChar(TempStr), length(TempStr) * SizeOf(char)); // þüpheli
          //Application.MessageBox(Pchar(tRecordBuffer(Buffer + Offset)), 'StrLCopy example', MB_OKCANCEL);
        end;
      ftInteger, ftSmallInt, ftDate, ftTime:
        begin
          if VarIsNull(Value) then
          ...
          //StrLCopy(PChar(TRecordBuffer(Buffer + Offset)), PChar(TempStr), length(TempStr) * SizeOf(char)); // þüpheli
          //Application.MessageBox(Pchar((Buffer + Offset)), 'StrLCopy example', MB_OKCANCEL);
        end;
      ftFloat, ftBCD, ftCurrency, ftDateTime:
        begin
          if VarIsNull(Value) then
...
begin
  ...
  begin
    ...
    case Field.DataType of
      ...
      ftString:
        begin
          //StrLCopy(PChar(TRecordBuffer(Buffer + Offset)), PChar(TempStr), length(TempStr) * SizeOf(char));
          StrLCopy(PChar(TRecordBuffer(Buffer)), PChar(TRecordBuffer(RecBuffer + Offset)), StrLen(PChar(TRecordBuffer(RecBuffer + Offset))));
          StrPCopy(PChar(TRecordBuffer(Buffer)), TrimRight(StrPas(PChar(TRecordBuffer(Buffer))))); // þüpheli
...
begin
  if PRecordInfo(Buffer + FDataSize)^.BookMark = nil then
  begin
    SetLength(PRecordInfo(Buffer + FDataSize)^.BookMark, BookmarkSize);
    //GetMem(PRecordInfo(Buffer + FDataSize)^.BookMark, BookmarkSize);
  end;
  //Move(PRecordInfo(Buffer + FDataSize).BookMark, Data, BookmarkSize);
  // bu olabilir olmazsa kullan
  Move(Bookmark[0], Data[Low(TBookmarkStr)], BookmarkSize);
end;
...
begin
  // GIS sorgusundan dönen sonuçlar listeye GISBeforePaintEntity de  eklendiginden.
...
procedure TlicgTable.DoGotoBookmark(Bookmark: TBookmark);
begin
...
procedure TlicgDesignTable.DoGotoBookmark(Bookmark: TBookmark);
begin
  GotoRecordID(Pointer(PInteger(Bookmark)^));
end;
