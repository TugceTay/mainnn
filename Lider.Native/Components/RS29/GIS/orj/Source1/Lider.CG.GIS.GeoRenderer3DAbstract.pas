...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoRenderer3DAbstract ;
{$ENDIF}
{$IFDEF CLR}
...
//?      e      : ElapsedEventArgs
  ...
  begin
    ...
    {$ELSE}
    if GisIsContainExtent( exExtent, Lider.CG.GIS.GeoFunctions.GisExtent( -180, -90, 180, 90 ) )
    {$ENDIF}
    then begin
      Result := True ;
      exit;
...
//      basePlane.Level := rangeExtent.ZMin - Max( zUnit, (rangeExtent.ZMax - rangeExtent.ZMin) / 10.0) ;
  ...
  var
    ...
    begin
      ...
      if not assigned( arBufSelInfo[_k].ShpInfo) then
