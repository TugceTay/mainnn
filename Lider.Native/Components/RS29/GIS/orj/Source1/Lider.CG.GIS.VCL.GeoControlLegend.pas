...
unit Lider.CG.GIS.VCL.GeoControlLegend ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.VCL.GeoControlLegend"'}
...
type
  ...
  TGIS_ControlLegend = class ( TCustomControl,
    ...
    private
      oLegend              : TGIS_Legend ;
      oRenderer            : TGIS_LegendRendererVCL ;
    ...
    protected // IGIS_LegendParent
      ...
      /// <inheritdoc from="IGIS_LegendParent"/>
      procedure ControlStyleDrawImage      ( const _bitmap   : TObject ;
                                             const _left     : Integer ;
                                             const _top      : Integer ;
                                             const _transparent
                                                             : Boolean
...
uses
  ...
  Lider.CG.GIS.VCL.GeoControlLegendForm,
  Lider.CG.GIS.VCL.GeoPrintLegend ;
...
//==============================================================================
  ...
  procedure TGIS_ControlLegend.fset_SelectedNode(
    const _value : TGIS_TreeNode
  ) ;
  begin
    oLegend.SelectedNode := _value ;
  end ;
...
//==============================================================================
      ...
      vkRight    :
        begin
          ...
          if ( not SelectedNode.Expanded ) or ( SelectedNode.Count = 0 ) then
            exit ;
          SelectedNode := SelectedNode.Items[0] ;
          Repaint ;
        end ;
      vkPrior    :
        begin
  ...
  begin
    ...
    try
      oLegend.MouseDown( _x, _y, False, handled ) ;
    finally
      if not handled then
        oGestHlpr.GestureMouseDown(
          ssShift  in _shift,
...
//==============================================================================
  ...
  begin
    ...
    oRenderer := TGIS_LegendRendererVCL.Create( Self ) ;
    fnt := TFont.Create ;
    try
    ...
    FBiDiModeFromTranslation := False ;
...
//==============================================================================
  ...
  begin
    ...
    TGIS_ControlLegend(Result).Font.Name := oRenderer.GetFont.Name ;
    TGIS_ControlLegend(Result).Font.Size := oRenderer.GetFont.Size ;
    TGIS_ControlLegend(Result).Font.Style := oRenderer.GetFont.Style ;
    TGIS_ControlLegend(Result).Font.Color := oRenderer.GetFont.Color ;
  end ;
  ...
  begin
    ...
    try
      pl.Draw ;
      Clipboard.Assign( Vcl.Graphics.TBitmap( bmp.NativeBitmap ) ) ;
    finally
      FreeObject( pl ) ;
      FreeObject( bmp ) ;
    end ;
