...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoLayerKML ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayerKML"'}
{$ENDIF}
{$IFDEF CLR}
...
const
   ...
   // Prefixex
    ...
    KML_TATUKGIS_SYMBOL_PATH = 'https://www.lideryazilim.com/Library/Symbols/KML/' ; // ilker deðiþtirme
  ...
  begin
    ...
    try
      sattributes.AddAttribute( KML_NOVAL, KML_NOVAL, KML_ID,
                                KML_NOVAL, 'LicadGIS_Style' // ilker deðiþtirme
                              ) ;
        ...
        if not IsStringEmpty( Path ) then begin
          if SafeFileExists( Path ) or IsServerPath( Path ) then begin
            ...
            if ext = '.KMZ' then begin
              bIsKMZ := True ;
              mstm := DecompressZipFile( Path, Name ) ;
              try
                mstm.Position := 0 ;
