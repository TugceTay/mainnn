...
{$IFDEF DCC}
   unit Lider.CG.GIS.GeoLayer ;
   {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayer"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_Layer = {$IFDEF OXYGENE} public abstract {$ENDIF}
    ...
    public
      ...
      /// <summary>
      ///   Dormant mode for the layer. Use: Lider.CG.GIS.GeoLayerDormantOff to turn
      ///   off to conserve memory, Lider.CG.GIS.GeoLayerDormantStandard to do standard
      ///   memory management, and Lider.CG.GIS.GeoLayerDormantAgressive to free as much
      ...
      if SetSection( 0, False ) then begin
        ...
        if assigned( SubLayers ) then begin
          for i := 0 to SubLayers.Count - 1 do begin
            ...
            SetSubSection( 0, False ) ;
            TGIS_Layer( SubLayers[ i ] ).applyConfigOptions(
              Viewer.Ref.ProjectFile
            ) ;
            TGIS_Layer( SubLayers[ i ] ).ParamsList.LoadFromConfig(
              Viewer.Ref.ProjectFile,
              True
            ) ;
          end ;
