...
unit Lider.CG.GIS.GeoDataSet ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.GeoDataSet"'}
...
type
  ...
  TGIS_DataSet = class (TDataSet, IGIS_Subscribe)
    ...
    public
      ...
      /// </returns>
      function  GotoShape             ( const _uid          : Integer
                                      ) : Boolean ; virtual ;
    public
      /// <summary>
      ...
      /// </summary>
      property ShowInternalFields : Boolean read  bShowInternalFields
                                            write bShowInternalFields ;
      /// <summary>
    ...
    published
      ...
      /// <remarks>
      ///   <note type="note">
      ///    If not assigned then TGIS_Viewer.OnBusy will be used.
      ///    </note>
      /// </remarks>
...
//==============================================================================
  ...
  function  TGIS_DataSet.getUid : Integer ;
  var
    rec : T_dsRecord ;
    r   : TRecInfo   ;
  begin
  ...
  begin
    ...
    try
      ...
      for i := 0 to Fields.Count-1 do begin
        fld := Fields[i] ;
        if ( fld.FieldName = GIS_FIELD_UID      ) or
           ( fld.FieldName = GIS_FIELD_LENGTH   ) or
           ( fld.FieldName = GIS_FIELD_AREA     ) then
          fld.ReadOnly := True
        else if bReadOnly then
          fld.ReadOnly := True
        else if Assigned( dsLayer ) then begin
          p := dsLayer.FindField( fld.FieldName ) ;
          if p >= 0 then
            if dsLayer.FieldInfo(p).ReadOnly then
              fld.ReadOnly := True ;
        end;
      end ;
  ...
  begin
    ...
    currShape := dsLayer.GetShape( getUid ).MakeEditable ;
  ...
  begin
    ...
    if bShowInternalFields then begin
      FieldDefs.Add( GIS_FIELD_UID     , Data.DB.ftInteger          ) ;
      FieldDefs.Add( GIS_FIELD_LENGTH  , Data.DB.ftFloat  , 0, True ) ;
      FieldDefs.Add( GIS_FIELD_AREA    , Data.DB.ftFloat  , 0, True ) ;
      FieldDefs.Add( GIS_FIELD_SELECTED, Data.DB.ftBoolean, 0, True ) ;
      FieldDefs.Add( GIS_FIELD_HIDDEN  , Data.DB.ftBoolean, 0, True ) ;
    end ;
  ...
  begin
    ...
    try
      ...
      if not (State in [dsCalcFields, dsFilter, dsNewValue]) then
        DataEvent(deFieldChange, Longint(_field));
    except
    end ;
  end ;
  ...
  begin
    ...
    if CompareText( _field.FullName, GIS_FIELD_UID ) = 0 then begin
      vinteger := uid ;
...
//==============================================================================
  ...
  constructor T_dsRecord.Create(const _uid : Integer) ;
  begin
    Uid := _uid ;
  end ;
