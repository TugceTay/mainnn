...
unit FMX.GisViewerWnd;
{$HPPEMIT '#pragma link "FMX.GisViewerWnd"'}
...
type
  ...
  TGIS_ViewerWnd = class( TStyledControl, IGIS_Viewer, IGIS_ViewerParent,
    ...
    private
      ...
      oTilerHelper      : TGIS_TilerHelper ;
      oTilerExtent      : TGIS_Extent ;
...
//==============================================================================
  ...
  begin
   ...
   if InPaint and not TiledPaint then begin
    ...
    if oWndHelper.UpdateCancelOptional and not TiledPaint then
      exit ;
  ...
  procedure TGIS_ViewerWnd.Paint ;
    ...
    begin
      ...
      r1.X := ( ctx.Extent.XMax - ctx.Extent.XMin ) / ControlCanvasWidth  ;
      r1.Y := ( ctx.Extent.YMax - ctx.Extent.YMin ) / ControlCanvasHeight ;
    ...
    begin
      ...
      try
        ...
        then begin
          ...
          try
            ...
            try
                    ...
                    try
                      for sc := 0 to tile.Count -1 do begin
                        ...
                        try
                          ...
                          Canvas.DrawBitmap(
                            T_FMXBitmap( tile.Subtiles[sc].Bitmap.NativeBitmap ),
                            RectF( 0, 0, oTilerHelper.TileSize, oTilerHelper.TileSize ),
                            oWndHelper.ScaledRect(
                              tile.Extent,
              ...
              if not oVwr.IsEmpty and
                 assigned( FTileContext  )
                 and
                 ( assigned( FTileContext.Basemap   ) or
                   assigned( FTileContext.Selection ) or
                   assigned( FTileContext.Charts    ) or
                   assigned( FTileContext.Labels    ) or
                   oVwr.IsTopmost or
                   assigned( FFlashContext.BaseMap )
                 ) then
              begin
                if assigned( FTileContext.Selection ) then begin
                  if contextNotScaled( ctx ) then
                    cnv.DrawBitmap(
                      T_FMXBitmap(FTileContext.Selection),
                      RectF( 0, 0,
                        T_FMXBitmap(FTileContext.Selection).Width,
                        T_FMXBitmap(FTileContext.Selection).Height
                      ),
                      RectF( 0, 0,
                        T_FMXBitmap(FTileContext.Selection).Width,
                        T_FMXBitmap(FTileContext.Selection).Height
                ...
                if assigned( FTileContext.Charts ) then
                  cnv.DrawBitmap(
                    T_FMXBitmap(FTileContext.Charts),
                    RectF( 0, 0,
                           T_FMXBitmap(FTileContext.Charts).Width,
                           T_FMXBitmap(FTileContext.Charts).Height
                         ),
                    scaledRect(ctx),
                    1, True
                  ) ;
                if not metPaintLabelsOnTop then begin
                  if assigned( FTileContext.Labels ) then
                    cnv.DrawBitmap(
                      T_FMXBitmap(FTileContext.Labels),
                      RectF( 0, 0,
                             T_FMXBitmap(FTileContext.Labels).Width,
                             T_FMXBitmap(FTileContext.Labels).Height),
                      scaledRect(ctx),
                      1, True
                    ) ;
                end ;
          ...
          if valid_bmp(oFullCache) then begin
            Canvas.DrawBitmap(
              oFullCache,
              RectF( 0, 0, oFullCache.Width, oFullCache.Height ),
              oWndHelper.ScaledRect( 1 ),
              1, False
            );
          end;
