...
unit VCL.GisRenderer;
{$HPPEMIT '#pragma link "VCL.GisRenderer"'}
    ...
    begin
      ...
      begin
        ...
        if PInteger( buf_src )^ <> 0 then begin
          ...
          PByte( buf_dst+2 )^ := Byte(PByte( buf_src+2 )^ +
                                 PByte( buf_dst+2 )^ * saneg shr 8) ;
          PByte( buf_dst+1 )^ := Byte(PByte( buf_src+1 )^ +
                                 PByte( buf_dst+1 )^ * saneg shr 8) ;
          PByte( buf_dst   )^ := Byte(PByte( buf_src   )^ +
...
procedure TGIS_RendererVclAbstract.PaintExtra(
...
begin
  if assigned( _event ) then begin
    if _context is TCanvas then
      _event( _sender, _context ) ;
    if _context is TGIS_Bitmap then
      _event( _sender, VCL.Graphics.TBitmap( TGIS_Bitmap(_context).NativeBitmap ).Canvas ) ;
  end ;
end ;

procedure TGIS_RendererVclAbstract.ViewerDrawBackground(
  const _canvas : TObject ;
  const _width  : Integer ;
  const _height : Integer ;
  const _color  : TGIS_Color
...
begin
  ...
  if not ( _canvas is TCanvas ) then exit ;

  TCanvas(_canvas).Brush.Style := bsSolid ;
  TCanvas(_canvas).Brush.Color := VCLColor( _color ) ;
  TCanvas(_canvas).FillRect( Rect( 0, 0, _width, _height ) ) ;
end ;

function TGIS_RendererVclAbstract.ViewerPrepareCache(
  const _bitmap : TGIS_Bitmap
) : TGIS_Bitmap ;
begin
  if TGIS_Bitmap.IsNilOrEmpty( _bitmap ) then
    Result := TGIS_Bitmap.Create
  else
    Result := TGIS_Bitmap.Create( TGIS_Bitmap( _bitmap ) ) ;
end ;

function TGIS_RendererVclAbstract.ViewerCreateTemporaryPaint(
  const _width   : Integer ;
  const _height  : Integer ;
  const _context : TObject
) : TObject ;
begin
  ...
  // Result is TGIS_Bitmap

  Result := TGIS_Bitmap.Create( _width, _height ) ;
end ;

function TGIS_RendererVclAbstract.ViewerCreateTemporaryPaintEx(
  const _width   : Integer ;
  const _height  : Integer ;
  const _context : TObject
) : TObject ;
begin
  Result := ViewerCreateTemporaryPaint( _width, _height, _context ) ;
end ;
...
begin
  VCL.Graphics.TBitmap( TGIS_Bitmap( _context ).NativeBitmap ).Canvas.Lock ;
  Result := _context ;
end ;

procedure TGIS_RendererVclAbstract.ViewerFreeTemporaryPaint(
  var _paint : TObject
) ;
begin
  // _paint is TGIS_Bitmap

  if not ( _paint is TGIS_Bitmap ) then exit ;
...
begin
  _paint := nil ;
  VCL.Graphics.TBitmap( TGIS_Bitmap(_bitmap).NativeBitmap ).Canvas.Unlock ;
end ;

procedure TGIS_RendererVclAbstract.ViewerClearTemporaryPaint(
  const _paint : TObject
) ;
begin
  // _paint is TGIS_Bitmap
  ...
  if not ( _paint is TGIS_Bitmap ) then exit ;
  if TGIS_Bitmap.IsNilOrEmpty( TGIS_Bitmap( _paint ) ) then exit ;
  if ( TGIS_Bitmap( _paint ).Width = 0 )
     or
     ( TGIS_Bitmap( _paint ).Height = 0 ) then exit ;

  clearTransparentBitmap( TGIS_Bitmap( _paint ).NativeBitmap ) ;
end ;

procedure TGIS_RendererVclAbstract.ViewerFlushTemporaryPaint(
  const _canvas : TObject ;
  const _width  : Integer ;
  const _height : Integer ;
  const _paint  : TObject ;
...
begin
  ...
  Winapi.Windows.AlphaBlend(
    TCanvas(_canvas).Handle,
    0, 0, _width, _height,
    VCL.Graphics.TBitmap( TGIS_Bitmap(_paint).NativeBitmap ).Canvas.Handle,
...
begin
  ViewerFlushTemporaryPaint( _canvas, _width, _height, _paint, _rect ) ;
end ;

procedure TGIS_RendererVclAbstract.ViewerStretchBitmapFast(
  const _bitmap : TObject ;
  const _paint  : TObject ;
  const _rect   : TRect
) ;
begin
  ...
  if ( TGIS_Bitmap( _bitmap ).Width = 0 )
     or
     ( TGIS_Bitmap( _bitmap ).Height = 0 ) then exit ;
  if not ( _paint is TGIS_Bitmap ) then exit ;
  if TGIS_Bitmap.IsNilOrEmpty( TGIS_Bitmap( _paint ) ) then exit ;
  if ( TGIS_Bitmap( _paint ).Width = 0 )
     or
  ...
  stretchBitmapFast(
    TGIS_Bitmap(_paint).NativeBitmap,
    TGIS_Bitmap(_bitmap).NativeBitmap,
    _rect
  ) ;
end ;
...
begin
  ViewerStretchBitmapFast( _bitmap, _paint, _rect ) ;
end ;

procedure TGIS_RendererVclAbstract.ViewerBlendBitmaps(
  const _src          : TObject ;
  const _dst          : TObject ;
  const _transparency : Integer ;
  const _merge_alpha  : Boolean
...
begin
  ...
  if ( TGIS_Bitmap( _src ).Width = 0 )
     or
     ( TGIS_Bitmap( _src ).Height = 0 ) then exit ;
  if not ( _dst is TGIS_Bitmap ) then exit ;
  if TGIS_Bitmap.IsNilOrEmpty( TGIS_Bitmap( _dst ) ) then exit ;
  if ( TGIS_Bitmap( _dst ).Width = 0 )
     or
     ( TGIS_Bitmap( _dst ).Height = 0 ) then exit ;

  blendBitmaps(
    TGIS_Bitmap(_src).NativeBitmap,
    TGIS_Bitmap(_dst).NativeBitmap,
    _transparency,
    _merge_alpha
  ) ;
end ;

procedure TGIS_RendererVclAbstract.ViewerBlendLabelBitmaps(
  const _src         : TObject ;
  const _dst         : TObject ;
  const _merge_alpha : Boolean
) ;
...
function TGIS_RendererVclAbstract.ViewerCreateFullCache(
  const _width  : Integer ;
  const _height : Integer ;
  const _canvas : TObject ;
  const _paint  : TObject
...
begin
  // _canvas is TGIS_Bitmap
  // _paint is not used

  Result := nil ;
  if not ( _canvas is TGIS_Bitmap ) then exit ;
  if TGIS_Bitmap.IsNilOrEmpty( TGIS_Bitmap( _canvas ) ) then exit ;
  if ( TGIS_Bitmap( _canvas ).Width = 0 )
     or
     ( TGIS_Bitmap( _canvas ).Height = 0 ) then exit ;

  Result := TGIS_Bitmap.Create( _width, _height ) ;
  copyBitmap(
    TGIS_Bitmap(_canvas).NativeBitmap,
    Result.NativeBitmap
  ) ;
end ;

procedure TGIS_RendererVclAbstract.ViewerDrawCache(
...
begin
  ...
  if ( TGIS_Bitmap( _cache ).Width = 0 )
     or
     ( TGIS_Bitmap( _cache ).Height = 0 ) then exit ;
  if not ( _paint is TGIS_Bitmap ) then exit ;
  if TGIS_Bitmap.IsNilOrEmpty( TGIS_Bitmap( _paint ) ) then exit ;
  if ( TGIS_Bitmap( _paint ).Width = 0 )
     or
  ...
  stretchBitmapFast(
    VCL.Graphics.TBitmap( TGIS_Bitmap( _cache ).NativeBitmap ),
    VCL.Graphics.TBitmap( TGIS_Bitmap( _paint ).NativeBitmap ),
    _rect
  ) ;
end ;
...
procedure TGIS_RendererVclAbstract.ViewerDrawProgressBitmap(
  const _bitmap : TObject ;
  const _paint  : TObject ;
  const _rect   : TRect
) ;
...
procedure TGIS_RendererVclAbstract.ViewerDrawZoomingRect(
  const _canvas : TObject ;
  const _x      : Integer ;
  const _y      : Integer ;
  const _width  : Integer ;
...
begin
  TCanvas(_canvas).Brush.Color := clBlack ;
  TCanvas(_canvas).Pen.Width   := 1 ;
  TCanvas(_canvas).Pen.Style   := psDot ;
  TCanvas(_canvas).Pen.Color   := VCLColor( _color ) ;
  old_penmode := TCanvas(_canvas).Pen.Mode ;
  try
    TCanvas(_canvas).Pen.Mode  := pmNotXor ;
    TCanvas(_canvas).Pen.Width := 1 ;
    TCanvas(_canvas).MoveTo( _x, _y ) ;
    TCanvas(_canvas).LineTo( _x,
                             _y + _height ) ;
    TCanvas(_canvas).LineTo( _x + _width,
                             _y + _height ) ;
    TCanvas(_canvas).LineTo( _x + _width,
                             _y ) ;
    TCanvas(_canvas).LineTo( _x, _y ) ;
  finally
    TCanvas(_canvas).Pen.Mode := old_penmode ;
  end ;
end ;

procedure TGIS_RendererVclAbstract.ViewerDrawDraggingTrack(
  const _canvas : TObject ;
  const _x1     : Integer ;
  const _y1     : Integer ;
  const _x2     : Integer ;
...
begin
  ...
  try
    TCanvas(_canvas).Pen.Mode  := pmNotXor ;
    TCanvas(_canvas).Pen.Width := 1 ;
    TCanvas(_canvas).MoveTo( _x1, _y1 ) ;
    TCanvas(_canvas).LineTo( _x2, _y2 ) ;
  finally
    TCanvas(_canvas).Pen.Mode := old_penmode ;
  end ;
end ;

procedure TGIS_RendererVclAbstract.ControlDrawNonTransparent(
  const _context : TObject ;
  const _bitmap  : TGIS_Bitmap ;
  const _x       : Integer ;
  const _y       : Integer
...
begin
  ...
  if not ( _context is TCanvas ) then exit ;
  if TGIS_Bitmap.IsNilOrEmpty( _bitmap ) then exit ;

  TCanvas( _context ).Draw(
    _x, _y,
    VCL.Graphics.TBitmap( _bitmap.NativeBitmap )
  ) ;
end ;

procedure TGIS_RendererVclAbstract.ControlDrawTransparent(
  const _context : TObject ;
  const _bitmap  : TObject ;
  const _x       : Integer ;
  const _y       : Integer
...
begin
  ...
  TCanvas(_context).Draw(
    _x, _y,
    VCL.Graphics.TBitmap( TGIS_Bitmap(_bitmap).NativeBitmap ),
    255
  ) ;
end ;
