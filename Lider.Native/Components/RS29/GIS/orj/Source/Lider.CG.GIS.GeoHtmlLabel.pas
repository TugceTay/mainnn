...
{$IFDEF DCC}
  unit GisHtmlLabel ;
  {$HPPEMIT '#pragma link "GisHtmlLabel"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
  ...
  begin
    ...
    {$ELSE}
      tmHeight := iSize.Y ;
    {$ENDIF}
    tmAscent := RoundS( oRenderer.CanvasTextBaseline( sText ) ) ;
...
//              {$ENDIF}
  ...
  var
    ...
    begin
      ...
      if not IsStringEmpty( s ) then begin
        try
          oRenderer.CanvasFont.Name := s.Trim(['''']) ;
        except
          //
        end;
      end ;
  ...
  begin
    ...
    if ( oElements.Count > 0 )
       and
       ( T_htmlText( oElements[oElements.Count-1] ).iBiDi = BIDI_RL_STRONG )
    then
      anyBiDi := True ;

    // attach any spaces before LR to LR context
    if anyBiDi
       and
       ( T_htmlText( oElements[oElements.Count-1] ).iBiDi = BIDI_LR_STRONG )
    then begin
      for  i := oElements.Count-2 downto 0 do begin
        if T_htmlText( oElements[i] ).sText = ' ' then
          T_htmlText( oElements[i] ).iBiDi := BIDI_LR_STRONG
        else
          break ;
      end;
    end;
