...
unit Lider.CG.GIS.VCL.GeoPrintTemplateDesigner ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.VCL.GeoPrintTemplateDesigner"'}
...
uses
  ...
  Lider.CG.GIS.VCL.GeoControlPrintPositionForm,
...
uses
  ...
  Lider.CG.GIS.VCL.GeoControlSizeForm,
  Lider.CG.GIS.VCL.GeoControlFontStyleForm,
...
//==============================================================================
  ...
  var
    ...
    cntrl : TControl ;
  begin
    if ( _attribute = GIS_TPL_DESIGNER_FLD_TEXT ) or
       ( _attribute = GIS_TPL_DESIGNER_FLD_GRAPHICSPATH ) or
       ( _attribute = GIS_TPL_DESIGNER_FLD_GRAPHICSPATH_BTN ) then begin
      for i := 0 to ControlCount - 1 do begin
        cntrl := Controls[i] ;
        if cntrl.Name = _attribute then begin
          cntrl.Enabled := _enable ;
          break ;
        end ;
      end ;
    end ;
...
//==============================================================================
  ...
  begin
    ...
    if not assigned( FTemplate.GIS_Viewer[idx] ) or
       not viewersCreated or
       not assigned( FViewers ) or
    ...
    FViewers[idx].Unlock ;
    _bitmap := TBitmap(FViewers[idx].Bitmap) ;
  end ;
  ...
  var
    ...
    begin
      ...
      try
        ...
        h := _rect.Height ;
        cache := VCL.Graphics.TBitmap(
                   TGIS_RendererVclAbstract(rnd).createTransparentBitmap( w, h )
                 ) ;
        ...
        _canvas.Draw( _rect.Left, _rect.Top, cache, 255 ) ;
      finally
        FreeObject( ctx ) ;
      end ;
    end ;
  ...
  procedure TGIS_ControlPrintTemplateDesignerForm.updateViewers ;
  var
    idx : Integer ;
  begin
    for idx := 1 to FTemplate.GIS_ViewerCount do
      if assigned( FTemplate.GIS_Viewer[idx] ) then begin
        FTemplate.GIS_ViewerExtent[idx] := FTemplateOrig.GIS_ViewerExtent[idx] ;
        FTemplate.GIS_ViewerScale[idx] := FTemplateOrig.GIS_ViewerScale[idx] ;
      end;
  end ;
  ...
  var
    ...
    begin
      ...
      expExtent.XMin := _gisExtent.XMin - wExtent / 2 ;
      expExtent.XMax := _gisExtent.XMax + wExtent / 2 ;
      expExtent.YMin := _gisExtent.YMin - hExtent / 2 ;
      expExtent.YMax := _gisExtent.YMax + hExtent / 2 ;
      //expExtent := GisCommonExtent( _gis.Extent, expExtent ) ;
      w := RoundS( _width * Abs(expExtent.XMax - expExtent.XMin) / Abs(wExtent) ) ;
      h := RoundS( _height * Abs(expExtent.YMax - expExtent.YMin) / Abs(hExtent) ) ;
      if w < h then
        h := 0
      else
        w := 0 ;
      ...
      try
        ...
        try
          exp.ExportFrom( _gis, tile, _gisExtent, _dpi ) ;
        except
          on ex : Exception do
            raise ex;
        end ;
  ...
  begin
    ...
    for i := 1 to FTemplate.GIS_ViewerCount do begin
      if assigned( FTemplate.GIS_Viewer[i] ) and
         ( not FTemplate.GIS_Viewer[i].IsEmpty ) then begin
        SetLength( FViewers, i+1 ) ;
        FViewers[i] := nil ;
        lp := nil ;
        if FTemplate.GIS_Viewer[i] is TGIS_ViewerWnd then begin
          width := TGIS_ViewerWnd( FTemplate.GIS_Viewer[i] ).Width ;
          height := TGIS_ViewerWnd( FTemplate.GIS_Viewer[i] ).Height ;
        end else begin
          width := 0 ;
          height := 0 ;
        end ;
        scale := FTemplate.GIS_Viewer[i].Scale ;
        cl := FTemplate.GIS_Viewer[i].Color ;
        try
          FTemplate.GIS_Viewer[i].Lock ;
          FTemplate.GIS_Viewer[i].Color := TGIS_Color.None;
          if FTemplate.GIS_ViewerScale[i] > 0 then
            FTemplate.GIS_Viewer[i].Scale := FTemplate.GIS_ViewerScale[i] ;
          try
            export_bmp( FTemplate.GIS_Viewer[i],
                        FTemplate.GIS_ViewerExtent[i],
                        width, height,
                        RoundS( CurrentPPI * scale / FTemplate.GIS_Viewer[i].Scale )
                      ) ;
          except
          end ;
        finally
          FTemplate.GIS_Viewer[i].Color := cl ;
          if FTemplate.GIS_ViewerScale[i] > 0 then
            FTemplate.GIS_Viewer[i].Scale := scale ;
          FTemplate.GIS_Viewer[i].Unlock ;
        end ;
        FViewers[i] := TGIS_ViewerBmp.Create( 100, 100 ) ;
        FViewers[i].Lock ;
        if assigned( lp ) then
          FViewers[i].Add( lp ) ;
        FViewers[i].Renderer := TGIS_RendererVclDirect2D.Create ;
        if FTemplate.GIS_UseViewerColor[i] then
          FViewers[i].Color := cl
        else
          FViewers[i].Color := TGIS_Color.None ;
        FViewers[i].Unlock ;
        FViewers[i].Lock ;
        FViewers[i].CustomPPI := 0 ;
        FViewers[i].VisibleExtent := FTemplate.GIS_ViewerExtent[i] ;
        if FTemplate.GIS_ViewerScale[i] <> 0 then
          FViewers[i].Scale := FTemplate.GIS_ViewerScale[i] ;
        FViewers[i].CustomPPI := FPrinter.PPI ;
        FViewers[i].RestrictedDrag := False ;
        FViewers[i].Unlock ;
      end ;
    end;
    viewersCreated := True ;
  ...
  var
    i : Integer ;
    l : TGIS_ControlLegend ;
    s : TGIS_ControlScale ;
    n : TGIS_ControlNorthArrow ;
  begin
    for i := 1 to FTemplate.GIS_LegendCount do
      if assigned( FTemplate.GIS_Legend[i] ) then begin
        l := TGIS_ControlLegend( FTemplate.GIS_Legend[i] ) ;
        FreeObject( l ) ;
      end;
    for i := 1 to FTemplate.GIS_ScaleCount do
      if assigned( FTemplate.GIS_Scale[i] ) then begin
        s := TGIS_ControlScale( FTemplate.GIS_Scale[i] ) ;
        FreeObject( s ) ;
      end;
    for i := 1 to FTemplate.GIS_NorthArrowCount do
      if assigned( FTemplate.GIS_NorthArrow[i] ) then begin
        n := TGIS_ControlNorthArrow( FTemplate.GIS_NorthArrow[i] ) ;
        FreeObject( n ) ;
      end;
  ...
  begin
    ...
    try
      ...
      Result := dlg_size.Execute( on_help ) ;
    finally
      FreeObject( dlg_size ) ;
    end ;
  end ;
  ...
  begin
    ...
    if name = GIS_TPL_DESIGNER_FLD_INDEX_MAP then begin
      ...
      for i := 1 to FTemplate.GIS_ViewerCount do
        if assigned( FTemplate.GIS_Viewer[i] ) and
           ( FTemplate.GIS_Viewer[i] is TControl ) then
