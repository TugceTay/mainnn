...
{$IFDEF DCC}
  unit GisLayerLandXML ;
  {$HPPEMIT '#pragma link "GisLayerLandXML"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
  ...
  begin
    ...
    if (startPtg.X <> 0) and (startPtg.Y <> 0) then
      oShp.AddPoint3D( startPtg ) ;
      a := 0.1 ;
      if (points[1].X <> 0) and (points[1].Y <> 0) then begin
        while a <= 1 do begin
          oShp.AddPoint3D( bezierPoint(a, points) ) ;
          a := a + 0.1 ;
        end ;
      end ;
    if (endPtg.X <> 0) and (endPtg.Y <> 0) then
      oShp.AddPoint3D( endPtg ) ;
  end ;
