...
type
  ...
  protected {My simplified methods to override}
    ...
    procedure AllocateBookMark(RecordID: Pointer; Bookmark: TBookmark); virtual; abstract;
    //Navigation methods
  ...
  protected
    ...
    function GetBookmarkFlag(Buffer: TRecordBuffer): TBookmarkFlag; overload;
    procedure SetBookmarkFlag(Buffer: TRecordBuffer; Value: TBookmarkFlag); overload;
...
{$ENDIF}
  ...
  protected
    ...
    procedure AllocateBookMark(RecordID: Pointer; Bookmark: TBookmark); override;
...
begin
  ...
  begin
    // ilker gerek yok FreeMem(PRecordInfo(Buffer + FDataSize)^.BookMark);
...
begin
  FillChar(TRecordBuffer(Buffer)^, CalcFieldsSize, 0); // orj FillChar(Buffer[FStartCalculated], CalcFieldsSize, 0);
end;
...
begin
  case State of
    ...
    dsNewValue, dsOldValue, dsCurValue: Result := TRecordBuffer(ActiveBuffer);
{$IFDEF level5}
...
procedure TlicgBaseDataset.InternalAddRecord(Buffer: TRecordBuffer; Append: Boolean);
begin
  if Append then
    InternalLast;
  ...
  begin
    ...
    case Fields[Index].DataType of
      ...
      ftInteger, ftSmallInt, ftDate, ftTime:
        begin
          Move(PInteger(Buffer)^, TempInt, sizeof(Integer));
          SetFieldValue(Fields[Index], TempInt);
        end;
      ftFloat, ftBCD, ftCurrency, ftDateTime:
        begin
          Move(PDouble(Buffer)^, TempDouble, sizeof(Double));
          SetFieldValue(Fields[Index], TempDouble);
        end;
      ftBoolean:
        begin
          Move(PBoolean(Buffer)^, TempBool, sizeof(Boolean));
  ...
  begin
    ...
    begin
      if BookMark = nil then
        GetMem(PByte(BookMark), BookmarkSize);
      ...
      ftInteger, ftSmallInt, ftDate, ftTime:
        begin
          if VarIsNull(Value) then
            ...
            TempInt := Value;
          Move(TempInt, PInteger(Buffer + Offset)^, sizeof(TempInt));
        end;
      ftFloat, ftBCD, ftCurrency, ftDateTime:
        begin
          if VarIsNull(Value) then
            ...
            TempDouble := Value;
          Move(TempDouble, PInteger(Buffer + Offset)^, sizeof(TempDouble));
        end;
      ftBoolean:
        begin
          if VarIsNull(Value) then
            ...
            TempBool := Value;
          Move(TempBool, PInteger(Buffer + Offset)^, sizeof(TempBool));
        end;
      ftMemo, ftGraphic:
        begin
          {
...
procedure TlicgBaseDataset.GetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin
  if BookMarkSize > 0 then
...
procedure TlicgBaseDataset.SetBookmarkData(Buffer: TRecordBuffer; Data: TBookmark);
begin {
  if PRecordInfo(Buffer + FDataSize)^.BookMark = nil then
...
procedure TlicgBaseDataset.InternalGotoBookmark(Bookmark: TBookmark);
begin
...
procedure TlicgTable.AllocateBookMark(RecordID: Pointer; Bookmark: TBookmark);
begin
...
procedure TlicgDesignTable.AllocateBookMark(RecordID: Pointer; Bookmark: TBookmark);
begin
  PInteger(Bookmark)^ := Integer(RecordID);
end;
