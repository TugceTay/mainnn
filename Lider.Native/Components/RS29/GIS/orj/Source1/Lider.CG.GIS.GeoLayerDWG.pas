...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoLayerDWG ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayerDWG"'}
{$ENDIF}
{$IFDEF CLR}
...
//=============================================================================
  ...
  var
    ...
    begin
      ...
      shp.Lock( TGIS_Lock.Projection ) ;
        ...
        if ( elipse.InitAngle < 0 ) then
          sa := elipse.InitAngle + 2*Pi
        else if ( elipse.InitAngle > (2*Pi) ) then
          sa := elipse.InitAngle - (2*Pi)
        else
          sa := elipse.InitAngle ;

        if ( elipse.EndAngle < 0 ) then
          ea := elipse.EndAngle + 2*Pi
        else if ( elipse.EndAngle > (2*Pi) ) then
          ea := elipse.EndAngle - (2*Pi)
        else
          ea := elipse.EndAngle ;
  ...
  class procedure GisLayerDWG.SelfRegisterLayer() ;
  begin
