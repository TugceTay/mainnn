...
unit VCL.GisControlScale ;
{$HPPEMIT '#pragma link "VCL.GisControlScale"'}
...
type
  ...
  TGIS_ControlScale = class( TCustomPanel, IGIS_Subscribe, IGIS_PrintableControl )
    ...
    private
      ...
      st_FontColor     : TColor  ;
      st_Units         : TGIS_CSUnits ;
      st_UnitsEPSG     : Integer ;
      st_Dividers      : Integer ;
      st_DividerColor1 : TColor ;
      st_DividerColor2 : TColor ;
      st_Bitmap        : TGIS_Bitmap ;
...
function TGIS_ControlScale.fget_Bitmap
  : TBitmap ;
begin
...
begin
  ...
  if update then begin
    ...
    FreeObject( FBitmap ) ;
    FBitmap := DrawBmp( FBitmap, GIS_Viewer.Scale, 0 ) ;
    ...
    if FBitmap.BitmapType = TGIS_BitmapType.VCL then
      VCLMakeCompatibleBitmap( Bitmap ) ;
  end;
...
begin
  ...
  if bLock then begin
    if assigned( st_Bitmap ) then
      Canvas.Draw( 0, 0, TBitmap(st_Bitmap.NativeBitmap) ) ;
    exit ;
  end ;
  ...
  try
    ...
    then begin
      ...
      if ( r.Width > 5 ) and ( r.Height > 5 ) then begin
        ...
        if not isDesignMode then
          bmp := DrawBmp( bmp, GIS_Viewer.Scale, 0 )
        else
          bmp := DrawBmp( bmp, 1/500000, 0 ) ;
...
begin
  ...
  st_Units     := Units ;
  st_UnitsEPSG := UnitsEPSG ;
  st_Dividers      := Dividers ;
  st_DividerColor1 := DividerColor1 ;
  st_DividerColor2 := DividerColor2 ;
  if assigned( FBitmap ) then
    st_Bitmap := TGIS_Bitmap.Create( FBitmap )
  else
    st_Bitmap := nil ;
  InternalName := Name ;
  Result := Self ;
...
var
  fnt : TFont ;
begin
  FreeObject( st_Bitmap ) ;
  Units := st_Units ;
  UnitsEPSG := st_UnitsEPSG ;
  Dividers := st_Dividers ;
  DividerColor1 := st_DividerColor1 ;
  DividerColor2 := st_DividerColor2 ;
  fnt := TFont.Create ;
  try
...
begin
  ...
  btrans := Transparent ;
  try
    DrawBmp( _bitmap, GIS_Viewer.ScaleAsFloat, GIS_Viewer.PPI ) ;
  finally
    Transparent := btrans ;
  end;
end ;
...
begin
  ...
  try
    ...
    if assigned( _bmp ) and
       ( _bmp.BitmapFactory <> rnd.BitmapFactory ) then begin
      FreeObject( rnd ) ;
      rnd := TGIS_RendererVclGdiPlus.Create ;
    end ;
    {$ENDIF}
    ...
    try
      ...
      rct_txt := Rect(
                   ...
                   r.Right,
                   // r.Bottom cannont be used here because of the skia renderer
                   y + dy + System.Math.Max( 2, rnd.CanvasTextExtent( 'Ay').Y div 8 ) + htxt
                 ) ;
