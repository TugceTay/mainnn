...
{$IFDEF DCC}
  unit GisCompression ;
  {$HPPEMIT '#pragma link "GisCompression"'}
{$ENDIF}
{$IFDEF CLR}
...
{$ENDIF}
  ...
  /// </param>
  /// <param name="_name">
  ///   internal file name to extract; if empty, the first file is used ;
  ///   if not found, the first file matching extension is used
  /// </param>
  /// <returns>
  ///   decompressed stream
  /// </returns>
...
{$ENDIF}
  ...
  begin
      ...
      try
        ...
        if IsStringEmpty( _name ) then begin
          ...
          for i := 0 to zip.FileCount-1 do begin
            ...
            if IsStringEmpty( str ) and (CompareText( GetFileExt( zip.FileName[i] ), ext ) = 0) then
              str := zip.FileName[i] ;
          end ;
          if not assigned( zstm ) then begin
            if not IsStringEmpty( str ) then
              zip.Read( str, zstm, hdr )
            else
              zip.Read( zip.FileName[0], zstm, hdr ) ;
            Result.CopyFrom( zstm, zstm.Size ) ;
          end;
          ...
          if zip.Entries.Count > 0 then begin
            if IsStringEmpty( _name ) then begin
              ...
              for i := 0 to zip.Entries.Count-1 do begin
                ...
                if (idx = -1) and (CompareText( GetFileExt( zip.Entries[i].FullName ), ext ) = 0) then
                  idx := i ;
              end ;
              if not assigned( zstm ) then begin
                if (idx = -1) then
                  idx := 0 ;
                zstm := zip.Entries[idx].Open ;
                zstm.CopyTo( Result ) ;
              end;
            end ;
          end ;
    ...
    {$IFDEF JAVA}
        ...
        if assigned( zip ) then begin
          if IsStringEmpty( _name ) then begin
            ...
            repeat
              ...
              if IsStringEmpty( str ) and (CompareText( GetFileExt( ze.Name ), ext ) = 0) then
                str := ze.Name ;
            until (ze = nil) ;
          end ;
        end ;
      ...
      if (i = 0) then
        Result := DecompressZipFile( _path, str ) ;
    {$ENDIF}
    {$IFDEF ISLAND}
      {$WARNING '### Verify ISLAND code'}
    {$ENDIF}
