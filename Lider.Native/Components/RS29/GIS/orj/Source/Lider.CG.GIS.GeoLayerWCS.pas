...
{$IFDEF DCC}
  unit GisLayerWCS ;
  {$HPPEMIT '#pragma link "GisLayerWCS"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
  ...
  begin
    ...
    if IsStringEmpty( oWCS.ForcedCRS ) then begin
      ...
      if assigned( lw ) and (lw.CRS.Count > 0) then begin
  ...
  class procedure Unit_GisLayerWCS.SelfRegisterLayer() ;
  begin
  ...
  {$IFNDEF OXYGENE}
    initialization
      Unit_GisLayerWCS.SelfRegisterLayer() ;
  {$ENDIF}
