...
{$IFDEF DCC}
  unit GisLayerWebTiles ;
  {$HPPEMIT '#pragma link "GisLayerWebTiles"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_LayerWebTiles = {$IFDEF OXYGENE} public {$ENDIF}
    ...
    private
      ...
      /// </param>
      /// <returns>
      ///   Text with expanded password
      /// </returns>
      function embedPassword  ( const _str   : String
                              ) : String ;
...
const
  ...
  WEBTILES_MAX_CACHE_MEMSIZE    = 1024 * 1024 * WEBTILES_CACHE_SIZE ; // 20MB of memory cache
...
begin
  ...
  oDrawingList := TList< T_subLayer >.Create ;
  oLock  := TCriticalSection.Create ;
end ;
...
begin
  ...
  try
    ...
    for ilay in oDrawingList do begin
      if imax <= 0 then
        exit ;

      if ilay.State = WEBTILES_THREAD_WAITING then begin
        dec( imax ) ;
        ilay.Start ;
      end
    end;
...
begin
  ...
  try
    ...
    try
      ...
      if assigned( oParent.Parent.Viewer ) then
        oParent.Layer.BitmapFactory := TGIS_RendererAbstract( oParent.Parent.Viewer.Ref.ViewerParent.ControlRenderer).BitmapFactory ;
      ...
      try
        if not oParent.Parent.bBatch then begin
          ...
          {$IFDEF DCC}
            if Assigned( NativeBitmapFactory )
               and
               ( NativeBitmapFactory.ClassName = 'TGIS_BitmapFactoryFMX' )
               and
...
function TGIS_LayerWebTiles.embedPassword(
  const _str : String
) : String ;
begin
  {$IFDEF OXYGENE}
    Result := TemplateProducer( _str, nil, @passwordCallBackW, False ) ;
  {$ELSE}
    Result := TemplateProducer( _str, nil, passwordCallBackW, False ) ;
  {$ENDIF}
end;
...
var
  ...
  procedure add_tile( _col, _row : Integer ) ;
  var
    tmp         : String      ;
    tile_width  : Integer     ;
    tile_height : Integer     ;
  ...
  begin
    if level >= 0 then begin
      if _col < startcol then exit ;
      if _col > endcol   then exit ;
      if _row < startrow then exit ;
      if _row > endrow   then exit ;

      //tiling enabled with toilernae of half pixel
      tile_ext.XMin := lstLod[ level ].TopLeft.X + tsx * _col ;
      tile_ext.XMax := tile_ext.XMin + tsx + lod.Resolution / 2 ;
      tile_ext.YMax := lstLod[ level ].TopLeft.Y - tsy * _row ;
...
begin
  ...
  try
    ...
    midcol := ( endcol - startcol ) div 2 + startcol ;
    midrow := ( endrow - startrow ) div 2 + startrow ;

    // add center first
    add_tile( midcol, midrow ) ;


    maxmargin := Max( Max( midcol - startcol, endcol - midcol ),
                      Max( midrow - startrow, endrow - midrow )
                    );

    margin:= 1;

    // add spiral around the ceter
    while margin <= maxmargin do begin

      for col := midcol-margin to midcol+margin-1 do
        add_tile( col, midrow-margin );
      for row := midrow-margin to midrow+margin-1 do
        add_tile( midcol+margin, row );
      for col := midcol+margin downto midcol-margin+1 do
        add_tile( col, midrow+margin );
      for row := midrow+margin downto midrow-margin+1 do
        add_tile( midcol-margin, row );
...
function  TGIS_LayerWebTiles.getGridData(
  const _extent  : TGIS_Extent      ;
  const _grid    : TGIS_GridArray
) : Boolean ;
var
...
begin
  ...
  try
    ...
    try
      ...
      if _c( 'CUSTOM' ) then begin
      ...
      else if _c( 'GEOPACKAGE' ) then begin
        iStartLevel       := 21  ;
        iStartLevelForced := 0  ;
        iEndLevel         := 0 ;
        ptTileSize.X      := 256 ;
        ...
        try
          ...
          odb.sqlQueryOpen( 'SELECT * FROM gpkg_contents where data_type=''tiles'' '+
            'and table_name=''' + Name+'''', 0
          ) ;
          if odb.sqlQueryEof(0) then begin
            odb.sqlQueryOpen( 'SELECT * FROM gpkg_contents where data_type=''tiles''', 0) ;
            if odb.sqlQueryEof(0) then
              do_raise( 'Name', Name ) ;
            Name := VarToString( odb.sqlQueryGetField('table_name',0) ) ;
          end ;

          try
            if not odb.sqlQueryEof(0) then begin
              epsg  := VarToInt32 ( odb.sqlQueryGetField('srs_id',0) ) ;
              min_x := VarToDouble( odb.sqlQueryGetField('min_x' ,0) ) ;
              max_x := VarToDouble( odb.sqlQueryGetField('max_x' ,0) ) ;
              min_y := VarToDouble( odb.sqlQueryGetField('min_y' ,0) ) ;
              max_y := VarToDouble( odb.sqlQueryGetField('max_y' ,0) ) ;

              extLayer := GisExtent( min_x, min_y, max_x, max_y ) ;
            end ;
          finally
            odb.sqlQueryClose(0) ;
          end ;

          odb.sqlQueryOpen( 'SELECT * FROM gpkg_tile_matrix_set where table_name=''' +
            Name+''' and srs_id='+ IntToStr(epsg), 0
          ) ;
          try
            if not odb.sqlQueryEof(0) then begin
              min_x := VarToDouble( odb.sqlQueryGetField('min_x',0) ) ;
              ...
              extTilesArea := GisExtent( min_x, min_y, max_x, max_y ) ;
            end ;
          finally
            odb.sqlQueryClose(0) ;
          end ;
          ...
          odb.sqlQueryOpen(
            'SELECT * FROM gpkg_tile_matrix where table_name=''' + Name+'''', 0
          ) ;
          try
            while not odb.sqlQueryEof(0) do begin
              zl := VarToInt32( odb.sqlQueryGetField('zoom_level',0) ) ;
              iStartLevel       := Min( iStartLevel, zl ) ;
              iStartLevelForced := iStartLevel ;
              iEndLevel         := Max( iEndLevel, zl) ;

              ptTileSize.X := VarToInt32( odb.sqlQueryGetField('tile_width' ,0) ) ;
              ptTileSize.Y := VarToInt32( odb.sqlQueryGetField('tile_height',0) ) ;

              mX := VarToInt32( odb.sqlQueryGetField('matrix_width' ,0) ) ;
              mY := VarToInt32( odb.sqlQueryGetField('matrix_height',0) ) ;
              dx := VarToDouble( odb.sqlQueryGetField('pixel_x_size',0) ) ;

              rct := Rect( 0, 0, mX-1, mY-1 ) ;
          ...
          url1 := 'select tile_data from [' + Name + '] where zoom_level={level} ' +
                  'and tile_column={col} and tile_row={row}' ;
        finally
          FreeObject( odb ) ;
        end ;
      ...
      if not IsStringEmpty( sStyle ) then begin
        ...
        style_pass := embedPassword(  sStylePassword );
