...
{$IFDEF DCC}
  unit GisDbAdoNet ;
  {$HPPEMIT '#pragma link "GisDbAdoNet"'}
{$ENDIF}
{$IFDEF CLR}
            ...
            TGIS_SubDataType.SdoGeometry :
              begin
                ...
                if assigned( ptype ) then begin
                  ...
                  if tname = 'Oracle.ManagedDataAccess.Client.OracleParameter' then begin
                    pinfo := ptype.GetProperty('UdtTypeName') ;
                    if assigned( pinfo ) then begin
                      ptype := pinfo.PropertyType;
                      var value: String := "MDSYS.SDO_GEOMETRY";
                      pinfo.SetValue( dbparam,  Convert.ChangeType(value, pinfo.PropertyType), nil ) ;
                    end;
                  end ;
                end ;
              end ;
