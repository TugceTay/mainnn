...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoRestServices ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoRestServices"'}
{$ENDIF}
{$IFDEF CLR}
  ...
  begin
    ...
    if not GisIsNoWorld( _filterRect ) and
      ...
      url := makeUrl( url, Format( 'geometry=%s,%s,%s,%s',
                                   [ DotFloatToStr( _filterRect.XMin ),
                                     DotFloatToStr( _filterRect.YMin ),
                                     DotFloatToStr( _filterRect.XMax ),
                                     DotFloatToStr( _filterRect.YMax )
                                   ] )
                    ) ;
