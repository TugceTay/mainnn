...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoLayerSql ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayerSql"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  const
    ...
    CREATE_MASTER =
      'CREATE TABLE <#ttkLider.CG.GIS.GeoLayerSQL#> '                                        +
      '( <#NAME#> <#VARCHAR#>(64),'                                             +
    ...
    CREATE_INDEX_NAME =
      'CREATE UNIQUE INDEX <#ttkLider.CG.GIS.GeoLayerSQL#><#_IDX_#><#NAME#>'                 +
      ' ON <#ttkLider.CG.GIS.GeoLayerSQL#> (<#NAME#>)'                                       ;
    ...
    INSERT_MASTER =
      'INSERT INTO <#ttkLider.CG.GIS.GeoLayerSQL#>'                                          +
    ...
    UPDATE_MASTER =
      'UPDATE <#ttkLider.CG.GIS.GeoLayerSQL#>'                                               +
    ...
    SELECT_MASTER =
      'SELECT * FROM <#ttkLider.CG.GIS.GeoLayerSQL#> WHERE <#NAME#>=''%s'''                  ;
    ID_SELECT_MASTER = ID_UPDATE_MASTER_OLD + 1                                 ;

    SELECT_MASTER_ALL =
      'SELECT <#NAME#>, <#SHAPETYPE#> FROM <#ttkLider.CG.GIS.GeoLayerSQL#>'                  ;
    ...
    DELETE_TABLE =
      'DELETE FROM <#ttkLider.CG.GIS.GeoLayerSQL#> WHERE <#NAME#>=''%s'''                    ;
    ...
    CREATE_TABLE_LOG_MANAGER =
      'CREATE TABLE <#ttkLider.CG.GIS.GeoLayerLOG#> ( '                                      +
    ...
    CREATE_TABLE_LOG_MANAGER_INDEX =
      'CREATE UNIQUE INDEX <#ttkLider.CG.GIS.GeoLayerLOG#><#_IDX_#><#NAME#>'                 +
      ' ON <#ttkLider.CG.GIS.GeoLayerLOG#> (<#NAME#>)'                                       ;
    ID_CREATE_TABLE_LOG_MANAGER_INDEX = ID_CREATE_TABLE_LOG_MANAGER + 1         ;

    DELETE_LOG_MANAGER =
      'DELETE FROM <#ttkLider.CG.GIS.GeoLayerLOG#> WHERE <#NAME#>=''%s'''                    ;
    ID_DELETE_LOG_MANAGER = ID_CREATE_TABLE_LOG_MANAGER_INDEX + 1               ;

    INSERT_MASTER_LOG =
      'INSERT INTO <#ttkLider.CG.GIS.GeoLayerLOG#>'                                          +
      ' VALUES(''%s'',''%s'')'                                                  ;
    ID_INSERT_MASTER_LOG = ID_DELETE_LOG_MANAGER + 1                            ;

    UPDATE_MASTER_LOG =
      'UPDATE <#ttkLider.CG.GIS.GeoLayerLOG#> SET ENABLED=%s WHERE <#NAME#>=''%s'''          ;
    ...
    CREATE_TRIGGER_GEO_AFTER_UPDATE_LOG =
      ...
      'FOR EACH ROW WHEN (SELECT ENABLED FROM <#ttkLider.CG.GIS.GeoLayerLOG#> WHERE NAME=''%s'') ' +
      'BEGIN'                                                                         +
        ...
        if GisIsWholeWorld( cursorState[ _cursor ].curRawExtent ) or
          ...
          if ( IsStringEmpty( sqlquery ) ) or cursorSql[_cursor].fullSearch then
            ...
            oGisDb.sqlQueryOpen( Format( getCmd( T_SQLN.ID_SELECT_JOIN_NOEXT_EX )+ order,
                                  [ ViewFeatures, TableGeometry, sqlquery,
                                    mf(' AND ',pixelfilter,'') ]
                                ), _cursor
                        ) ;
        end
          ...
          if hasRtree then begin
            if ( IsStringEmpty( sqlquery ) ) or cursorSql[_cursor].fullSearch then
              oGisDb.sqlQueryOpen( Format( getCmd( T_SQLN.ID_SELECT_JOIN_RTREE ) + order,
                                    [ ViewFeatures, TableGeometry, TableIndex,
