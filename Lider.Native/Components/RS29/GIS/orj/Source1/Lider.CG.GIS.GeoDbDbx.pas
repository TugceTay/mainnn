...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoDbDbx ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoDbDbx"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  /// <summary>
  ///   Class that can read DBX.
  /// </summary>
  TGIS_DbDbx = class( TGIS_DbAbstract )
    private

      /// <summary>
      ///   DBX shared connection object.
      /// </summary>
      FSharedConnection : TSQLConnection ;
    ...
    private
      ...
      cursorDB : array of record
        ...
        myQuery           : TSQLQuery ;  // Query object.
      ...
      ///   Table object.
      /// </summary>
      myTable           : array [0..2] of TSQLQuery ;
      ...
      ///   Transaction object.
      /// </summary>
      myTransaction     : TTransactionDesc ;
    ...
    // for internal use
      ...
      /// <inheritdoc/>
      procedure sqlQueryUnPrepareGetGeom(
                                        const _cursor        : Integer
                                      ) ; override;
    ...
    public
      ...
      /// </summary>
      property SharedConnection : TSQLConnection read  FSharedConnection
...
//=============================================================================
  ...
  begin
    ...
    for i := Low( myTable ) to High( myTable ) do
      myTable[ i ] := TSQLQuery.Create( nil ) ;

    for i := Low( myParams ) to High( myParams ) do
      myParams[ i ] := TParams.Create( nil ) ;

    for i := 0 to BUILTIN_CURSORS - 1 do
      cursorOpen( i ) ;
  end ;

  destructor  TGIS_DbDbx.Destroy ;
  var
    i : Integer ;
  begin
    sqlDisconnect ;
  ...
  function TGIS_DbDbx.sqlGetParams(
    const _idx : Integer
  ) : TStringList ;
  begin
    Result := nil ;
  ...
  function TGIS_DbDbx.safeString(
    const _txt  : String  ;
    const _size : Integer
   ) : String ;
  begin
  ...
  procedure TGIS_DbDbx.sqlConnect(
  ...
  begin
    if not Assigned( SharedConnection ) then begin
      ...
      begin
        ...
        try
          ...
          myConnection := SharedConnections.OpenDBX(
                            _sqlParameters.Values['DriverName'   ],
                            _sqlParameters.Values['GetDriverFunc'],
                            _sqlParameters.Values['VendorLib'    ],
                            _sqlParameters.Values['LibraryName'  ],
    ...
    else if cursorDB[0].myQuery.SQLConnection <> SharedConnection  then begin
      cursorDB[0].myQuery.SQLConnection := SharedConnection ;
      myTable[ 0 ].SQLConnection := SharedConnection ;
      myTable[ 1 ].SQLConnection := SharedConnection ;
      myTable[ 2 ].SQLConnection := SharedConnection ;
    end ;
  end ;

  procedure TGIS_DbDbx.sqlTableCreateParam(
    const _id       : Integer ;
    const _name     : String ;
    const _type     : TGIS_DataType ;
    const _subtype  : TGIS_SubDataType ;
  ...
  procedure TGIS_DbDbx.sqlDisconnect ;
  var
    i : Integer ;
  begin
    sqlQueryClose(0) ;
    cursorDB[0].myQuery.SQLConnection := nil ;

    for i := Low( myTable) to High( myTable ) do begin
      sqlTableClose( i ) ;
      myTable[ i ].SQLConnection := nil ;
    end ;

    if not Assigned( SharedConnection ) then begin
      SharedConnections.CloseDBX( myConnection ) ;
      myConnection := nil ;
    end ;
  end ;

  procedure TGIS_DbDbx.sqlTransactUpdateStart ;
  begin
    if options.UpdateTransact then
      if not Assigned( SharedConnection ) then begin
        myTransaction.TransactionID  := 1 ;
        myTransaction.IsolationLevel := xilREADCOMMITTED ;
        myConnection.StartTransaction( myTransaction );
      end;
  end ;

  procedure TGIS_DbDbx.sqlTransactUpdateCommit ;
  begin
    if options.UpdateTransact then
      if not Assigned( SharedConnection ) then
        myConnection.Commit( myTransaction ) ;
  end ;

  procedure TGIS_DbDbx.sqlTransactRestructStart ;
  begin
  ...
  procedure TGIS_DbDbx.sqlTransactRestructCommit ;
  begin
    if options.RestructTransact then
      if not Assigned( SharedConnection ) then
        myConnection.Commit( myTransaction ) ;
  ...
  procedure TGIS_DbDbx.sqlTransactGlobalUpdateStart ;
  begin
  ...
  procedure TGIS_DbDbx.sqlTransactGlobalUpdateCommit ;
  begin
    if options.GlobalUpdateTransact then
      if not Assigned( SharedConnection ) then
        myConnection.Commit( myTransaction ) ;
  ...
  procedure TGIS_DbDbx.sqlTransactRollback ;
  begin
    if not Assigned( SharedConnection ) then
      myConnection.Rollback( myTransaction ) ;
  end ;

  procedure TGIS_DbDbx.sqlQueryOpen( const _query   : String ;
                                     const _cursor  : Integer
                                    ) ;
  var
    tmp : String ;
  ...
  procedure TGIS_DbDbx.sqlQueryReset( const _cursor  : Integer ) ;
  begin
    cursorDB[_cursor].oldQuery := '' ;
  end ;

  procedure TGIS_DbDbx.sqlQueryClose( const _cursor : Integer
                                     ) ;
  begin
    ...
    cursorDB[_cursor].myQuery := TSQLQuery.Create( nil ) ;

    if not Assigned( SharedConnection ) then
      cursorDB[_cursor].myQuery.SQLConnection := myConnection
    else
      cursorDB[_cursor].myQuery.SQLConnection := SharedConnection ;
  end;

  procedure TGIS_DbDbx.sqlTableClose( const _id : Integer ) ;
  begin
    ...
    myTable[ _id ] := TSQLQuery.Create( nil ) ;

    if not Assigned( SharedConnection ) then
      myTable[ _id ].SQLConnection := myConnection
    else
      myTable[ _id ].SQLConnection := SharedConnection ;
  end ;

  procedure TGIS_DbDbx.sqlTablePost(
    const _id : Integer
  ) ;
  begin
    if Assigned( FOnSQLExecute ) then
  ...
  procedure TGIS_DbDbx.sqlTableExec(
    const _id : Integer
  ) ;
  begin
    if Assigned( FOnSQLExecute ) then
  ...
  procedure TGIS_DbDbx.sqlTableOpenRead( const _id     : Integer ;
                                         const _query  : String
                                        ) ;
  begin
  ...
  procedure TGIS_DbDbx.sqlTableAppend( const _id       : Integer ;
                                       const _query    : String
                                      ) ;
  var
    i : Integer ;
  ...
  function TGIS_DbDbx.sqlTablePrepared(
    const _id  : Integer
  ) : Boolean ;
  begin
  ...
  procedure TGIS_DbDbx.sqlTableOpenWrite(
    const _id     : Integer ;
    const _query  : String
  ) ;
  var
  ...
  begin
    ...
    if Assigned(SharedConnection) then
      SharedConnection.ExecuteDirect( _command )
    else begin
      Assert( myConnection.Connected, 'Database not connected' ) ;
      myConnection.ExecuteDirect( _command ) ;
    end ;
  end ;

  procedure TGIS_DbDbx.sqlQueryMoveFirst(
    const _cursor : Integer
  ) ;
  begin
  ...
  procedure TGIS_DbDbx.sqlQueryMoveNext(
    const _cursor : Integer
  ) ;
  begin
    Assert( cursorDB[_cursor].myQuery.Active, 'Query not connected' ) ;
  ...
  function  TGIS_DbDbx.sqlQueryEof(
    const _cursor : Integer
  ) : Boolean ;
  begin
  ...
  function TGIS_DbDbx.sqlTableEof(
    const _id : Integer
  ) : Boolean ;
  begin
  ...
  function TGIS_DbDbx.sqlQueryGetFieldIndex(
    const _name    : String ;
    const _cursor  : Integer
  ) : Integer ;
  var
  ...
  function TGIS_DbDbx.sqlQueryGetField(
    const _name   : String ;
    const _cursor : Integer
   ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlQueryGetFieldById(
    const _id     : Integer ;
    const _cursor : Integer
   ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlTableGetField(
    const _id   : Integer ;
    const _name : String
   ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlTableGetParam(
    const _id   : Integer ;
    const _name : String
   ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlBindField(
    const _field  : String ;
    const _cursor : Integer
   ) : Integer ;
  var
  ...
  procedure TGIS_DbDbx.sqlTableSetField(
    const _id           : Integer ;
    const _name         : String ;
    const _val          : Variant ;
    const _defSize      : Integer
  ...
  begin
    ...
    param := myTable[ _id ].Params.ParamByName( _name ) ;
  ...
  function TGIS_DbDbx.sqlGetBindedField(
  ...
  procedure TGIS_DbDbx.sqlQueryStructure(
  ...
  function TGIS_DbDbx.sqlQueryNameGEOUID(
    const _field   : String ;
    const _fieldEx : String ;
    const _cursor  : Integer
   ) : Integer ;
  ...
  function TGIS_DbDbx.sqlQueryGetGEOUID(
    const _field   : String ;
    const _cursor  : Integer
  ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlQueryGetSHAPETYPE(
    const _field  : String ;
    const _cursor : Integer
   ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlQueryGetXMIN(
    const _field   : String ;
    const _cursor  : Integer
  ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlQueryGetYMIN(
    const _field   : String ;
    const _cursor  : Integer
   ) : Variant ;
  begin
  ...
  function TGIS_DbDbx.sqlQueryGetGeomVAR(
    const _field       : String ;
    const _fieldEx     : String ;
    const _cursor      : Integer;
    const _forceValue  : Boolean = False
  ...
  {$IFDEF CLR}
  function TGIS_DbDbx.sqlQueryGetGeomPtr(
    const _field    : String ;
    const _cursor   : Integer
      var _size     : Integer
   ) : TBytes ;
  ...
  procedure TGIS_DbDbx.sqlQueryUnPrepareGetGeom(
    const _cursor        : Integer
  ) ;
  begin
    // for safe inheritance
  ...
  function TGIS_DbDbx.sqlQueryGetBlob(
    const _name    : String  ;
    const _cursor  : Integer
   ) : TStream ;
  var
  ...
  function TGIS_DbDbx.sqlQueryGetGeomObj(
    const _field   : String ;
    const _cursor  : Integer
  ) : TObject ;
  begin
  ...
  procedure TGIS_DbDbx.sqlTableSetGeometry(
    const _id    : Integer ;
    const _name  : String ;
    const _data  : OleVariant ;
    const _blob  : TGIS_MemoryStream
  ...
  procedure TGIS_DbDbx.sqlTableSetBlob(
    const _id    : Integer ;
    const _name  : String ;
    const _data  : OleVariant ;
    const _blob  : TGIS_MemoryStream
  ...
  procedure TGIS_DbDbx.sqlTableUpdateParams(
    const _idx : Integer
  ) ;
  var
    i   : Integer ;
  ...
  procedure TGIS_DbDbx.sqlUpdateStart(
    const _id     : Integer ;
    const _table  : String
  ) ;
  begin
  ...
  function TGIS_DbDbx.sqlQueryGeometryIsText(
    const _cursor : Integer
  ) : Boolean ;
  begin
  ...
  procedure TGIS_DbDbx.cursorOpen(
    const _cursor : Integer
  ) ;
  begin
  ...
  procedure TGIS_DbDbx.cursorClose(
    const _cursor : Integer
  ) ;
  var
    i : Integer ;
  ...
  function TGIS_DbDbx.getLastCursor : Integer ;
  begin
