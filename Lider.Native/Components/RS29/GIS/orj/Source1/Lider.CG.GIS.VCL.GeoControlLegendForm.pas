...
unit Lider.CG.GIS.VCL.GeoControlLegendForm;
{$HPPEMIT '#pragma link "Lider.CG.GIS.VCL.GeoControlLegendForm"'}
...
begin
  ...
  doPageChange(tvPages.Items.Item[0]);
end;
...
{$ENDIF}
  ...
  if (gisPreviewI.Width <> TBitmap(gisPreview.Bitmap).Width) or
    (gisPreviewI.Height <> TBitmap(gisPreview.Bitmap).Height) or
...
begin
  ...
  try
    ...
    gisPreviewI.Picture.Assign(TBitmap(gisPreview.Bitmap));
  finally
    Dec(iBusy);
  end;
end;
...
begin
  ...
  if assigned(_legend) then
    ...
    prnt := TGIS_ViewerWnd(_layer.Viewer.Ref.ViewerParent);
