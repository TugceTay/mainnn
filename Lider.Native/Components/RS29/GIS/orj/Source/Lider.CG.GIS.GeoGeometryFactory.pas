...
{$IFDEF DCC}
  unit GisGeometryFactory ;
  {$HPPEMIT '#pragma link "GisGeometryFactory"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_GeometryFactory = {$IFDEF OXYGENE} public {$ENDIF} class
    private
      ...
      class function getDblPrec  : Integer ;
      class function getJSONPolygonRightHandRule : Boolean ;
      class function dblToStrPrec( const _val  : Double ;
                                   const _prec : Integer
...
const
  ...
  { Precision of double to string conversion. }
    METADATA_DOUBLE_STRING_PRECISION = 'TGIS_GeometryFactory.DoubleToStringPrecision' ;
...
//==============================================================================
  ...
  class function TGIS_GeometryFactory.getDblPrec : Integer ;
  begin
    Result := GisMetadataAsInteger( 'TGIS_GeometryFactory.DoubleToStringPrecision', -1 ) ;
  end ;

  class function TGIS_GeometryFactory.getJSONPolygonRightHandRule : Boolean ;
  begin
    Result := GisMetadataAsBoolean( 'TGIS_GeometryFactory.JSONPolygonRightHandRule', False ) ;
  end ;

  class function TGIS_GeometryFactory.dblToStrPrec(
    const _val  : Double ;
    const _prec : Integer
  ) : String ;
  begin
    if _prec > -1 then
      Result := DotFloatToStrPrec( _val, _prec )
    else
      Result := DotFloatToStr( _val ) ;
  end ;
...
//          _shp.AddPoint3D( parr3d[p] ) ;
          ...
          case _shp.Dimension of
            TGIS_DimensionType.XYZ :
              sb.Append( Format( '%s %s %s',
                                [ dblToStrPrec(ptg.X, prec),
                                  dblToStrPrec(ptg.Y, prec),
                                  dblToStrPrec(ptg.Z, prec)
                                ]
                               ) ) ;
          ...
          case _shp.Dimension of
            ...
            TGIS_DimensionType.XYZ :
              begin
                ...
                sb.Append( Format( '%s %s %s',
                                   [ dblToStrPrec(ptg3D.X, prec),
                                     dblToStrPrec(ptg3D.Y, prec),
                                     dblToStrPrec(ptg3D.Z, prec)
                                   ]
                                 ) );
              end ;
            TGIS_DimensionType.XYM :
  ...
  begin
    ...
    try
      ...
      case _shp.Dimension of
        TGIS_DimensionType.XYZ  :
          coord := Format( '%s %s %s', [ dblToStrPrec(ptg.X, prec), dblToStrPrec(ptg.Y, prec),
                                         dblToStrPrec(ptg.Z, prec) ]
                          ) ;
        TGIS_DimensionType.XYZM :
          coord := Format( '%s %s %s %s', [ dblToStrPrec(ptg.X, prec), dblToStrPrec(ptg.Y, prec),
                                            dblToStrPrec(ptg.Z, prec), dblToStrPrec(ptg.M, prec) ]
                          )
        else
          coord := Format( '%s %s', [ dblToStrPrec(ptg.X, prec), dblToStrPrec(ptg.Y, prec) ] ) ;
      end ;
  ...
  var
    ...
    begin
      ...
      if force_ccw then begin
        tpl := TGIS_Topology.Create ;
        try
          for i := 0 to icnt do begin
            ps   := tpl.PartStatus( TGIS_ShapePolygon( _shp ), i, p ) ;
            iscw := isClockwisePart( _shp, i ) ;
            sb.Append( '[' ) ;
            jcnt := _shp.GetPartSize( i ) - 1 ;
            if ( (ps >= 0) and iscw ) or ( (ps < 0) and not iscw ) then begin
              for j := jcnt downto 0 do begin
                appendCoordinate( i, j ) ;
                if j <> 0 then
                  sb.Append( ',' )
              end ;
  ...
  begin
    ...
    prec      := getDblPrec ;
    force_ccw := getJSONPolygonRightHandRule ;
          ...
          if ( int_count > 0 ) then begin
            ...
            for r := part_no to part_no + int_count do begin
              ...
              if ( r = part_no ) then begin
                points := _shp.GetPartSize( r ) ;
                // Polygon Geometry
                if force_ccw and isClockwisePart( _shp, r ) then begin
                  for p := points-1 downto 0 do begin
                    appendCoordinate( r, p ) ;
                    if p <> 0 then
                      sb.Append( ',' )
                  end ;
                ...
                points := _shp.GetPartSize( r ) ;
                // Polygon hole Geometry
                if force_ccw and not isClockwisePart( _shp, r ) then begin
                  for p := points-1 downto 0 do begin
                    appendCoordinate( r, p ) ;
                    if p <> 0 then
                      sb.Append( ',' )
            ...
            points := _shp.GetPartSize( part_no ) ;
            // Polygon Geometry
            if force_ccw and isClockwisePart( _shp, part_no ) then begin
              for p := points - 1 downto 0 do begin
                appendCoordinate( part_no, p ) ;
                if p <> 0 then
                  sb.Append( ',' )
