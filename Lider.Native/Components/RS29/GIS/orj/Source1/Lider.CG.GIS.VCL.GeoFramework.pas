...
unit Lider.CG.GIS.VCL.GeoFramework ;
...
type
  ...
  TGIS_BitmapInternal = class
    ...
    public
      ...
      /// <summary>
      ///   VCL bitmap used during standard VCL rendering.
      /// </summary>
      property Bitmap    : VCL.Graphics.TBitmap
                                       read  FBitmap ;
      /// <summary>
...
function TGIS_BitmapVCL.fget_NativeBitmap
  : TObject ;
begin
  Result := FBitmap ;
end;

procedure TGIS_BitmapVCL.fset_NativeBitmap(
...
begin
  ...
  // force GdiPlus renderer for faster creation
  rdr := TGIS_RendererVclGdiPlus.Create ;
  try
    vwr := TGIS_ViewerBmp.Create( TBitmap( self.NativeBitmap ), rdr ) ;
    try
      if _ppi > 0 then
        vwr.CustomPPI := _ppi ;
...
var
  ...
  old_cnt : Boolean          ;
begin
  vwr := TGIS_ViewerBmp.Create( TBitmap( self.NativeBitmap ) ) ;
  try
    if _ppi > 0 then
      vwr.CustomPPI := _ppi ;
...
begin
  FreeObject( FViewer ) ;
  FViewer := TGIS_ViewerBmp.Create( TBitmap( self.NativeBitmap ), nil ) ;
...
begin
  ...
  else if assigned( Bitmap ) then begin
    Bitmap.Canvas.Brush.Style := bsSolid ;
    Bitmap.Canvas.Brush.Color := _color ;
