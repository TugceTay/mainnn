...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoLayerPixel ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoLayerPixel"'}
{$ENDIF}
{$IFDEF CLR}
...
begin
  ...
  if baseRotation then begin
    player.cntrRotX := player.FExtent.XMin ;
    player.cntrRotY := player.FExtent.YMax ;
...
begin
 ...
 then
  ...
  if assigned(cutpolygon) then begin
    ...
    if (cut_common_ext.XMin <> cut_common_ext.XMax) and
       (cut_common_ext.YMin <> cut_common_ext.YMax) then
    begin
      ...
      for icc := low(cut_idx_y) to high(cut_idx_y) do begin
        ycutv := ptg1.Y -icc*lh ;
        if (ycutv >=   cut_ext.YMax) or (ycutv <= cut_ext.YMin) then
      ...
      for icc := low(cut_idx_x) to high(cut_idx_x) do begin
        xcutv := ptg1.X +icc*lw ;
        if (xcutv >=   cut_ext.XMax) or (xcutv <= cut_ext.XMin) then
          cut_idx_x[icc] := -1
        else
        cut_idx_x[icc] :=
...
type
  ...
  T_AltitudeZoneGenerator = class
    ...
    private
      ...
      /// <summary>
      ///   Generate proper definition for Params.Pixel.AltitudeMapZones property.
      /// </summary>
      /// <param name="_fmt">
      ///   format string
      /// </param>
      ...
      ///   zone definition
      /// </returns>
      function prepareZoneDefinition   ( const _fmt     : String ;
                                         const _min     : Double ;
                                         const _max     : Double ;
                                         const _color   : TGIS_Color
                                       ) : String ;
    ...
    public
      ...
      /// <summary>
      ///   Generates single zone definition to Params.Pixel.AltitudeMapZones property.
      /// </summary>
      /// <param name="_fmt">
      ///   format string
      /// </param>
      ...
      ///   zone color
      /// </param>
      /// <param name="_is_last_zone">
      ///   True, if zone is last
      /// </param>
      procedure AddZoneDefinition      ( const _fmt          : String ;
                                         const _min          : Double ;
                                         const _max          : Double ;
                                         const _color        : TGIS_Color ;
                                         const _is_last_zone  : Boolean
                                       ) ;
  end;
  ...
  function T_AltitudeZoneGenerator.prepareZoneDefinition(
    const _fmt   : String ;
    const _min   : Double ;
    const _max   : Double ;
    const _color : TGIS_Color
  ...
  begin
    ...
    Result := Format(
      _fmt,
      [ DotFloatToStrPrec( _min, -precDigits ),
        DotFloatToStrPrec( _max, -precDigits ),
        ConstructParamColor( _color ),
        DotFloatToStrPrec( _min, -precDigits ) ]
    ) ;
  end;
  ...
  begin
    ...
    if _is_last_zone and not GisIsSameValue( zone_max_prec, _max, dZEpsilon ) then
      zone_max_prec := doPrec( _max, 1 ) ;

    zone_def := prepareZoneDefinition(
      _fmt,
      doPrec( _min ),
      zone_max_prec,
      _color
  ...
  begin
    ...
    for i := 0 to zones_count - 1 do begin
      ...
      zone_max_prec := doPrec( zone_max ) ;
      if is_last_zone and not GisIsSameValue( zone_max_prec, _max_val, dZEpsilon ) then
        // but if rounded max value doesn't match exact max, use precision offset = 1
        zone_max_prec := doPrec( zone_max, 1 ) ;
...
begin
  ...
  then begin
    ...
    {$ELSE}
      while ( not ready ) and ( ( GetTickCount - tm ) < GETBITMAP_TIMEOUT ) do begin
    {$ENDIF}
...
begin
  ...
  if assigned(tempFileStream) then begin
    tempFileStream.Position := Int64( _linenr * baseCellWidth + _startIdx) * sizeOf( Single ) ;
    {$IFDEF OXYGENE}
  ...
  try
    ...
    for idx := 1 to color_map_len - 1 do begin
      ...
      begin
        ...
        zone_gen.AddZoneDefinition( fmt, curr_min, curr_max, curr_color, False ) ;
        curr_min := zone_min ;
        curr_max := zone_max ;
    ...
    fmt := '%s,%s,%s,%s' ;
    zone_gen.AddZoneDefinition( fmt, curr_min, curr_max, curr_color, True ) ;
...
var
  ...
  xrot : Double ;
  pt   : TGIS_Point  {$IFDEF GIS_NORECORDS} := new TGIS_Point {$ENDIF} ;
  rpt  : TGIS_Point  {$IFDEF GIS_NORECORDS} := new TGIS_Point {$ENDIF} ;
begin
  scaleX := _a ;
  ...
  if baseRotation then begin
    pt.X := FExtent.XMin + FBitWidth*scaleX ;
    pt.Y := FExtent.YMin - FBitHeight*scaleY ;
    rpt := basePointRot(pt) ;

    if (Abs(rpt.X - pt.X) < (scaleX/4)) and (Abs(rpt.Y - pt.Y) < (-scaleY/4)) then
      baseRotation := False ;
  end ;
end ;
...
begin
  ...
  if baseRotation then begin
    if (scxF_yRotDivScxRotDivSMcsyF <> 0) then begin
      ...
      Result.X := xRotDivSc*tmp.Y ;

      Result.Y := yRotDivSc*tmp.X ;
    end;
  end;
end ;
...
begin
  ...
  if baseRotation then begin
    {$IFDEF GIS_NORECORDS}pt := new TGIS_Point;{$ENDIF}
    pt.X := FExtent.XMin + FBitWidth*scaleX ;
    pt.Y := FExtent.YMin - FBitHeight*scaleY ;
    rpt := basePointRot(pt) ;
...
begin
  ...
  if IsGridImage then begin
    ...
    if (FGridBand <> Params.Pixel.GridBand) or (FGridBand <> mmzgbn) then begin
      FGridBand := Params.Pixel.GridBand ;
      prepareMinMaxZ ;
      FMinThresholdZ := -GIS_MAX_SINGLE ;
      FMaxThresholdZ :=  GIS_MAX_SINGLE ;
...
begin
  ...
  if do_transform then begin
    trans := T_TransClass.Create(Self) ;
    try
      while ( not Result ) and ( ( GetTickCount - tm ) < GETBITMAP_TIMEOUT ) do
...
begin
  ...
  while ( not Result ) and ( ( GetTickCount - tm ) < GETBITMAP_TIMEOUT ) do begin
