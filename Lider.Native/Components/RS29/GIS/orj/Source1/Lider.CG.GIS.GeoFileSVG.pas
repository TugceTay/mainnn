...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoFileSVG ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoFileSVG"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
  ...
  begin
    ...
    try
      ...
      vcx := RoundS( ( viewBox.Right  - viewBox.Left ) / 2 ) ;
      vcy := RoundS( ( viewBox.Bottom - viewBox.Top  ) / 2 ) ;
        ...
        {$ENDIF}
          ...
          if Points > 0 then begin
            ...
            if ( Kind = SVG_ENTRY_LINE ) or ( Kind = SVG_ENTRY_SURFACE ) or
              ...
              htxt := _canvas.CanvasTextExtent( 'Wy' ).Y ;

              len := len - 2*sizeOf(Double)-sizeOf(Integer) ;
              if ( len > 0 ) then begin
                SetLength( btext, len ) ;
                {$IFDEF OXYGENE}
                ...
                //RectEx
                rct := RectF( pt.X, pt.Y - htxt,
                              pt.X+32000, pt.Y+32000
                            ) ;
                rot := rot + _symRot ;
                ...
                if Abs( _canvas.CanvasFont.Size ) > 1 then begin
                  if rot <> 0 then begin
                    ...
                    _canvas.CanvasSetTransformation(
                       rot,
                       RoundS( pt.X ),
                       RoundS( pt.Y )
                    )  ;
                  end ;
                  rcti := Rect( RoundS( rct.Left   ),
                                RoundS( rct.Top    ),
                                RoundS( rct.Right  ),
                                RoundS( rct.Bottom )
                              ) ; // make it RectF some day
                  _canvas.CanvasDrawText( rcti, TEncoding.UTF8.GetString( btext ) ) ;
                  if rot <> 0 then
