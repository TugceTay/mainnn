...
{$IFDEF DCC}
  unit GisRegistredLayers ;
  {$HPPEMIT '#pragma link "GisRegistredLayers"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  begin
    ...
    else if IsEmbeddedSQLPath( _path ) and (i < StringFirst) then
      // sql path but without url and ? parameter
      path := parsePath( _path, drv )
    else
      path := _path ;
    ...
    if not assigned( Result ) then begin
      if _u( _path  ) then begin
        {$IFDEF MONO OR ISLAND OR BLAZOR}
          {$WARNING '### Verify MONO code'}
          {$WARNING '### Verify ISLAND code'}
          {$WARNING '### Verify BLAZOR code'}
        {$ELSE}
  ...
  {$IFDEF OXYGENE}
    ...
    begin
      ...
      try
        ...
        try
          {$IFDEF CLR}
            {$IFDEF DOTNET_STANDARD or BLAZOR}
