...
uses
  System.SysUtils, System.Types, System.UITypes, System.Classes, System.TimeSpan,
...
type
  TfrmMain = class(TForm)
    ...
    procedure chkbxUseOpenCLChange(Sender: TObject);
...
begin
  if not Lider.CG.GIS.GeoOpenCLEngine.Available then begin
    ShowMessage( 'OpenCL is not available. Falling back to CPU...' ) ;
    chkbxUseOpenCL.IsChecked := False ;
    chkbxUseOpenCL.Enabled := False ;
    btnOpenCLInfo.Enabled := False ;
    btnDeviceInfo.Enabled := False ;
  end
  else
    Lider.CG.GIS.GeoOpenCLEngine.Enabled := True ;

  GIS.Open( GisSamplesDataDir + 
...
begin
  ...
  if not Lider.CG.GIS.GeoOpenCLEngine.Enabled then
    clearOutput ;
...
procedure TfrmMain.chkbxUseOpenCLChange(Sender: TObject);
begin
  Lider.CG.GIS.GeoOpenCLEngine.Enabled := chkbxUseOpenCL.IsChecked ;
end ;
...
begin
  ...
  try
    ...
    for i := 0 to Lider.CG.GIS.GeoOpenCLEngine.PlatformCount - 1 do begin
      pi := Lider.CG.GIS.GeoOpenCLEngine.Platforms[i] ;
...
var
  ...
  l   : Integer ;
begin
  di := Lider.CG.GIS.GeoOpenCLEngine.ActiveDevice ;
  sl := TStringList.Create ;
  try
