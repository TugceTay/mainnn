...
{$IFDEF DCC}
  unit GisLayerTIFF ;
  {$HPPEMIT '#pragma link "GisLayerTIFF"'}
{$ENDIF}
{$IFDEF CLR}
...
//?
  ...
  const
    ...
    begin
      Result := aun.ToBase( defval( _default, _value1, _value2, _value3 ) ) ;
    end;
    ...
    begin
      Result := lun.ToBase( defval( _default, _value1, _value2, _value3 ) ) ;
    end;
      ...
      case keyProjCoordTrans of
         1 : // CT_TransverseMercator
           begin
             ...
             prm.FalseEasting       := deflin( 0, keyProjFalseEasting      ) ;
             prm.FalseNorthing      := deflin( 0, keyProjFalseNorthing     ) ;
           end;
         ...
         7 : // CT_Mercator,
           begin
             ...
             prm.ScaleFactor        := defval( 1, keyProjScaleAtNatOrigin  ) ;
             prm.FalseEasting       := deflin( 0,
                                               keyProjCenterEasting   ,
                                               keyProjFalseEasting
                                             ) ;
             prm.FalseNorthing      := deflin( 0,
         ...
         8 : // CT_LambertConfConic_2SP,
           begin
             ...
             prm.StandardParallel_2 := defdeg( 0,
                                               keyProjStdParallel2
                                             ) ;
             prm.FalseEasting       := deflin( 0,
                                               keyProjFalseOriginEasting,
                                               keyProjFalseEasting
                                             ) ;
             prm.FalseNorthing      := deflin( 0,
                                               keyProjFalseOriginNorthing,
                                               keyProjFalseNorthing
                                             ) ;
           end;
         ...
         26 : // CT_NewZealandMapGrid,
           begin
             ...
             prm.FalseEasting       := deflin( 2510000, keyProjFalseEasting  ) ;
             prm.FalseNorthing      := deflin( 6023150, keyProjFalseNorthing ) ;
           end;
  ...
  begin
    try
      ...
      if ( scaleX = 0 ) and ( scaleY = 0 ) then begin
        ...
        if (scale_x = 0) and ( high( tiepoint ) >= 9)  then begin
          scale_x := (tiepoint[9] -tiepoint[3])/FBitWidth ;
          scale_y := - scale_x ;
        end;
        ...
        then  begin
          ...
          if ptag <> -1 then begin
            ...
            if doubleparams[ 5 ] > 0 then begin
              scaleYFactor := -1 ;
              scaleY:= -scaleY;
              baseRotation := True ;
            end
            ...
            ext.XMin := doubleparams[ 3 ] ;
            ext.XMax := doubleparams[ 3 ] ;

            ext.YMin := doubleparams[ 7 ] ;
            ext.YMax := doubleparams[ 7 ] ;
            if (xrot <> 0) or (yrot <> 0) or baseRotation then begin
              ...
              if (scxF_yRotDivScxRotDivSMcsyF <> 0) and (scyF_yRotDivScxRotDivSMcsxF <> 0)then
                ...
                if rot90 then begin
                    ...
                    if doubleparams[ 1 ]*doubleparams[ 4 ] > 0 then
                     xRotDivSc := -xRotDivSc ;
                end
                else
                  baseRotation := False ;
...
//?    maxsidx := length(_buffSrc) -1 ;
      ...
      begin
        ...
        if isTiffTiled then begin
          ...
          if colidx = cols -1 then begin
            bytes := _pixCount*((bitsPerPixel +7) div 8) ;
        ...
        if isCompressed then begin
          ...
          if psi.StripByteCounts <> 0 then begin
          {$IFDEF OXYGENE}
          ...
          {$ENDIF}
            ...
            for bytes := 0 to tilebytes -1 do
               wbuffer[buffoff +bytes] := 0 ;
            readbytes := tilebytes ;
          end;
  ...
  begin
    ...
    try
      ...
      if isPlanar1 then begin
        SetLength(buf, bytescount +bytesPerPixel*startIdx) ;
...
//==============================================================================
  ...
  class procedure Unit_GisLayerTIFF.SelfRegisterLayer() ;
  begin
...
{$IFDEF DCC}
  initialization
    Unit_GisLayerTIFF.SelfRegisterLayer() ;
{$ENDIF}
