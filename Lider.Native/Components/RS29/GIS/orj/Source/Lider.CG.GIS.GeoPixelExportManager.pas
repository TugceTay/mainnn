...
{$IFDEF DCC}
  unit GisPixelExportManager ;
  {$HPPEMIT '#pragma link "GisPixelExportManager"'}
{$ENDIF}
{$IFDEF CLR}
    ...
    begin
      ...
      try
        if not oLayer.IsGridImage then begin
          ...
          try
            bmp.Assign( oDrawer.GIS_Bitmap ) ;
            // make sure the data is native
            bmp.GetData( NativeBitmapFactory ) ;
