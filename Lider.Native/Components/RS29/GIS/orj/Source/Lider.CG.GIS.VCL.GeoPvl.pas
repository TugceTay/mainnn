...
//=============================================================================
{
  Implementation of PVL controls for VCL
}

unit VCL.GisPvl;
{$HPPEMIT '#pragma link "VCL.GisPvl"'}
...
type
  ...
  type
    ...
    TGIS_PvlControlVcl = class;
    TGIS_PvlContextVcl = class;

  {#gendoc:hide}
  // component to be added as a proxy to a form to allow automatic disposal
  TGIS_PvlComponentVcl = class( TComponent )
    public
    ...
    private
      oOwner                      : TGIS_PvlContextVcl;
      oMaster                     : TGIS_PvlContext;
      lstControls                 : TObjectList<TGIS_PvlControlVcl>;
    public
      procedure   Add             ( const _control    : TGIS_PvlControlVcl
                                  );
      procedure   Remove          ( const _control    : TGIS_PvlControlVcl
                                  );

  end;

  {#gendoc:hide}
  TGIS_PvlContextVcl = class( TGIS_PvlContextBase, IGIS_PvlContext )
    public
    ...
    protected
      ...
      iVMargin                    : Integer;
      bBiDi                       : Boolean;
      oOwner                      : TGIS_PvlComponentVcl; // component to be owned by
  ...
  {#gendoc:hide}
  TGIS_PvlObjectVcl = class( TGIS_PvlBaseVcl, IGIS_PvlObject )
    public
      constructor Create          ( const _context    : TGIS_PvlContext;
                                    const _parent     : IGIS_PvlBase
                                  );
  ...
  {#gendoc:hide}
  TGIS_PvlControlVcl = class( TGIS_PvlBaseVCL, IGIS_PvlControl )
    private
...
function checkIfTransparent( _bmp : TBitmap ) : Boolean ;
begin
  Result := _bmp.Canvas.Pixels[0,0] = clFuchsia ;
end;

function convertBitmapToDisabled( const _bmp : TBitmap; const _trnsp : Boolean ) : TBitmap;
type
  TRGBArray = array[0..32767] of TRGBTriple;
  PRGBArray = ^TRGBArray;
var
...
begin
  if not _trnsp then begin
    ...
    for y := 0 to Result.Height - 1 do begin
      ...
      for x := 0 to Result.Width - 1 do begin
        gray := ( row[ x ].rgbtBlue + row[ x ].rgbtGreen + row[ x ].rgbtRed + 2 * 255 ) div ( 3 + 2 );
        row[x].rgbtRed    := gray;
        row[x].rgbtGreen  := gray;
        row[x].rgbtBlue   := gray;
      end;
...
begin
  ...
  if System.Classes.ssHorizontal in _value then
...
{$ENDREGION}
  ...
  /// </summary>
  T_PvlImagePanel = class( TCustomControl )
    public
      constructor Create          (       _owner       : TComponent
                                  );
                                  override;
  ...
  T_PvlLabel = class( TGIS_PvlControlVcl, IGIS_PvlLabel )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlIconButton = class ( TGIS_PvlControlVcl, IGIS_PvlIconButton )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlButton = class( TGIS_PvlControlVcl, IGIS_PvlButton )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlModalButton = class( TGIS_PvlControlVcl, IGIS_PvlModalButton )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlEdit = class( TGIS_PvlControlVcl, IGIS_PvlEdit )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlMemo = class( TGIS_PvlControlVcl, IGIS_PvlMemo )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlColorPreview = class ( TGIS_PvlControlVcl, IGIS_PvlColorPreview )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlColorWheel = class( TGIS_PvlControlVcl, IGIS_PvlColorWheel )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlColorBar = class( TGIS_PvlControlVcl, IGIS_PvlColorBar )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlCustomComboBox = class( TGIS_PvlControlVcl, IGIS_PvlCustomComboBox )
  ...
  T_PvlColorRampComboBox = class ( TGIS_PvlControlVcl, IGIS_PvlColorRampComboBox )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlComboBox = class( TGIS_PvlControlVcl, IGIS_PvlComboBox )
    private
      bManualFontHeight           : Boolean ;
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlSearchBox = class ( TGIS_PvlControlVcl, IGIS_PvlSearchBox )
    private
      FOnKeyPress                 : TGIS_PvlKeyPressEvent    ;
      FOnChange                   : TGIS_PvlEvent;
      oList                       : TListBox ;
  ...
  T_PvlRadioButton = class ( TGIS_PvlControlVcl, IGIS_PvlRadioButton )
    private
      FOnChange                   : TGIS_PvlEvent ;
    public
      /// <inheritdoc"/>
  ...
  T_PvlCheckBox = class ( TGIS_PvlControlVcl, IGIS_PvlCheckBox )
    private
      FOnChange                   : TGIS_PvlEvent ;
    public
      /// <inheritdoc"/>
  ...
  T_PvlPanel = class ( TGIS_PvlControlVcl, IGIS_PvlPanel )
    protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlCanvas = class( TGIS_PvlControlVcl, IGIS_PvlCanvas )
    private
  ...
  T_PvlPreviewPanel = class( TGIS_PvlControlVcl, IGIS_PvlPreviewPanel  )
     protected
      /// <inheritdoc"/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlGroupBox = class ( TGIS_PvlControlVcl, IGIS_PvlGroupBox )
    public
      /// <inheritdoc/>
      procedure doCreate          ( const _context    : TGIS_PvlContext
                                  );
  ...
  T_PvlTrackBar = class( TGIS_PvlControlVcl, IGIS_PvlTrackBar )
    protected
      FOnChange                   : TGIS_PvlEvent;
    protected
      /// <inheritdoc"/>
  ...
  T_PvlTree = class ( TGIS_PvlControlVcl, IGIS_PvlTree )
    protected
  ...
  T_PvlListBox = class( TGIS_PvlControlVcl, IGIS_PvlListBox )
    private
  ...
  T_PvlSVGList = class( TGIS_PvlControlVcl, IGIS_PvlSVGList )
    private
      lstItm    : TGIS_ListOfStrings ;
      FOnChange : TGIS_PvlEvent ;
     protected
...
{$REGION 'T_PvlImagePanel'}
constructor T_PvlImagePanel.Create(
    _owner : TComponent
  );
begin
  inherited;
...
procedure T_PvlImagePanel.Paint;
var
  w, h   : Integer;
  rw, rh : Integer;
begin
...
{$REGION 'T_SpeedButton'}
  ...
  constructor T_SpeedButton.Create(_owner: TComponent);
  begin
    inherited ;
    Images := TImageList.Create( Self ) ;
    DisabledImages := TImageList.Create( Self ) ;
  end;
  ...
  begin
    ...
    if Enabled then
      ...
      DisabledImages.Draw(Canvas, (Width - Images.Width) div 2,
        (Height - Images.Height) div 2, 0, True);
  end;
...
{$IFDEF LEVEL_RX101_VCL}
  ...
  var
    i   : Integer;
    cmp : TGIS_PvlComponentVcl;
    itm : TGIS_PvlControlVcl;
  begin
    ...
    for i := 0 to oForm.ComponentCount -1 do begin
      if oForm.Components[i] is TGIS_PvlComponentVcl then begin
        cmp := TGIS_PvlComponentVcl( oForm.Components[i] );
        for itm in cmp.lstControls do begin
...
{$REGION 'TGIS_PvlComponentVcl'}

constructor TGIS_PvlComponentVcl.Create(
  _owner : TComponent
);
begin
  inherited;
  lstControls := TObjectList<TGIS_PvlControlVcl>.Create;
  lstControls.OwnsObjects := False ;
end;

destructor TGIS_PvlComponentVcl.Destroy;
begin
...
procedure TGIS_PvlComponentVcl.Add(
  const _control : TGIS_PvlControlVcl
);
begin
  lstControls.Add( _control );
end;

procedure TGIS_PvlComponentVcl.Remove(
  const _control : TGIS_PvlControlVcl
);
begin
...
{$ENDREGION 'TGIS_PvlComponentVcl'}
...
{$ENDREGION 'TGIS_PvlBaseVCL'}

{$REGION 'TGIS_PvlObjectVcl'}
constructor TGIS_PvlObjectVcl.Create(
  const _context : TGIS_PvlContext;
  const _parent  : IGIS_PvlBase
);
begin
...
procedure TGIS_PvlObjectVcl.doCreate(
  const _context: TGIS_PvlContext
) ;
begin
...
procedure TGIS_PvlObjectVcl.doDestroy;
begin
  TGIS_PvlObject( oParent ).Platform := nil ;

  inherited;
end;
{$ENDREGION 'TGIS_PvlObjectVcl'}

{$REGION 'TGIS_PvlControlVcl'}
constructor TGIS_PvlControlVcl.Create(
  const _context : TGIS_PvlContext ;
  const _parent  : IGIS_PvlBase
);
begin
  ...
  if _context.RightToLeft then
    TControl( oControl ).BiDiMode := TBiDiMode.bdRightToLeft
  else
    TControl( oControl ).BiDiMode := TBiDiMode.bdLeftToRight ;


  TGIS_PvlContextVcl( _context.Platform ).oOwner.Add( self );
end;

procedure TGIS_PvlControlVcl.doDestroy;
begin
  TGIS_PvlControl( oParent ).Platform := nil ;
  inherited;
end;

procedure TGIS_PvlControlVcl.DoRedraw;
begin
  // only for safe inheritance
end;

procedure TGIS_PvlControlVcl.ProcessMessages;
begin
  Application.ProcessMessages ;
end;

procedure TGIS_PvlControlVcl.doOnClick(
        _sender     : TObject
);
begin
...
procedure TGIS_PvlControlVcl.doOnDblClick(
        _sender     : TObject
);
begin
...
procedure TGIS_PvlControlVcl.doOnMouseDown(
        _sender     : TObject       ;
        _button     : TMouseButton  ;
        _shift      : TShiftState   ;
        _x          : Integer       ;
...
procedure TGIS_PvlControlVcl.doOnMouseMove(
        _sender     : TObject       ;
        _shift      : TShiftState   ;
        _x          : Integer       ;
        _y          : Integer
...
procedure TGIS_PvlControlVcl.doOnMouseUp(
        _sender     : TObject       ;
        _button     : TMouseButton  ;
        _shift      : TShiftState   ;
        _x          : Integer       ;
...
procedure TGIS_PvlControlVcl.doOnMouseWheel(
        _sender     : TObject       ;
        _shift      : TShiftState   ;
        _delta      : Integer       ;
        _pos        : TPoint        ;
...
procedure TGIS_PvlControlVcl.doOnMouseEnter(
        _sender     : TObject
) ;
begin
...
procedure TGIS_PvlControlVcl.doOnMouseLeave(
        _sender     : TObject
) ;
begin
...
function TGIS_PvlControlVcl.fget_OnClick
  : TGIS_PvlEvent;
begin
  Result := FOnClick;
end;

procedure TGIS_PvlControlVcl.fset_OnClick(
  const _value: TGIS_PvlEvent
);
begin
  FOnClick := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnClick := doOnClick
  else
    T_Control( oControl ).OnClick := nil ;
end;

function TGIS_PvlControlVcl.fget_OnDblClick
  : TGIS_PvlEvent;
begin
  Result := FOnDblClick;
end;

procedure TGIS_PvlControlVcl.fset_OnDblClick(
  const _value : TGIS_PvlEvent
);
begin
  FOnDblClick := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnDblClick := doOnDblClick
  else
    T_Control( oControl ).OnDblClick := nil ;
end;

function TGIS_PvlControlVcl.fget_OnMouseDown
  : TGIS_PvlMouseEvent;
begin
  Result := FOnMouseDown;
end;

procedure TGIS_PvlControlVcl.fset_OnMouseDown(
  const _value : TGIS_PvlMouseEvent
);
begin
  FOnMouseDown := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnMouseDown := doOnMouseDown
  else
    T_Control( oControl ).OnMouseDown := nil ;
end;

function TGIS_PvlControlVcl.fget_OnMouseMove
  : TGIS_PvlMouseMoveEvent;
begin
  Result := FOnMouseMove;
end;

procedure TGIS_PvlControlVcl.fset_OnMouseMove(
  const _value : TGIS_PvlMouseMoveEvent
);
begin
  FOnMouseMove := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnMouseMove := doOnMouseMove
  else
    T_Control( oControl ).OnMouseMove := nil ;
end;

function TGIS_PvlControlVcl.fget_OnMouseUp
  : TGIS_PvlMouseEvent;
begin
  Result := FOnMouseUp;
end;

procedure TGIS_PvlControlVcl.fset_OnMouseUp(
  const _value : TGIS_PvlMouseEvent
);
begin
  FOnMouseUp := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnMouseUp := doOnMouseUp
  else
    T_Control( oControl ).OnMouseUp := nil ;
end;

function TGIS_PvlControlVcl.fget_OnMouseWheel
  : TGIS_PvlMouseWheelEvent;
begin
  Result := FOnMouseWheel;
end;

procedure TGIS_PvlControlVcl.fset_OnMouseWheel(
  const _value : TGIS_PvlMouseWheelEvent
);
begin
  FOnMouseWheel := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnMouseWheel := doOnMouseWheel
  else
    T_Control( oControl ).OnMouseWheel := nil ;
end;

function TGIS_PvlControlVcl.fget_OnMouseEnter
  : TGIS_PvlEvent;
begin
  Result := FOnMouseEnter;
end;

procedure TGIS_PvlControlVcl.fset_OnMouseEnter(
  const _value : TGIS_PvlEvent
);
begin
  FOnMouseEnter := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnMouseEnter := doOnMouseEnter
  else
    T_Control( oControl ).OnMouseEnter := nil ;
end;

function TGIS_PvlControlVcl.fget_OnMouseLeave
  : TGIS_PvlEvent;
begin
  Result := FOnMouseLeave;
end;

procedure TGIS_PvlControlVcl.fset_OnMouseLeave(
  const _value : TGIS_PvlEvent
);
begin
  FOnMouseLeave := _value ;
  if Assigned( _value ) then
    T_Control( oControl ).OnMouseLeave := doOnMouseLeave
  else
    T_Control( oControl ).OnMouseLeave := nil ;
end;

procedure TGIS_PvlControlVcl.add(
...
procedure TGIS_PvlControlVcl.fset_Context(
  const _value: TGIS_PvlContext
) ;
begin
  TGIS_PvlContextVcl( _value.Platform ).oOwner.Add( self );
  TGIS_PvlContextVcl( oContext.Platform ).oOwner.Remove( self );
  oContext := _value ;
  TControl( oControl ).Parent := TWinControl( _value.NativeParent );
end;

function TGIS_PvlControlVcl.fget_NativeControl
  : TObject;
begin
  Result := oControl;
end;

function TGIS_PvlControlVcl.fget_IsStyled
  : Boolean;
begin
  Result := True;
end;

function  TGIS_PvlControlVcl.fget_Anchors
  : TGIS_PvlAnchors;
begin
  Result := [];
...
procedure TGIS_PvlControlVcl.fset_Anchors(
...
function  TGIS_PvlControlVcl.fget_Align
  : TGIS_PvlAlign;
begin
...
procedure TGIS_PvlControlVcl.fset_Align(
...
function TGIS_PvlControlVcl.fget_Enabled
  : Boolean;
begin
  Result := TControl( oControl ).Enabled;
end;

procedure TGIS_PvlControlVcl.fset_Enabled(
  const _value : Boolean
);
begin
...
function TGIS_PvlControlVcl.fget_Width
  : Integer;
begin
  Result := Round( TControl( oControl ).Width / oContext.PPIFix );
end;

procedure TGIS_PvlControlVcl.fset_Width(
  const _value : Integer
);
begin
...
function TGIS_PvlControlVcl.fget_Height
  : Integer;
begin
  Result := Round( TControl( oControl ).Height / oContext.PPIFix );
end;

procedure TGIS_PvlControlVcl.fset_Height(
  const _value : Integer
);
begin
...
function TGIS_PvlControlVcl.fget_Left
  : Integer;
begin
  Result := Round( TControl( oControl ).Left / oContext.PPIFix );
end;

procedure TGIS_PvlControlVcl.fset_Left(
  const _value : Integer
);
begin
...
function TGIS_PvlControlVcl.fget_Top
  : Integer;
begin
  Result := Round( TControl( oControl ).Top / oContext.PPIFix );
end;

procedure TGIS_PvlControlVcl.fset_Top(
  const _value : Integer
);
begin
...
function TGIS_PvlControlVcl.fget_TabOrder
  : Integer;
begin
  Result := 0;
end;

procedure TGIS_PvlControlVcl.fset_TabOrder(
  const _value : Integer
);
begin
...
function TGIS_PvlControlVcl.fget_Visible
  : Boolean;
begin
  Result := TControl( oControl ).Visible;
end;

procedure TGIS_PvlControlVcl.fset_Visible(
  const _value : Boolean
);
begin
...
function TGIS_PvlControlVcl.fget_Hint
  : String;
begin
  Result := TControl( oControl ).Hint;
end;

procedure TGIS_PvlControlVcl.fset_Hint(
  const _value : String
);
begin
...
procedure TGIS_PvlControlVcl.SetFocus;
begin
//  No set focus for control
//  TControl( oControl ).SetFocus;
end;

{$ENDREGION 'TGIS_PvlControlVcl'}
...
begin
  ...
  if oContext.PPIFix > 1 then
  begin
    //Trick to ensure labels are not higher than it should be on 4k.
    rct.Left := 0;
    rct.Right := TLabel( oControl ).Width ;
    rct.Top := 0 ;
    rct.Bottom := 0 ;
    lbl := _value ;
    TLabel( oControl ).Canvas.Font := TLabel( oControl ).Font ;
    TLabel( oControl ).Canvas.TextRect(
              {var} rct, //will be modified to fit the text dimensions
              {var} lbl, //not modified, unless you use the "tfModifyingString" flag
              [tfCalcRect] //flags to say "compute text dimensions with line breaks"
            );
    assert( rct.Top = 0 ) ;

    TLabel( oControl ).Height := rct.Bottom ;
  end;
end;
...
function T_PvlLabel.fget_FontColor
  : TGIS_Color;
begin
  Result := GISColor( TLabel( oControl ).Font.Color );
end;
...
begin
  TLabel( oControl ).Font.Color := VCLColor( _value );
end;
...
begin
  ...
  try
    ...
    tmp.DrawGlyph(smb, oContext.PPI, clr, True);

    if TBitmap( tmp.NativeBitmap ).Canvas.Pixels[0,0] = 0 then
      // already transparent
      trnsp := True
    else begin
      trnsp := checkIfTransparent( TBitmap( tmp.NativeBitmap ) ) ;
    ...
    T_SpeedButton(oControl).Images.Clear ;
    ...
    T_SpeedButton(oControl).Images.DrawingStyle := TDrawingStyle.dsTransparent;

    bmptrnsp := convertBitmapToDisabled(bmp, trnsp) ;

    T_SpeedButton(oControl).DisabledImages.Clear ;
    T_SpeedButton(oControl).DisabledImages.Height := tmp.Height;
    T_SpeedButton(oControl).DisabledImages.Width := tmp.Width;
    T_SpeedButton(oControl).DisabledImages.ColorDepth := TColorDepth.cd32Bit;
    T_SpeedButton(oControl).DisabledImages.AddMasked(bmptrnsp, bmp.TransparentColor);
    T_SpeedButton(oControl).DisabledImages.DrawingStyle := TDrawingStyle.dsTransparent;
  finally
    FreeObject(tmp);
    FreeObject( bmptrnsp );
  end;
...
function T_PvlEdit.fget_FontSize
  : Integer;
begin
  Result := TEdit( oControl ).Font.Size;
end;
...
function T_PvlEdit.fget_FontFamily
  : string;
begin
  Result := TEdit( oControl ).Font.Name;
end;
...
begin
  TEdit( oControl ).Font.Name := _value;
end;
...
begin
  ...
  if _value then
    TEdit( oControl ).Color := clWhite
  else
    TEdit( oControl ).Color := clBtnFace ;
end;
...
begin
  TEdit( oControl ).Font.Color := RGB( 198, 31, 31 ) ;  // #c61f1f
end;
...
begin
  TEdit( oControl ).Font.Color := RGB( 0, 0, 238 )  ;
  TEdit( oControl ).Font.Style := TEdit( oControl ).Font.Style + [ TFontStyle.fsUnderline ];
end;

procedure T_PvlEdit.SetBackgroundAlarm;
begin
  TEdit( oControl ).Color := RGB( 250, 224, 224 ) ;  // #fae0e0
end;

procedure T_PvlEdit.SetBackgroundDefault;
begin
  TEdit( oControl ).Color := clWindow ;
end;
...
function T_PvlMemo.fget_FontSize
  : Integer;
begin
  Result := TMemo( oControl ).Font.Size;
end;
...
function T_PvlMemo.fget_FontFamily
  : string;
begin
  Result := TMemo( oControl ).Font.Name;
end;
...
begin
  TMemo( oControl ).Font.Name := _value;
end;
...
begin
  ...
  if _value then
    TMemo( oControl ).Color := clWhite
  else
    TMemo( oControl ).Color := clBtnFace ;
end;
...
begin
  TMemo( oControl ).Font.Color := clRed;
end;

procedure T_PvlMemo.SetFontDefault;
begin
  TMemo( oControl ).Font.Color := clWindowText;
end;
...
begin
  oControl := T_PvlImagePanel.Create( TWinControl( _context.NativeParent ) );
  TControl( oControl ).Parent := TWinControl( _context.NativeParent );
  ...
  T_PvlImagePanel( oControl ).OnPaintEvent := doOnPaint;
  T_PvlImagePanel( oControl ).OnMouseDown  := doMouseDown;
  T_PvlImagePanel( oControl ).OnMouseMove  := doMouseMove;
  T_PvlImagePanel( oControl ).OnMouseUp    := doMouseUp;
end;
...
begin
  ...
  colorWheel.SetSize( RoundS( T_PvlImagePanel( oControl ).Width ),
                      RoundS( T_PvlImagePanel( oControl ).Height )
  );
  ...
  colorWheel.Canvas.Rectangle( 0, 0, T_PvlImagePanel( oControl ).Width, T_PvlImagePanel( oControl ).Height );

  w2 := RoundS( ( T_PvlImagePanel( oControl ).Width  - GIS_COLOR_WHEEL_MARGIN )/2 );
  h2 := RoundS( ( T_PvlImagePanel( oControl ).Height - GIS_COLOR_WHEEL_MARGIN )/2 );
  ...
  currState.SetSize( T_PvlImagePanel( oControl ).Width, T_PvlImagePanel( oControl ).Height );
...
begin
  ...
  T_PvlImagePanel( oControl ).Canvas.Draw( 0, 0, currState );
end;
...
begin
  ...
  T_PvlImagePanel( oControl ).Repaint;
...
begin
  ...
  gradient.SetSize( T_PvlImagePanel( oControl ).Width, T_PvlImagePanel( oControl ).Height );
  ...
  gradient.Canvas.Rectangle( 0, 0, T_PvlImagePanel( oControl ).Width, T_PvlImagePanel( oControl ).Height );
  ...
  gradient.Canvas.Rectangle(
    ...
    GIS_COLOR_GRADIENT_MARGIN2 - 1,
    T_PvlImagePanel( oControl ).Width  - GIS_COLOR_GRADIENT_MARGIN2 + 1,
    T_PvlImagePanel( oControl ).Height - GIS_COLOR_GRADIENT_MARGIN2 + 1
  );
...
begin
  ...
  for i := GIS_COLOR_GRADIENT_MARGIN2
      to T_PvlImagePanel( oControl ).Height - GIS_COLOR_GRADIENT_MARGIN2 - 1 do begin

    ptr := gradient.ScanLine[i];

    for k := GIS_COLOR_GRADIENT_MARGIN2
        to T_PvlImagePanel( oControl ).Width - GIS_COLOR_GRADIENT_MARGIN2 - 1 do begin

      ptr[3*k  ] :=
        RoundS(
          (
            ( k - GIS_COLOR_GRADIENT_MARGIN2 )/
            ( T_PvlImagePanel( oControl ).Width - GIS_COLOR_GRADIENT_MARGIN )
          ) * FColor.B
        );
      ptr[3*k+1] :=
        RoundS(
...
begin
  ...
  currState.Canvas.Polygon( [
    Point( currX - 1 , 0          ),
    Point( currX - 2 , 1          ),
    Point( currX - 2 , T_PvlImagePanel( oControl ).Height - 2 ),
    Point( currX - 1 , T_PvlImagePanel( oControl ).Height - 1 ),
    Point( currX + 1 , T_PvlImagePanel( oControl ).Height - 1 ),
    Point( currX + 2 , T_PvlImagePanel( oControl ).Height - 2 ),
    Point( currX + 2 , 1          ),
    Point( currX + 1 , 0          ),
...
begin
 ...
 T_PvlImagePanel( oControl ).Invalidate;
end;
          ...
          begin
            if assigned( FGetBitmapEvent ) then begin
              ...
              if not TGIS_Bitmap.IsNilOrEmpty( bmp ) then begin
                if ( odDisabled in _state ) or not _control.Enabled then
...
var
  ...
  frme : VCL.GisLineSymbolEditor.TGIS_LineSymbolEditor;
...
begin
  ...
  if ar[2] = GIS_PARAMTXT_TYPE_CODE then begin
    frme := VCL.GisLineSymbolEditor.TGIS_LineSymbolEditor.create( TComponent( oContext.NativeParent ), True );
    try
...
begin
  ...
  with (Control as TComboBox) do begin
    ...
    if not Control.Enabled then begin
      bmp := convertBitmapToDisabled( TBitmap( bmpMap[Index] ), False );
      try
...
function T_PvlComboBox.fget_FontSize
  : Integer;
begin
  Result := TComboBox( oControl ).Font.Size;
end;
...
begin
  TComboBox( oControl ).Font.Size := _value;
  bManualFontHeight := True ;
end;
...
begin
  iStyle := GisGetEmptyFontStyle;
  if TFontStyle.fsBold in TComboBox( oControl ).Font.Style then
    iStyle := GisAddFontStyle( iStyle, TGIS_FontStyle.Bold );
  if TFontStyle.fsItalic in TComboBox( oControl ).Font.Style then
    iStyle := GisAddFontStyle( iStyle, TGIS_FontStyle.Italic );
  if TFontStyle.fsUnderline in TComboBox( oControl ).Font.Style then
    iStyle := GisAddFontStyle( iStyle, TGIS_FontStyle.Underline );
  if TFontStyle.fsStrikeOut in TComboBox( oControl ).Font.Style then
    iStyle := GisAddFontStyle( iStyle, TGIS_FontStyle.StrikeOut );
  Result := iStyle;
end;
...
begin
  if TGIS_FontStyle.Bold in _value then
    TComboBox( oControl ).Font.Style := TComboBox( oControl ).Font.Style + [ TFontStyle.fsBold ];
  if TGIS_FontStyle.Italic in _value then
    TComboBox( oControl ).Font.Style := TComboBox( oControl ).Font.Style + [ TFontStyle.fsItalic ];
  if TGIS_FontStyle.Underline in _value then
    TComboBox( oControl ).Font.Style := TComboBox( oControl ).Font.Style + [ TFontStyle.fsUnderline ];
  if TGIS_FontStyle.StrikeOut in _value then
    TComboBox( oControl ).Font.Style := TComboBox( oControl ).Font.Style + [ TFontStyle.fsStrikeOut ];
end;

function T_PvlComboBox.fget_FontFamily
  : string;
begin
  Result := TComboBox( oControl ).Font.Name;
end;
...
begin
  TComboBox( oControl ).Font.Name := _value;
end;
...
begin
  inherited;
  if ( _value > 0 ) and not bManualFontHeight then
...
begin
  TComboBox( oControl ).Font.Color := RGB( 198, 31, 31 ) ;  // #c61f1f
end;

procedure T_PvlComboEdit.SetFontDefault;
begin
  TComboBox( oControl ).Font.Color := clWindowText;
end;

procedure T_PvlComboEdit.SetBackgroundAlarm;
begin
  TComboBox( oControl ).Color := RGB( 250, 224, 224 ) ;  // #fae0e0
end;

procedure T_PvlComboEdit.SetBackgroundDefault;
begin
  TComboBox( oControl ).Color := clWindow ;
end;
...
begin
  ...
  oList := TListBox.Create( TWinControl( _context.NativeParent ) ) ;
  oList.Parent := TWinControl( _context.NativeParent ) ;
  oList.Visible := False ;
  oList.OnKeyDown := doListboxKeyDown;
...
begin
  ...
  oList.Left := TEdit( oControl ).Left ;
  oList.Top := TEdit( oControl ).Top + TEdit( oControl ).Height ;
  oList.Width := TruncS( fget_Width * oContext.PPIFix ) ;
  if oList.Items.Count < iDropDownCount then
    oList.ClientHeight := oList.ItemHeight * oList.Items.Count
  else
    oList.ClientHeight := oList.ItemHeight * iDropDownCount ;
   ...
   case _key of
    ...
    VK_RETURN:
      begin
        ...
        TEdit( oControl ).SetSelText(oList.Items[oList.ItemIndex]);
        bDropped := True ;
        dropUp;
      end;
  end;
...
begin
  ...
  if lstItems.IndexOf( sSearch ) <> -1 then
    exit ;
...
begin
  ...
  TGIS_ViewerWnd( oControl ).Parent := TWinControl( _context.NativeParent );
...
begin
  if Assigned( _value ) then begin
    ...
    // assign as PNG image because Picture does nos support semitransparent bitmaps
    strm := TMemoryStream.Create;
    try
      _value.SaveToStream( strm, TGIS_PixelFormat.ARGB, TGIS_PixelSubFormat.PNG, 100 );
      strm.Position := 0;
      {$IFNDEF LEVEL_RX102_VCL}
        png := TPngImage.Create ;
        try
          png.LoadFromStream( strm );
          oImage.Picture.Assign( png ) ;
        finally
          FreeObject( png );
        end;
      {$ELSE}
        oImage.Picture.LoadFromStream( strm ) ;
      {$ENDIF}
    finally
      FreeObject( strm) ;
    end;
...
begin
  ...
  TTreeView( oControl ).HideSelection := False;
...
begin
  TGIS_PvlTreeNode( _node.Data ).InternalRemove ;
...
begin
  if not Assigned( TTreeView( oControl ).Selected ) then
    Result := -1 
  else if not Assigned( TTreeView( oControl ).Selected.Parent ) then
    Result := TTreeView( oControl ).Selected.Index  //? To double check !
  else 
    Result := TTreeView( oControl ).Selected.Parent.IndexOf( TTreeView( oControl ).Selected ) ;
end;
...
begin
  ...
  TListView( oControl ).OnChange := doOnChange;
  applyStyle;
end;
...
procedure T_PvlSVGList.doOnChange(_sender     : TObject;
                                  _item       : TListItem;
                                  _change     : TItemChange);
begin
  if Assigned( FOnChange ) then
...
{$REGION 'TGIS_PvlContextVcl'}

constructor TGIS_PvlContextVcl.Create(
...
begin
  ...
  oOwner := TGIS_PvlComponentVcl.Create(TComponent(oParent));
...
procedure TGIS_PvlContextVcl.FreeContext ;
begin
  // force freeing of context
  oOwner.Free ;
end;

function TGIS_PvlContextVcl.GetAllFonts
  : TStringList;
begin
...
class function TGIS_PvlContextVcl.CreateOpenDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlOpenDialog
) : TGIS_PvlOpenDialog;
begin
...
class function TGIS_PvlContextVcl.CreateSaveDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlSaveDialog
) : TGIS_PvlSaveDialog;
begin
...
class function TGIS_PvlContextVcl.CreateSelectFolderDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlSelectFolderDialog
) : TGIS_PvlSelectFolderDialog;
begin
...
class function TGIS_PvlContextVcl.CreateOptionDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlOptionDialog
) : TGIS_PvlOptionDialog;
begin
...
class function TGIS_PvlContextVcl.CreateInfoDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlInfoDialog
) : TGIS_PvlInfoDialog ;
begin
...
class function TGIS_PvlContextVcl.CreateWarningDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlWarningDialog
) : TGIS_PvlWarningDialog ;
begin
...
class function TGIS_PvlContextVcl.CreateErrorDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlErrorDialog
) : TGIS_PvlErrorDialog ;
begin
...
class function TGIS_PvlContextVcl.CreateCSDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlControlCSSystem
) : TGIS_PvlControlCSSystem ;
begin
  Result := VCL.GisSystemForms.TGIS_ControlCSSystem.Create( _context, _self ) ;
end;

class function TGIS_PvlContextVcl.CreateLineSymbologyDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlLineSymbolEditor
) : TGIS_PvlLineSymbolEditor ;
begin
  Result := VCL.GisSystemForms.TGIS_LineSymbolEditor.Create( _context, _self ) ;
end;

class function TGIS_PvlContextVcl.CreatePrintPreviewSimpleDialog(
  const _context: TGIS_PvlContext;
  const _self   : TGIS_PvlControlPrintPreviewSimple
) : TGIS_PvlControlPrintPreviewSimple ;
begin
  Result := VCL.GisSystemForms.TGIS_ControlPrintPreviewSimple.Create( _context, _self ) ;
end;

class function TGIS_PvlContextVcl.Support(
  const _parent  : TObject
) : Boolean;
var
  o : TObject;
begin
  ...
  Result := ( o is TWinControl ) or ( not Assigned( _parent ) ) ;
end;

function TGIS_PvlContextVcl.CreateObject(
...
function TGIS_PvlContextVcl.ClientWidth
  : Integer;
begin
  Result := RoundS( TWinControl( oParent ).ClientWidth / fget_PPIFix );
end;

function TGIS_PvlContextVcl.fget_HMargin
  : Integer;
begin
  Result := iHMargin ;
end;

procedure TGIS_PvlContextVcl.fset_HMargin(
  const _value: Integer
) ;
begin
  iHMargin := _value ;
...
function TGIS_PvlContextVcl.fget_VMargin
  : Integer;
begin
  Result := iVMargin ;
end;

procedure TGIS_PvlContextVcl.fset_VMargin(
  const _value: Integer
) ;
begin
  iVMargin := _value ;
...
function TGIS_PvlContextVcl.fget_HSpace
  : Integer;
begin
  Result := 4 ;
end;

function TGIS_PvlContextVcl.fget_VSpace
  : Integer;
begin
  Result := 4 ;
end;

function TGIS_PvlContextVcl.fget_LSpace
  : Integer;
begin
  Result := 2 ;
end;

function TGIS_PvlContextVcl.fget_CanvasScale
  : Single;
begin
  Result := oMaster.CanvasScale; //?dCanvasScale;
end;

function TGIS_PvlContextVcl.fget_PPIFix
  : Single;
begin
  Result := oMaster.PPIFix;//?dPPIFix;
end;

function TGIS_PvlContextVcl.fget_PPI
  : Integer;
begin
  Result := oMaster.PPI//?iPPI;
end;

{$ENDREGION 'TGIS_PvlContextVcl'}
...
var
  ...
  itm : TGIS_PvlControlVcl;
begin
  for i := 0 to TForm(T_dpiManager(_form).oForm).ComponentCount -1 do begin
    if TForm(T_dpiManager(_form).oForm).Components[i] is TGIS_PvlComponentVcl then begin
      cmp := TGIS_PvlComponentVcl( TForm(T_dpiManager(_form).oForm).Components[i] );
      if _redraw then
        for itm in cmp.lstControls do begin
  //        itm.oContext.UpdatePPI( _ppi, _ppifix, 1);
          itm.oContext.Refresh ;
          itm.DoRedraw;
        end;
    end;
  end;
...
initialization
  TGIS_PvlContext.&Register( TGIS_PvlContextVcl );
