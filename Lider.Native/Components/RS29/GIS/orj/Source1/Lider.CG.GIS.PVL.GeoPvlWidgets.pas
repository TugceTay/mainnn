...
{$IFDEF DCC}
  unit Lider.CG.GIS.PVL.GeoPvlWidgets;
...
begin
  oPlatform := _context.Platform.CreateObject(self, 'Panel') ;
  container := oPlatform as IGIS_PvlPanel ;

  oLabel := TGIS_PvlLabel.Create( _context ) ;
  oEdit  := TGIS_PvlEdit.Create( _context ) ;

  container.AddComponent( oLabel ) ;
  container.AddComponent( oEdit ) ;
end;
...
begin
  ...
  oLabel.Place( Width - 2* Context.HMargin, 0, nil, Context.HMargin, nil, Context.VMargin ) ;
  oEdit.Place( Width - 2* Context.HMargin, 0, nil, Context.HMargin, oLabel, 0 ) ;

  if _height = 0 then
    Height := Context.HMargin * 2 + oEdit.Height + Context.HSpace + oLabel.Height ;
end;
...
begin
  ...
  oLabel := TGIS_PvlLabel.Create( Context ) ;
  oCombo  := TGIS_PvlComboBox.Create( Context ) ;

  container.AddComponent( oLabel ) ;
  container.AddComponent( oCombo ) ;
end;
...
begin
  ...
  oLabel.Place( Width - 2* Context.HMargin, 0, nil, Context.HMargin, nil, Context.VMargin ) ;
  oCombo.Place( Width - 2* Context.HMargin, 0, nil, Context.HMargin, oLabel, 0 ) ;

  if _height = 0 then
    Height := Context.HMargin * 2 + oCombo.Height + Context.HSpace + oLabel.Height ;
end;
...
begin
  ...
  oLabel := TGIS_PvlLabel.Create( Context ) ;
  oCombo := TGIS_PvlComboEdit.Create( Context ) ;
...
begin
  ...
  oLabel := TGIS_PvlLabel.Create( Context ) ;
  oCombo := TGIS_PvlColorComboBox.Create( Context ) ;
...
begin
  ...
  cmbRamp := TGIS_PvlColorRampComboBox.Create( Context ) ;

  chkDiscrete := TGIS_PvlCheckBox.Create( Context ) ;
  ...
  chkShowNames := TGIS_PvlCheckBox.Create( Context ) ;
  ...
  container.AddComponent( cmbRamp ) ;
  container.AddComponent( chkDiscrete ) ;
  container.AddComponent( chkReverse ) ;
  container.AddComponent( chkShowAll ) ;
  container.AddComponent( chkShowNames ) ;
end;
