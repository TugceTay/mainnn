...
{$IFDEF DCC}
  unit Lider.CG.GIS.PVL.GeoControlColor ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.PVL.GeoControlColor"'}
{$ENDIF}
{$IFDEF CLR}
    ...
    try
      ...
      for i := 0 to _columns - 1 do begin
        for j := 0 to _rows - 1 do begin
          arrPalette[i][j] := TGIS_PvlColorPreview.Create( Context )  ;
          ...
          if ( i = 0 ) and ( j = 0 ) then
            ...
            arrPalette[i][j].Place( PREVIEW_SIZE, PREVIEW_SIZE, arrPalette[i-1, j], RoundS( Context.VMargin / 2 / Context.PPIFix ), nil, 0 )
          else
            arrPalette[i][j].Place( PREVIEW_SIZE, PREVIEW_SIZE, arrPalette[i-1, j], RoundS( Context.VMargin / 2 / Context.PPIFix ), arrPalette[i, j-1], 0 ) ;

          pPalette.AddComponent( arrPalette[i][j] ) ;
        end;
      end;
    finally
      UnlockWindow ;
    ...
    begin
      case cmbType.ItemIndex of
        ...
        1 : begin
          ...
          pHsl.AddComponent( lblHue ) ;
          pHsl.AddComponent( edtHue ) ;
          pHsl.AddComponent( lblSat ) ;
          pHsl.AddComponent( edtSat ) ;

          if not ( ( prevType = _rsrcna( GIS_RS_COLOR_CONTROL_HSV ) ) or ( prevType = _rsrcna( GIS_RS_COLOR_CONTROL_CMYK ) ) ) then begin
            lblAlpha.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_ALPHA ) + ' (%)' ;
        ...
        2 : begin
          ...
          // Adding components here to reuse it as both HSL and HSV uses Hue and Saturation
          pHsv.AddComponent( lblHue ) ;
          pHsv.AddComponent( edtHue ) ;
          pHsv.AddComponent( lblSat ) ;
          pHsv.AddComponent( edtSat ) ;

          if not ( ( prevType = _rsrcna( GIS_RS_COLOR_CONTROL_HSL ) ) or ( prevType = _rsrcna( GIS_RS_COLOR_CONTROL_CMYK ) ) ) then begin
            lblAlpha.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_ALPHA ) + ' (%)' ;
        ...
        3 : begin
          ...
          if not ( ( prevType = _rsrcna( GIS_RS_COLOR_CONTROL_HSL ) ) or ( prevType = _rsrcna( GIS_RS_COLOR_CONTROL_HSV ) ) ) then begin
            lblAlpha.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_ALPHA ) + ' (%)' ;
...
//==============================================================================
  ...
  begin
    ...
    {$REGION 'RGB panel'}
      lblRed := TGIS_PvlLabel.Create( Context ) ;
      lblRed.Place( lblAlpha.Width, 0, nil, 0, nil, 0 ) ;
      lblRed.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_RED ) ;

      edtRed := TGIS_PvlEdit.Create( Context ) ;
      edtRed.Place( cmbType.Width - lblRed.Width - Context.VSpace, 0, lblRed, Context.VSpace, nil, 0 ) ;
      ...
      lblGreen := TGIS_PvlLabel.Create( Context ) ;
      lblGreen.Place( lblAlpha.Width, 0, nil, 0, edtRed, 0 ) ;
      lblGreen.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_GREEN ) ;

      edtGreen := TGIS_PvlEdit.Create( Context ) ;
      edtGreen.Place( cmbType.Width - lblGreen.Width - Context.VSpace, 0, lblGreen, Context.VSpace, edtRed, 0 ) ;
      ...
      lblBlue := TGIS_PvlLabel.Create( Context ) ;
      lblBlue.Place( lblAlpha.Width, 0, nil, 0, edtGreen, 0 ) ;
      lblBlue.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_BLUE ) ;

      edtBlue := TGIS_PvlEdit.Create( Context ) ;
      edtBlue.Place( cmbType.Width - lblBlue.Width - Context.VSpace, 0, lblBlue, Context.VSpace, edtGreen, 0 ) ;
      ...
      pRgb.AddComponent( lblRed ) ;
      pRgb.AddComponent( edtRed ) ;
      pRgb.AddComponent( lblGreen ) ;
      pRgb.AddComponent( edtGreen ) ;
      pRgb.AddComponent( lblBlue ) ;
      pRgb.AddComponent( edtBlue ) ;
    {$ENDREGION 'RGB panel'}

    {$REGION 'HSV and HSL panel'}
      lblHue := TGIS_PvlLabel.Create( Context ) ;
      lblHue.Place( lblAlpha.Width, 0, nil, 0, nil, 0 ) ;
      lblHue.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_HUE ) + ' (°)' ;

      edtHue := TGIS_PvlEdit.Create( Context ) ;
      edtHue.Place( cmbType.Width - lblHue.Width - Context.VSpace, 0, lblHue, Context.VSpace, nil, 0 ) ;
      ...
      lblSat := TGIS_PvlLabel.Create( Context ) ;
      lblSat.Place( lblAlpha.Width, 0, nil, 0, edtHue, 0 ) ;
      lblSat.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_SAT ) + ' (%)' ;

      edtSat := TGIS_PvlEdit.Create( Context ) ;
      edtSat.Place( cmbType.Width - lblHue.Width - Context.VSpace, 0, lblSat, Context.VSpace, edtHue, 0 ) ;
    ...
    {$REGION 'HSL panel'}
      lblLig := TGIS_PvlLabel.Create( Context ) ;
      lblLig.Place( lblAlpha.Width, 0, nil, 0, edtSat, 0 ) ;
      lblLig.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_LIGHTNESS ) + ' (%)' ;

      edtLig := TGIS_PvlEdit.Create( Context ) ;
      edtLig.Place( cmbType.Width - lblHue.Width - Context.VSpace, 0, lblLig, Context.VSpace, edtSat, 0 ) ;
      ...
      pHsl.Height := edtLig.Top + edtLig.Height + Context.VMargin ;

      pHsl.AddComponent( lblHue ) ;
      pHsl.AddComponent( edtHue ) ;
      pHsl.AddComponent( lblSat ) ;
      pHsl.AddComponent( edtSat ) ;
      pHsl.AddComponent( lblLig ) ;
      pHsl.AddComponent( edtLig ) ;
    {$ENDREGION 'HSL panel'}

    {$REGION 'HSV panel'}
      lblVal := TGIS_PvlLabel.Create( Context ) ;
      lblVal.Place( lblAlpha.Width, 0, nil, 0, edtSat, 0 ) ;
      lblVal.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_VALUE ) + ' (%)' ;

      edtVal := TGIS_PvlEdit.Create( Context ) ;
      edtVal.Place( cmbType.Width - lblHue.Width - Context.VSpace, 0, lblLig, Context.VSpace, edtSat, 0 ) ;
      ...
      pHsv.Height := edtVal.Top + edtVal.Height + Context.VMargin ;

      pHsv.AddComponent( lblHue ) ;
      pHsv.AddComponent( edtHue ) ;
      pHsv.AddComponent( lblSat ) ;
      pHsv.AddComponent( edtSat ) ;
      pHsv.AddComponent( lblVal ) ;
      pHsv.AddComponent( edtVal ) ;
    {$ENDREGION 'HSV panel'}

    {$REGION 'CMYK panel'}

      lblCyan := TGIS_PvlLabel.Create( Context ) ;
      lblCyan.Place( lblAlpha.Width, 0, nil, 0, nil, 0 ) ;
      lblCyan.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_CYAN ) + ' (%)' ;

      edtCyan := TGIS_PvlEdit.Create( Context ) ;
      edtCyan.Place( cmbType.Width - lblCyan.Width - Context.VSpace, 0, lblCyan, Context.VSpace, nil, 0 ) ;
      ...
      lblMagenta := TGIS_PvlLabel.Create( Context ) ;
      lblMagenta.Place( lblAlpha.Width, 0, nil, 0, edtCyan, 0 ) ;
      lblMagenta.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_MAGENTA ) + ' (%)' ;

      edtMagenta := TGIS_PvlEdit.Create( Context ) ;
      edtMagenta.Place( cmbType.Width - lblCyan.Width - Context.VSpace, 0, lblMagenta, Context.VSpace, edtCyan, 0 ) ;
      ...
      lblYellow := TGIS_PvlLabel.Create( Context ) ;
      lblYellow.Place( lblAlpha.Width, 0, nil, 0, edtMagenta, 0 ) ;
      lblYellow.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_YELLOW ) + ' (%)' ;

      edtYellow := TGIS_PvlEdit.Create( Context ) ;
      edtYellow.Place( cmbType.Width - lblCyan.Width - Context.VSpace, 0, lblYellow, Context.VSpace, edtMagenta, 0 ) ;
      ...
      lblKey := TGIS_PvlLabel.Create( Context ) ;
      lblKey.Place( lblAlpha.Width, 0, nil, 0, edtYellow, 0 ) ;
      lblKey.Caption := _rsrcna( GIS_RS_COLOR_CONTROL_KEY ) + ' (%)' ;

      edtKey := TGIS_PvlEdit.Create( Context ) ;
      edtKey.Place( cmbType.Width - lblCyan.Width - Context.VSpace, 0, lblKey, Context.VSpace, edtYellow, 0 ) ;
      ...
      pCmyk.Height := edtKey.Top + edtKey.Height + Context.VMargin ;

      pCmyk.AddComponent( lblCyan ) ;
      pCmyk.AddComponent( edtCyan ) ;
      pCmyk.AddComponent( lblMagenta ) ;
      pCmyk.AddComponent( edtMagenta ) ;
      pCmyk.AddComponent( lblYellow ) ;
      pCmyk.AddComponent( edtYellow ) ;
      pCmyk.AddComponent( lblKey ) ;
      pCmyk.AddComponent( edtKey ) ;
    ...
    pPalette.Place( cmbPalette.Width + RoundS( Context.HMargin / 2 ) {$IFDEF GIS_MOBILE_DIALOGS} + RoundS( Context.HMargin ) {$ENDIF}, RoundS( 10 * ( PREVIEW_SIZE + Context.VMargin / 2 ) ), prevNewColor, 5 * Context.VSpace, cmbPalette, 0 ) ;
