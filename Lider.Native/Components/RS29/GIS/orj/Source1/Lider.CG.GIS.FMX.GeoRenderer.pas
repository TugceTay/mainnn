...
unit Lider.CG.GIS.FMX.GeoRenderer;
{$HPPEMIT '#pragma link "Lider.CG.GIS.FMX.GeoRenderer"'}
          ...
          if not _selectionOnly then begin
            ...
            then begin
              if assigned( params_line.Symbol ) then begin
                ...
                for i:=0 to mypath.Count -1 do begin
                  pt := mypath.Points[i].Point ;
                  ar[i] := Point( RoundS( pt.X ), RoundS( pt.Y ) ) ;
                end ;
        ...
        if not _selectionOnly then begin
          ...
          begin
            ...
            for i := 0 to length(part_buf) - 1 do begin
              ...
              for j := 0 to part_buf[i] -1 do begin
                pt := mypath.Points[k+j].Point ;
                ar[j] := Point( RoundS( pt.X ), RoundS( pt.Y ) ) ;
              end ;
              TGIS_SymbolLineHelper.DrawLine( Viewer, ar,
    ...
    try
      ...
      case TGIS_Shape( _shp ).ShapeType of
        ...
        TGIS_ShapeType.MultiPatch  :
            doShapePolygon   ( TGIS_ShapeMultiPatch( _shp ),
