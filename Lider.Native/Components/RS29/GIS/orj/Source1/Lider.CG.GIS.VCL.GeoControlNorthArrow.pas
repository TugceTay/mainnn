...
unit Lider.CG.GIS.VCL.GeoControlNorthArrow ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.VCL.GeoControlNorthArrow"'}
  ...
  begin
    ...
    if Result and
       ( ( ( oldRenderer = 0 ) and not ( GIS_Viewer.Renderer is TGIS_RendererVclDirect2D ) ) or
         ( ( oldRenderer = 1 ) and not ( GIS_Viewer.Renderer is TGIS_RendererVclGdiPlus ) ) ) then
      Result := False ;
    if Result and
       ( oldColor <> Color ) then
      Result := False ;
...
function TGIS_ControlNorthArrow.CreateCopy
  : IGIS_PrintableControl ;
begin
  Result := TGIS_ControlNorthArrow.Create( Parent ) ;
  TGIS_ControlNorthArrow(Result).Parent := Parent ;
  TGIS_ControlNorthArrow(Result).InternalName := Name ;
  TGIS_ControlNorthArrow(Result).Visible := False ;
  TGIS_ControlNorthArrow(Result).GIS_Viewer := GIS_Viewer ;
  TGIS_ControlNorthArrow(Result).Style := Style ;
  TGIS_ControlNorthArrow(Result).Color1 := Color1 ;
  TGIS_ControlNorthArrow(Result).Color2 := Color2 ;
  TGIS_ControlNorthArrow(Result).Path := Path ;
end ;
...
begin
  ...
  btrans := Transparent ;
  try
    DrawBmp( VCL.Graphics.TBitmap(_bitmap.NativeBitmap), 0 ) ;
  finally
    Transparent := btrans ;
  end;
end ;
...
begin
  ...
  try
    ...
    if vwr.Renderer is TGIS_RendererVclDirect2D then
      oldRenderer := 0
    else
    if vwr.Renderer is TGIS_RendererVclGdiPlus then
      oldRenderer := 1
    else
      oldRenderer := 2 ;
    oldColor := Color ;
