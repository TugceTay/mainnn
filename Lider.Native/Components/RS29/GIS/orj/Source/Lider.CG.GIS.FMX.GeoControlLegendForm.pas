...
unit FMX.GisControlLegendForm ;
{$HPPEMIT '#pragma link "FMX.GisControlLegendForm"'}
...
{$R FMX.GisControlLegendForm_16x16.RES}
...
begin
  ...
  try
    ...
    if Assigned( shp.Params.Area.Symbol ) then begin
      ...
      else if shp.Params.Area.Symbol is TGIS_SymbolHatch then begin
...
procedure T_panelMarker.doCallback(
  ...
  begin
    ...
    bsymlin := Assigned( sym ) and ( ( sym is TGIS_SymbolLine   ) or
                                     ( sym is TGIS_SymbolLineEx ) or
                                     ( sym is TGIS_SymbolHatch  )
                                    ) ;
