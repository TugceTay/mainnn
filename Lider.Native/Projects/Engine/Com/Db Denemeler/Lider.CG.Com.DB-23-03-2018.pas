...
type
  ...
  protected {My simplified methods to override}
    ...
    function Navigate(Buffer: TRecordBuffer; GetMode: TGetMode; doCheck: Boolean): TGetResult; virtual; abstract;
  ...
  public
    ...
    //function GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean; overload;
  ...
  protected
    ...
    function Navigate(Buffer: TRecordBuffer; GetMode: TGetMode; doCheck: Boolean): TGetResult; override;
  ...
  public
    ...
    procedure RectangleFilter(const AxMin, AyMin, AxMax, AyMax: Double; operator:
      TlicgGraphicOperator; const QueryExpression: string;
      ClearBefore: Boolean);
    ...
    procedure PolylineIntersects(Polyline: IlicgEntity; const QueryExpression:
      string; ClearBefore: Boolean);
    procedure FilterFromLayer(SourceLayer: TlicgBaseLayer; const QueryExpression:
      string; operator: TlicgGraphicOperator; ClearBefore: Boolean);
...
begin
  ...
  FillChar(Result[0], FRecordSize, 0); // orj   FillChar(Result^, FRecordSize, 0);
...
function TlicgBaseDataset.GetRecord(Buffer: TRecordBuffer; GetMode: TGetMode; DoCheck: Boolean): TGetResult;
begin
...
begin
  FreeRecordPointers(Buffer);
  // eski FillChar(Buffer^, FRecordSize, 0);
    ...
    case Fields[Index].DataType of
      ftString:
        begin
          ...
          //StrLCopyW(PWChar(Buffer + Offset), PWChar(TempStr), length(TempStr));
        end;
      ftInteger, ftSmallInt, ftDate, ftTime:
        begin
          if VarIsNull(Value) then
...
//function TlicgBaseDataset.GetFieldData(Field: TField; var Buffer: TValueBuffer): Boolean;
...
begin
  ...
  if not FisOpen then
    exit;
  RecBuffer := TRecordBuffer(GetActiveRecordBuffer);
  ...
  begin
    ...
    case Field.DataType of
      ...
      ftCurrency, ftFloat:
        Move((RecBuffer + Offset)^, Buffer^, sizeof(Double));
      ftDateTime:
        begin
...
begin
  ...
  for I := 0 to FMapFields.Count - 1 do
    ...
    begin
      ...
      begin
	    FreeAndNil(FMapFields[I].FResolver);
...
function TlicgTable.Navigate(Buffer: TRecordBuffer; GetMode: TGetMode; doCheck: Boolean): TGetResult;
var
  Acceptable: Boolean;
begin
...
begin
  ...
  if (FLayer = nil) or (FGIS = nil) then
    Exit;
  FGIS.QuerySelectTool.QueryPolygon(Polyline, FLayer.Name, QueryExpression, goIntersects,
    FGraphicFilterList, ClearBefore);
  FGraphicFiltered := True;
end;
procedure TlicgTable.FilterFromLayer(SourceLayer: TlicgBaseLayer; const
  QueryExpression: string; operator: TlicgGraphicOperator; ClearBefore: Boolean);
var
  E: IlicgEntity;
  WasInit: Boolean;
...
begin
  ...
  begin
    ...
    try
      try
        if not (E.EntityID in [idLine, idPolyline, idPolygon, idPolyArc, idPolyEllipse,
          idArc, idEllipse, idSpline, idTriangle, idPafta]) then
        begin
          Sourcelayer.Next;
          Continue;
        end;
...
function TlicgDesignTable.Navigate(Buffer: TRecordBuffer; GetMode: TGetMode;
