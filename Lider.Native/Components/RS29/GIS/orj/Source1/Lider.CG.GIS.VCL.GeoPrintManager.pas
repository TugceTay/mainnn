...
unit Lider.CG.GIS.VCL.GeoPrintManager ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.VCL.GeoPrintManager"'}
...
//==============================================================================
  ...
  var
    ...
    begin
      if assigned( oPrinter.Graphics ) then begin
        ...
        oPrinter.Canvas.Draw( _rect.Left, _rect.Top, cache, 255 ) ;
    end ;
    ...
    begin
      ...
      ctx := TGIS_RendererContext.Create ;
      try
        if Assigned( oViewerRenderer ) then begin
          if oViewerRenderer is TGIS_RendererVclDirect2D then
            rnd := TGIS_RendererVclGdiPlus.Create
          else
            rnd := TGIS_RendererAbstract( oViewerRenderer ).CreateInstance ;
        end else begin
        ...
        cache := VCL.Graphics.TBitmap(TGIS_RendererVclAbstract(rnd).createTransparentBitmap(w,h)) ;
  ...
  begin
    ...
    try
      ...
      oRenderer := TGIS_RendererAbstract( oViewerRenderer ).CreateInstance ;
      // printing
  ...
  var
    ...
    var
      ...
      transparent : Boolean ;
      b           : TGIS_GdipBitmap ;
      ctx_labels  : VCL.Graphics.TBitmap ;
      ctx_labels_topmost : VCL.Graphics.TBitmap ;
      old_OverlappedExtentMargin : Integer ;
    ...
    begin
      ...
      try
        try
          ...
          if Assigned( oViewerRenderer ) then
            oRenderer := TGIS_RendererAbstract( oViewerRenderer ).CreateInstance
          else if not assigned( oViewer ) then
            // template designer
            oRenderer := TGIS_RendererVclDirect2D.Create
          else
            oRenderer := TGIS_RendererVclGdi32.Create ;

          tcache := TGIS_Bitmap.Create( w, h ) ;
          cache  := VCL.Graphics.TBitmap(tcache.NativeBitmap) ;
          if not transparent then begin
            cache.Canvas.Lock ;
            try
              cache.Canvas.Brush.Color := VCLColor( _viewer.Color ) ;
              cache.Canvas.FillRect( Rect( 0, 0, w, h ) ) ;
            finally
              cache.Canvas.Unlock ;
            end ;
          end;

          ctx.AssignBaseMap( cache, False ) ;
          ...
          try
            ...
            if Assigned( ctx.Charts ) then
              if ( oPrinter is TGIS_PrinterPreview ) and transparent then begin
                TGIS_RendererVclAbstract( oRenderer ).stretchBitmapFast(
                  ctx.Charts, ctx.BaseMap, Rect( 0, 0, w, h )
                ) ;
              end else begin
                TGIS_RendererVclAbstract(oRenderer).blendBitmaps(
                  ctx.Charts,
            ...
            if Assigned( ctx.Labels ) then begin
              if not metPaintLabelsOnTop then begin
                if ( oPrinter is TGIS_PrinterPreview ) and transparent then begin
                  TGIS_RendererVclAbstract( oRenderer ).stretchBitmapFast(
                    ctx.Labels, ctx.BaseMap, Rect( 0, 0, w, h )
                  ) ;
                end else begin
                  TGIS_RendererVclAbstract( oRenderer ).blendBitmaps(
                    ctx.Labels,
                ...
                ctx_labels := VCL.Graphics.TBitmap(
                  TGIS_RendererVclAbstract(oRenderer).createTransparentBitmap( w, h )
                ) ;
                ctx_labels.Assign( VCL.Graphics.TBitmap( ctx.Labels ) ) ;
              end ;
            end ;
            ...
            if _viewer.IsTopmost then begin
              ...
              if Assigned( ctx.Labels ) then begin
                if not metPaintLabelsOnTop then begin
                  ...
                  ctx_labels_topmost.Assign( VCL.Graphics.TBitmap( ctx.Labels ) ) ;
                end ;
              end;
            ...
            if assigned( ctx_labels ) then begin
              ...
              if ( oPrinter is TGIS_PrinterPreview ) and transparent then begin
                TGIS_RendererVclAbstract( oRenderer ).stretchBitmapFast(
                  ctx_labels, ctx.BaseMap, Rect( 0, 0, w, h )
                ) ;
              end else begin
                TGIS_RendererVclAbstract( oRenderer ).blendBitmaps(
                  ctx_labels,
            ...
            if assigned( ctx_labels_topmost ) then begin
              ...
              if ( oPrinter is TGIS_PrinterPreview ) and transparent then begin
                TGIS_RendererVclAbstract( oRenderer ).stretchBitmapFast(
                  ctx_labels_topmost, ctx.BaseMap, Rect( 0, 0, w, h )
                ) ;
              end else begin
                TGIS_RendererVclAbstract( oRenderer ).blendBitmaps(
                  ctx_labels_topmost,
          ...
          if oPrinter is TGIS_PrinterPreview then begin
            if not transparent then
              fix_alpha( cache ) ;
            TGIS_RendererVclAbstract( oRenderer ).stretchBitmapFast(
              cache, TGIS_PrinterPreview( oPrinter ).Bitmap, _rect
            ) ;
          end else begin
            if transparent then begin
              if assigned( oPrinter.Graphics ) then begin
                ...
                oPrinter.Canvas.Draw( _rect.Left, _rect.Top,
                                      VCL.Graphics.TBitmap(cache),
                                      255 ) ;
            end else begin
              cache.PixelFormat := pf24bit ;
              VCLCanvasDrawBitmapEx( oPrinter.Canvas, _rect, cache );
            end ;
          end ;
          FreeObject( oRenderer ) ;
        end ;
