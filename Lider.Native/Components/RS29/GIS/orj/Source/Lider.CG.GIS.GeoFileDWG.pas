...
{$IFDEF DCC}
  unit GisFileDWG ;
  {$HPPEMIT '#pragma link "GisFileDWG"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  /// </summary>
  {#gendoc:hide}
  T_FileDwg_Color = {$IFDEF OXYGENE} assembly {$ENDIF} {$IFDEF JAVA} class {$ELSE} record {$ENDIF}
    public
      /// <summary>
      ///   Color definition.
      /// </summary>
  ...
  T_FileDwg_Block = {$IFDEF OXYGENE} assembly {$ENDIF} class( T_FileDwg_Entity )
    ...
    public
      ...
      /// <summary>
      ///   Find entity in block
      /// </summary>
      /// <param name="_handle">
      ///   entity handle
      /// </param>
      function FindEntity( const _handle : UInt64 ) : T_FileDwg_Entity ;
  end ;
...
type
  ...
  DwgObjHandle = {$IFDEF JAVA} class {$ELSE} record {$ENDIF}
    public
      typ    : duint32;
      handle : duint64;
      loc    : duint32;
  ...
  begin
    ...
    try
      ...
      if dwgVersionHigher( _version, DWG_Version.AC1014 ) then begin
        ...
        prevData := _buf.getBitLong shl 3 ;
        if prevData > 0 then
          for j := 0 to prevData - 1 do begin
            _buf.getRawChar8;
            if not _buf.isGood then break ;
          end;
      end ;
      if dwgVersionHigher( _version, DWG_Version.AC1018 ) then begin
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if ( hasAttrib = 1 ) then begin
      attribs := TList<DwgHandle>.Create ;
      if dwgVersionLower( _version, DWG_Version.AC1018 ) then begin
        attH := _buf.getHandle;
        attribs.Add( attH ) ;
        attH := _buf.getHandle;
...
// ----------------------------------------------------------------------------
  ...
  begin
    ...
    if assigned( _data.attribs ) then begin
      ins.Attributes := TList<UInt64>.Create ;
      for i := 0 to _data.attribs.Count-1 do
        ins.Attributes.Add( _data.attribs[i].ref ) ;
    end;
...
// ----------------------------------------------------------------------------
  ...
  function T_FileDwg_Block.FindEntity(
    const _handle : UInt64
  ) : T_FileDwg_Entity ;
  var
    e : T_FileDwg_Entity ;
  begin
    Result := nil ;
    for e in Entities do begin
      if e.Handle = _handle then begin
        Result := e ;
        break ;
      end ;
    end ;
...
// ----------------------------------------------------------------------------
 ...
 {$IFNDEF OXYGENE}
 destructor T_FileDwg_Insert.Destroy;
 begin
   FreeObject( Attributes ) ;
