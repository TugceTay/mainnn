...
unit FMX.GisViewer3D;
{$HPPEMIT '#pragma link "FMX.GisViewer3D"'}
...
type
  ...
  public
    ...
    /// <inheritdoc from="IGIS_Viewer3D" />
    property KeepSunCamera   : Boolean        read  fget_KeepSunCamera
...
//------------------------------------------------------------------------------
  ...
  begin
    ...
    sunCameraDelta := SunPosition.Y - CameraPosition.Y ;
    if not KeepSunCamera then
      sceneRepaint ;
  end ;

  function TGIS_Viewer3D.fget_KeepSunCamera
    : Boolean ;
  begin
    Result := oRenderer.GetKeepSunCamera;
  end ;

  procedure TGIS_Viewer3D.fset_KeepSunCamera(
    const _value : Boolean
  ) ;
  begin
    if not bCanRender then
