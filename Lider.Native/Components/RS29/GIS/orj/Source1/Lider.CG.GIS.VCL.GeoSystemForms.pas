...
unit Lider.CG.GIS.VCL.GeoSystemForms ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.VCL.GeoSystemForms"'}
...
function TGIS_SelectFolderDialog.Execute
  : TGIS_PvlModalResult ;
begin
  if SelectDirectory( '', '', sDir, [], TWinControl( oParent ) ) then
    Result := TGIS_PvlModalResult.OK
  else
    Result := TGIS_PvlModalResult.Cancel ;
end;
...
begin
  ...
  oForm := Lider.CG.GIS.VCL.GeoLineSymbolEditor.TGIS_LineSymbolEditor.Create( TWinControl( oParent ) );
end;

function TGIS_LineSymbolEditor.fget_Symbol
  : String;
begin
  Result := Lider.CG.GIS.VCL.GeoLineSymbolEditor.TGIS_LineSymbolEditor( oForm ).Symbol;
end;
...
begin
  Lider.CG.GIS.VCL.GeoLineSymbolEditor.TGIS_LineSymbolEditor( oForm ).Symbol := _symbol;
end;
...
var
  res : Integer;
begin
  res := Lider.CG.GIS.VCL.GeoLineSymbolEditor.TGIS_LineSymbolEditor( oForm ).Execute( _path, _onhelp );
  if res = mrOk then
    result := TGIS_PvlModalResult.OK
...
begin
  ...
  oForm := Lider.CG.GIS.VCL.GeoControlCsSystem.TGIS_ControlCSSystem.Create( TWinControl( oParent ) );
end;

function TGIS_ControlCSSystem.fget_CS
  : TGIS_CSCoordinateSystem;
begin
  Result := Lider.CG.GIS.VCL.GeoControlCsSystem.TGIS_ControlCSSystem( oForm ).CS;
end;
...
var
  res : Integer;
begin
  res := Lider.CG.GIS.VCL.GeoControlCsSystem.TGIS_ControlCSSystem( oForm ).Execute( _cs, _onhelp );
  if res = mrOk then
    result := TGIS_PvlModalResult.OK
  else if res = mrCancel then
    result := TGIS_PvlModalResult.Cancel
...
begin
  ...
  oForm := Lider.CG.GIS.GeoControlPrintPreviewSimple.TGIS_ControlPrintPreviewSimple.Create( TWinControl( oParent ) );
end;
