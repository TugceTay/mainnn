...
{$IFDEF DCC}
  unit GisLayerPNG ;
  {$HPPEMIT '#pragma link "GisLayerPNG"'}
{$ENDIF}
{$IFDEF CLR}
...
//==============================================================================
    ...
    try
      if assigned( Viewer ) then
        png := TGIS_Bitmap.Create(
                 TGIS_RendererAbstract( Viewer.Ref.ViewerParent.ControlRenderer).BitmapFactory
               )
      else
        png := TGIS_Bitmap.Create( BitmapFactory ) ;
      try
  ...
  class procedure Unit_GisLayerPNG.SelfRegisterLayer() ;
  begin
    {$IFNDEF GIS_NOPNG}
...
{$IFNDEF OXYGENE}
  initialization
    Unit_GisLayerPNG.SelfRegisterLayer() ;
{$ENDIF}
