...
unit Lider.CG.GIS.VCL.GeoViewerBmp ;
{$HPPEMIT '#pragma link "VCL.Lider.CG.GIS.GeoViewerBmp"'}
...
type
  ...
  TGIS_ViewerBmp = class( TComponent, IGIS_Viewer, IGIS_ViewerParent,
    ...
    private // ordinal functions
      ...
      procedure createForcedBitmap  ( const _bitmap     : TBitmap
                                    ) ;
    ...
    private // properties access functions of IGIS_ViewerBmp
      function  fget_Bitmap             : {$IFNDEF GIS_XDK}
                                            TObject ;
                                          {$ELSE}
                                            TBitmap ;
                                          {$ENDIF}
    ...
    public // properties of IGIS_ViewerBmp
      ...
      /// <inheritdoc from="IGIS_ViewerBmp"/>
      property Bitmap : {$IFNDEF GIS_XDK}
                          TObject
                        {$ELSE}
                          TBitmap
                        {$ENDIF}
...
//==============================================================================
  ...
  function TGIS_ViewerBmp.ControlSystemPPI
    : Integer;
  begin
    Result := oBitmap.Canvas.Font.PixelsPerInch ;
  end ;
...
//==============================================================================
  ...
  begin
    ...
    try
      if oVwr.Color.ARGB = TGIS_Color.None.ARGB then begin
        for i := 0 to oBitmap.Height-1 do begin
          prgbr := PRGBQuad(oBitmap.ScanLine[i]);
          for j := 0 to oBitmap.Width-1 do begin
            with prgbr^ do begin
              rgbReserved := 0 ;
              rgbRed      := 0 ;
              rgbGreen    := 0 ;
        ...
        try
          ...
          if Assigned( ctx.Labels ) then begin
            ...
            if not metPaintLabelsOnTop then
              ...
              ctxLabels := VCL.Graphics.TBitmap(
                TGIS_RendererVclAbstract(oRenderer).createTransparentBitmap(
                                                      oBitmap.Width,
                                                      oBitmap.Height )
              ) ;
              ctxLabels.Assign( VCL.Graphics.TBitmap( ctx.Labels ) ) ;
            end ;
          end ;
          oBitmap.Canvas.UnLock ;
  ...
  begin
    ...
    {$ELSE}
      ...
      {$ELSE}
        ...
        if not TGIS_RendererVclDirect2D( oRenderer ).Supported then begin
          FreeObject( oRenderer ) ;
          oRenderer := TGIS_RendererVclGdiPlus.Create ;
        end ;
      {$ENDIF}
  ...
  begin
    oBitmap := _bitmap ;
    oBitmap.AlphaFormat := afDefined ;
    oBitmap.PixelFormat := pf32bit ;

    Assert( oBitmap.PixelFormat = pf32bit ) ;
