...
{$IFDEF DCC}
  unit Lider.CG.GIS.GeoProjectQgis ;
  {$HPPEMIT '#pragma link "Lider.CG.GIS.GeoProjectQgis"'}
{$ENDIF}
{$IFDEF CLR}
                ...
                if layer.Raster.Shader.ColorRampType = 'INTERPOLATED' then begin
                  ...
                  for j := 0 to layer.Raster.Shader.ColorRamps.Count-2 do begin
                    ...
                    prms.Pixel.AltitudeMapZones.Add(
                      Format( '%s,%s,%s,%s',
                              [ DotFloatToStr( cr1.Value ),
                                DotFloatToStr( cr2.Value ),
                                ConstructParamColor( cr1.Color ),
                                cr1.&Label
                              ]
                            )
                    ) ;
                    if j = layer.Raster.Shader.ColorRamps.Count-2 then
                      prms.Pixel.AltitudeMapZones.Add(
                        Format( '%s,%s,%s,%s',
                                [ DotFloatToStr( cr2.Value ),
                                  DotFloatToStr( layer.Raster.ClassMax ),
                                  ConstructParamColor( cr2.Color ),
                                  cr2.&Label
                                ]
                              )
                    ) ;
