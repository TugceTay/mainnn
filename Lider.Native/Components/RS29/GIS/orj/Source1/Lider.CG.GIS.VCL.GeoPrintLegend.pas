...
unit Lider.CG.GIS.VCL.GeoPrintLegend ;
{$HPPEMIT '#pragma link "Lider.CG.GIS.VCL.GeoPrintLegend"'}
...
uses
  ...
  Lider.CG.GIS.VCL.GeoLegendUtilsFactory,
...
type
  ...
  TGIS_PrintLegend = class
    ...
    private
      ...
      procedure fset_Mode         ( const _mode    : TGIS_ControlLegendMode
                                  ) ;
      function  fget_ReverseOrder : Boolean ;
      procedure fset_ReverseOrder ( const _reverse : Boolean
                                  ) ;
      function  fget_RTL          : Boolean ;
      procedure fset_RTL          ( const _rtl     : Boolean
                                  ) ;
      function  fget_Font         : TFont ;
      procedure fset_Font         ( const _val : TFont
                                  ) ;
      function  fget_CompactView  : Boolean ;
      procedure fset_CompactView  ( const _val : Boolean
                                  ) ;
      function  fget_DrawIconStyle: TGIS_LegendIconStyle ;
      procedure fset_DrawIconStyle( const _value : TGIS_LegendIconStyle
                                  ) ;
    private
      function  decode_mode       ( const _mode : TGIS_ControlLegendMode
...
//==============================================================================
  ...
  function TGIS_PrintLegend.decode_mode(
    const _mode : TGIS_ControlLegendMode
  ) : TGIS_ControlLegendMode ;
  begin
    if _mode = TGIS_ControlLegendMode.Groups then
      result := TGIS_ControlLegendMode.Groups
    else
      result := TGIS_ControlLegendMode.Layers ;
  end ;
  ...
  begin
    if not assigned( oLegend ) then
      FReverseOrder := _reverse ;
  end ;

  function TGIS_PrintLegend.fget_RTL : Boolean ;
  begin
    if Assigned( oLegend ) then
      Result := oLegend.BiDiMode <> TBiDiMode.bdLeftToRight
    else
      Result := FRTL ;
  end ;
  ...
  function TGIS_PrintLegend.fget_Font
    : TFont ;
  begin
    if Assigned( oLegend ) then
      Result := oLegend.Font
  ...
  begin
    ...
    if FCompactView <> _val then begin
      FreeObject( oBMPFactory ) ;
      oBMPFactory := TGIS_LegendIconFactory.Create( sizIco, _val, FDrawIconStyle ) ;
      FCompactView := _val ;
    end ;
  end ;
  ...
  var
    ...
    procedure draw_bitmap(
      const _l : Integer ; const _t : Integer ; const _b : TBitmap
    ) ;
    var
      gg : TGIS_Bitmap ;
      rr : TRect ;
    begin
      gg := TGIS_Bitmap.Create ;
      try
        gg.NativeBitmap := _b ;
        if RightToLeft then
          rr := Rect( oBitmap.Width - _l - gg.Width, _t,
                      oBitmap.Width - _l, _t + gg.Height )
        else
          rr := Rect( _l, _t, _l + gg.Width, _t + gg.Height ) ;
        oRenderer.CanvasDrawBitmap( gg, rr ) ;
      finally
        FreeObject( gg ) ;
      end ;
    end ;
    ...
    begin
      ...
      for i_zone := 1 to Abs( zones_count ) do begin
        ...
        begin
          draw_bitmap( _l, _t + 1, TBitmap( oBMPFactory.GetBitmap ) ) ;
      ...
      if _data.RowCount >= 0 then begin
        ...
        if src.Marker.ShowLegend then begin
          ...
          if _data.RenderMarker then
            ...
            fact.SetParams( src ) ;
            draw_bitmap( l0+l, t+1, TBitmap( fact.GetBitmap ) ) ;
        ...
        if src.Chart.ShowLegend and
          ...
          for ii := 1 to Length( _data.ChartMap ) do begin
            ...
            draw_bitmap( l0, t+1, TBitmap( fact.GetBitmap ) ) ;
  ...
  begin
    ...
    oRenderer.CanvasFont.Name := Font.Name ;
    oRenderer.CanvasFont.Size := Font.Size ;
    ...
    oRenderer.CanvasFont.Color := GISColor( Font.Color ) ;
  ...
  begin
    ...
    if oViewer is TGIS_ViewerWnd then begin
      if TGIS_ViewerWnd(oViewer).Renderer is TGIS_RendererVclDirect2D then
    ...
    try
      ...
      oRenderer.CanvasFont.Name := oBitmap.Canvas.Font.Name ;
