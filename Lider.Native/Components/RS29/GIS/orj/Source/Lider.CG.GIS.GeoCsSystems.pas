...
{$IFDEF DCC}
  unit GisCsSystems ;
  {$HPPEMIT '#pragma link "GisCsSystems"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_CSCoordinateSystem = {$IFDEF OXYGENE} public {$ENDIF}
    ...
    protected
      ...
      /// <summary>
      ///   Ignoring Z reprojection.
      /// </summary>
      FIgnoreVertCS : Boolean ;
    ...
    public // properties
        ...
        ///   Area code.
        /// </summary>
        property AreaEPSG : Integer read FAreaEPSG;

        /// <summary>
        ///   If true then VertCS will not be applied upon reprojection.
        /// </summary>
        property IgnoreVertCS     : Boolean
                                    read  FIgnoreVertCS
                                    write FIgnoreVertCS;
  end ;
...
const
  {#gendoc:hide}
  METADATA_CS_IGNORERVERTCS
    = 'TGIS_CSCoordinateSystem.IgnoreVertCS' ;
...
{$ENDIF}
  ...
  begin
    ...
    try
      lst.Text := FullWKT ;
      lst.SaveToFile( _path, TEncoding.ASCII ) ;
...
//------------------------------------------------------------------------------
  ...
  begin
    _IgnoreVertCS := GisMetadataAsBoolean( METADATA_CS_IGNORERVERTCS, _IgnoreVertCS );
...
//------------------------------------------------------------------------------
  ...
  var
    ...
    begin
      if IsStringEmpty( wkt ) then
        wkt := 'Custom_PCS' ;
      Result := Add( -1, wkt, geogcs, units, projection, _parameters, _reversed, _area ) ;
    end ;
