...
type
  ...
  TGIS_ParamsRender = {$IFDEF OXYGENE} public {$ENDIF} class ( TGIS_ParamsVector )
    ...
    public
      ...
      /// </summary>
      /// <remarks>
      ///   This property must follow the format:
      ///   'minimum:maximum:val1:val2:...valN', where:
      ///   <list type="bullet">
      ///     <item>
      ///       'minimum' and 'maximum' define Y-axis limits for 'Bar' charts.
      ///       For 'Pie' chart both values to 0.
      ///     </item>
      ///     <item>
      ///       'val1', 'val2',... , 'valN'
      ///       are the values displayed on the chart.
      ///       These can be numeric values, field names, or basic formulas.
      ///     </item>
      ///   </list>
      /// </remarks>
  ...
  TGIS_ParamsChart = {$IFDEF OXYGENE} public {$ENDIF} class ( TGIS_ParamsFeature )
    ...
    public
      ...
      /// <summary>
      ///   Size of the chart. Grouping as a text property all properties that
      ///   affect Size presentation like Size or SizeEx and represent sizes
      ///   in a human readable form like '10pt', '3in'.
      ///   <para>
      ///     Uses AsText parameter syntax.
      ...
      /// <summary>
      ///   Colors of the chart elements defined in RGBA format.
      /// </summary>
      /// <remarks>
      ///   This property must follow this format:
      ///   'R1:G1:B1:A1, R2:G2:B2:A2, ..., RN:GN:BN:AN',
      ///   where each RGBA color definition corresponds to the respective chart value.
      ///   R, G, B, and A are integer values (0–255).
      /// </remarks>
      ...
      /// </summary>
      /// <remarks>
      ///   See 'Values' property documentation.
      /// </remarks>
...
{$REGION 'Public functions for parameter conversion'}
   ...
   /// <summary>
   ///   Constructs AltitudeMapZone definition based on provided params.
   /// </summary>
   ...
   /// </param>
   /// <param name="_usePrecision">
   ///   indicates whether to use rounding to a specified precision; False by default
   /// </param>
   /// <param name="_precision">
   ///   number of decimal digits to round to
   /// </param>
   /// <returns>
   ///   Text representation of the AltitudeMapZone.
   /// </returns>
   /// <remarks>
   ...
   /// </remarks>
   function ConstructParamAltitudeMapZone( const _minValue     : Double ;
                                           const _maxValue     : Double ;
                                           const _color        : TGIS_Color ;
                                           const _legend       : String ;
                                           const _usePrecision : Boolean = False ;
                                           const _precision    : Integer = 0
                                         ) : String ;
...
begin
  ...
  colon_pos := Pos( ':', _text ) ;
  if colon_pos < StringFirst then begin
    _type := _text ;
    exit ;
  end ;

  _type  := UpperCase( Copy( _text, StringFirst, colon_pos-StringFirst  ) ) ;
  _value := Copy( _text, colon_pos+1, 4096  ) ;
end ;
...
begin
  if assigned( _symbol ) then begin
    if _symbol is TGIS_SymbolHatch then
      Result := GIS_PARAMTXT_TYPE_HATCH + ':' + _symbol.Name
    else
    Result := GIS_PARAMTXT_TYPE_SYMBOL + ':' + _symbol.Name
  end
...
begin
  ...
  else if _bsymbol and _stype( GIS_PARAMTXT_TYPE_HATCH ) then begin
    _symbol := ParamSymbol( _value, nil ) ;
    _bitmap := nil ;
  end
...
var
  ...
  var
    param_pos : Integer ;
  begin
    param_pos := Pos( UpperCase( GIS_PARAMTXT_ROTATION_DEG ), UpperCase( _val ) ) ;
    if param_pos > StringFirst then begin
      Result :=
        DegToRad(
          DotStrToFloat( Trim( Copy( _val, StringFirst, param_pos - StringFirst ) ) )
        ) ;
      exit ;
    end ;

    param_pos := Pos( UpperCase( GIS_PARAMTXT_ROTATION_RAD ), UpperCase( _val ) ) ;
    if param_pos > StringFirst then begin
      Result :=
        DotStrToFloat( Trim( Copy( _val, StringFirst, param_pos - StringFirst ) ) ) ;
      exit ;
...
function TGIS_ParamsAbstract.IsModified
  : Boolean ;
begin
  Result := FSerial <> FSerialSeed ;
end ;
...
begin
  ...
  cmap := GisColorRampList.ByName(TGIS_ColorRampNames.Set1).RealizeColorMap(
    TGIS_ColorMapMode.Continuous, 9
  );
  for i := 0 to 8 do
    ColorsInternal[i] := cmap[i].RGB ;
end ;
...
function TGIS_ParamsList.IsModified
  : Boolean ;
var
  i : Integer ;
begin
  ...
  for i := 0 to listObj.Count - 1 do begin
    Result := Result or TGIS_ParamsAbstract( listObj[i] ).IsModified ;
    if Result then
      break ;
  end ;
end;
...
function ConstructParamAltitudeMapZone(
...
const
  ZONE_FORMAT_START_END_COLOR        = '%s,%s,%s' ;
  ZONE_FORMAT_START_END_COLOR_LEGEND = '%s,%s,%s,%s' ;
var
  min_value_txt : String ;
  max_value_txt : String ;
  zone_format   : String ;
  color_txt     : String ;
begin
  zone_format := ZONE_FORMAT_START_END_COLOR_LEGEND ;
  if IsStringEmpty(_legend) then
    zone_format := ZONE_FORMAT_START_END_COLOR ;

  if _usePrecision then begin
    min_value_txt := DotFloatToStrPrec( _minValue, _precision ) ;
    max_value_txt := DotFloatToStrPrec( _maxValue, _precision ) ;
  end
  else begin
    min_value_txt := DotFloatToStr( _minValue ) ;
    max_value_txt := DotFloatToStr( _maxValue ) ;
  end ;

  color_txt := ConstructParamColor( _color ) ;

  Result := Format( zone_format, [min_value_txt, max_value_txt, color_txt, _legend] ) ;
end;
