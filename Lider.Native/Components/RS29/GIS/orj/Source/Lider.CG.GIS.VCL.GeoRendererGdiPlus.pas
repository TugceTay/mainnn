...
unit VCL.GisRendererGdiPlus ;
{$HPPEMIT '#pragma link "VCL.GisRendererGdiPlus"'}
...
type
  ...
  TGIS_RendererVclGdiPlus = class( TGIS_RendererVclAbstract )
    ...
    private
      ...
      /// </summary>
      function getShapeOrigin    : TPoint ; overload ;

      /// <summary>
      ///   Get left-up corner of the original shape's projected extent.
      /// </summary>
      function getShapeOrigin( const _shape : TObject ) : TPoint ; overload ;
...
//=============================================================================
  ...
  function TGIS_RendererVclGdiPlus.fget_BitmapFactory
    : TGIS_BitmapFactory ;
  begin
  ...
  procedure TGIS_RendererVclGdiPlus.PrepareDrawCharts ;
  begin
    prevCanvas := FCanvas;
    prepareChartsCanvas;
  ...
  procedure TGIS_RendererVclGdiPlus.AfterDrawCharts ;
  begin
    FCanvas := prevCanvas ;
  end;

  procedure TGIS_RendererVclGdiPlus.PrepareDrawLabels ;
  begin
    prevCanvas := FCanvas;
    prepareLabelsCanvas;
    FCanvas := oLabelsCanvas ;
  ...
  procedure TGIS_RendererVclGdiPlus.AfterDrawLabels ;
  begin
    FCanvas := prevCanvas ;
  end;
  ...
  function TGIS_RendererVclGdiPlus.getShapeOrigin(
  ...
  begin
    Result := getShapeOrigin( sourceShape ) ;
  end ;
  ...
  begin
    preparePen( _canvas, _color, _style, _bitmap, _pattern, nil, _cap, _join, _width ) ;
  end;
  ...
  begin
    ...
    T_Pen(_canvas.GdipPen).LineDash := _dash ;
            ...
            if assigned( params_marker.Symbol ) then begin
              ...
              if ( params_marker.Style = TGIS_MarkerStyle.Circle     ) and
                ...
                try
                  ...
                  drawMarker( params_marker.Style, isize + TwipsToPixels( params_marker.OutlineWidth ),
                              Point( pt_x, pt_y ), False, params_marker ) ;
                finally
                  FCanvas.usePen := usepen ;
                end ;
              end
              else begin
                prepareBrush(
    ...
    var
      ...
      begin
        ...
        try
          ...
          // verify
          tbmp.SetData( NativeBitmapFactory, bmp ) ;
          FCanvas.usePen := False ;
          try
            prepareBrush(
              FCanvas,
      ...
      begin
        ...
        if (rct.Width > 0) and (rct.Height > 0) then begin
          bmp := Vcl.Graphics.TBitmap(createTransparentBitmap( rct.Width, rct.Height )) ;
          cnv := FCanvas.Canvas ;
          cnvp := FCanvas.GdipCanvas ;
          FCanvas.Canvas := bmp.Canvas ;
          FCanvas.GdipCanvas := TGIS_GdipGraphics.Create( FCanvas.Canvas.Handle ) ;
          ...
          try
            ...
            T_Brush(FCanvas.GdipBrush).BitmapOrigin := getShapeOrigin( _shp ) ;
            drawPolyPolygon( draw_buf, part_buf, num_parts ) ;
          finally
            FCanvas.usePen := True ;
            // frees also bmp
  ...
  procedure TGIS_RendererVclGdiPlus.doShapeMultiPatch(
    const _shp           : TGIS_ShapePolygon ;
    const _source        : TGIS_ShapePolygon ;
    const _selectionOnly : Boolean
  ) ;
  ...
  begin
    ...
    // verify
    VCL.Graphics.TBitmap( bmp.GetData( BitmapFactory ) ).AlphaFormat
      := TAlphaFormat.afPremultiplied ;
    // verify
    renderBitmapInternal(
      nil,
      VCL.Graphics.TBitmap( bmp.GetData( BitmapFactory ) ),
      _bmp,
      _size,
    ...
    if Assigned( Viewer ) and Viewer.Editor.InEdit then begin
      if assigned( Viewer.Editor.CurrentShape ) then begin
        ...
        if assigned( TGIS_Shape( Viewer.Editor.CurrentShape ).Layer ) then begin
          ...
          drawEditingPoints(
            TBitmap( TGIS_Bitmap( _context ).NativeBitmap ).Canvas,
  ...
  var
    ...
    begin
      ...
      if ( _context is TCanvas ) then begin
        ...
        if assigned( TBitmap(TGIS_Bitmap(_context).NativeBitmap) ) then
          FCanvas := TGIS_CanvasInternal.Create(
                       TBitmap(TGIS_Bitmap(_context).NativeBitmap).Canvas ) ;
      end;
    end;
  ...
  function TGIS_RendererVclGdiPlus.FriendlyName
    : String ;
  begin
    Result := 'GdiPlus' ;
  end ;
  ...
  function TGIS_RendererVclGdiPlus.CanvasTextBaseline(
    const _text : String
  ) : Single ;
  begin
    assert( assigned( FCanvas ) ) ;
  ...
  begin
    ...
    w_outline := Abs( FCanvas.Pen.Width ) ;

    if ( FCanvas.Pen.Style <> TGIS_PenStyle.Clear ) and
