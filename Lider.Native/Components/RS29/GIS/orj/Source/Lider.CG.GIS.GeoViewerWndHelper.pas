...
{$IFDEF DCC}
  unit GisViewerWndHelper;
  {$HPPEMIT '#pragma link "GisViewerWndHelper"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_ViewerWndHelper = {$IFDEF OXYGENE} public {$ENDIF} class( TGIS_ObjectDisposable )
    ...
    public
      ...
      /// <summary>
      ///   Reset context to provided cache size and provided extent
      ///   of the map
      /// </summary>
      /// <param name="_extent">
      ///   context extented
      ...
      ///   cache cahce width
      /// </param>
      procedure Reset              ( const _extent : TGIS_Extent;
      ...
      ///   This overload is generally to be used upon scaling basemap.
      /// </remarks>
      function ScaledRect          ( const _vext   : TGIS_Extent ;
    ...
    public
      ...
      /// <summary>
      ///  Future extent of the map after zoom etc.
      /// </summary>
      property DesiredExtent : TGIS_Extent read extDesired;
...
//==============================================================================
  ...
  var
    rct : TRectF ;
  begin
    rct := RectF( 0, 0, _width, _height ) ;

    rctOriginal := rct ;
    rctScaled   := rct ;
    extOriginal := _TGIS_Extent( _extent ) ;
    extNonscale := _TGIS_Extent( _extent ) ;
  end;
...
//?    Result := oContext.oVwr.Interrupt ;
 ...
 function TGIS_ViewerWndHelper.ScaledRect(
  ...
  begin
    ...
    sc := ( ex.XMax - ex.XMin ) / rctOriginal.Width / zm ;

    rct := RectF( ( _ext.XMin - ex.XMin ) / sc + ( oVwr.Parent.ControlCanvasWidth  - rctOriginal.Width  * zm ) /2,
                  ( ex.YMax - _ext.YMax ) / sc + ( oVwr.Parent.ControlCanvasHeight - rctOriginal.Height * zm ) /2,
                  ( _ext.XMin - ex.XMin ) / sc + ( oVwr.Parent.ControlCanvasWidth  - rctOriginal.Width  * zm ) /2 + ( _ext.XMax - _ext.XMin ) / sc,
                  ( ex.YMax - _ext.YMax ) / sc + ( oVwr.Parent.ControlCanvasHeight - rctOriginal.Height * zm ) /2 + ( _ext.YMax - _ext.YMin ) / sc
                );
  ...
  begin
    ...
    Result.YMin :=  ex.YMax - ( ex.YMax - ex.YMin) / hc * _rct.Bottom ;
    Result.XMax :=  ex.XMin + ( ex.XMax - ex.XMin) / wc * _rct.Right  ;
    Result.YMax :=  ex.YMax - ( ex.YMax - ex.YMin) / hc * _rct.Top    ;
  end;
