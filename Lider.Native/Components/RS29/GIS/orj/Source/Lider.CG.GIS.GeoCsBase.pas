...
const
  ...
  SECONDS_PER_RADIAN  : Double = 206264.8062471      ;
  // Toms region 1 constant
  AD_C                : Double = 1.0026000           ;
  // cosine of 67.5 degrees
  COS_67_5            : Double = 0.38268343236508977 ;
  // Polar limit
  MOLODENSKY_MAX      : Double = 89.75 * Pi / 180.0  ;
...
type
  ...
  T_CSGridShift = {$IFDEF OXYGENE} abstract {$ENDIF} class
    ...
    protected
      ...
      ///   coordinates to be shifted
      /// </param>
      procedure doTransformFwd          ( var   _ptg       : TGIS_Point3D
                                        ) ; virtual ;

      /// <summary>
      ///   Transform single point reverse.
      /// </summary>
      /// <param name="_ptg_src">
      ///   coordinates to be shifted
      /// </param>
      /// <param name="_ptg">
      ///   shifted coordinats (for iteration purposes)
      /// </param>
      procedure doTransformRev          ( const _ptg_src   : TGIS_Point3D ;
    ...
    public
      ...
      ///   shifted coordinats (for iteration purposes)
      /// </param>
      procedure From_Ref                ( const _ptg_src   : TGIS_Point3D ;
                                          var   _ptg       : TGIS_Point3D
                                        ) ;
...
procedure T_CSGridShift.doTransformFwd(
  var   _ptg       : TGIS_Point3D
) ;
var
  dcol,
...
procedure T_CSGridShift.doTransformRev(
  const _ptg_src   : TGIS_Point3D ;
  var   _ptg       : TGIS_Point3D
) ;
var
  dcol,
...
begin
  ...
  _ptg.X := _ptg_src.X - xshift ;
  _ptg.Y := _ptg_src.Y - yshift ;
end ;
  ...
  try
    ...
    if assigned( oCurrentGrid ) then begin
      ...
      if assigned( oFallback ) then begin
        ...
        ptg_z := _TGIS_Point3D( _ptg ) ;
        oFallback.ToWGS3D_Ref( ptg_z );
        _ptg.Z := ptg_z.Z ;
      end ;
  ...
  try
    ...
    for i := 1 to 4 do begin
      ...
      if assigned( oCurrentGrid ) then begin
        ...
        if assigned( oFallback ) and ( i = 1 ) then begin
          // fall back
          ptg_z := _TGIS_Point3D( _ptg ) ;
          oFallback.FromWGS3D_Ref( ptg_z );
          ptg.Z := ptg_z.Z ;
        end ;

        // calcucate current iteration
        T_CSGridShift( oCurrentGrid ).From_Ref( _ptg, ptg );
      end
      else
      if assigned( oFallback ) and ( i = 1 ) then begin
        // fall back
    ...
    if not assigned( Result ) then begin
      if _transform <= 0 then begin
        if _ellipsoid = 7030 then begin
          ...
          if _ellipsoid < GIS_EPSG_AUTO then
