...
{$IFDEF DCC}
   unit GisClasses ;
   {$HPPEMIT '#pragma link "GisClasses"'}
{$ENDIF}
{$IFDEF CLR}
...
type
  ...
  TGIS_EnvironmentInfo = {$IFDEF OXYGENE} public {$ENDIF} class
    ...
    public
      ...
      ///   Is ARM 32-bit
      /// </summary>
      property IsARM32   : Boolean read FIsARM32   ;

      /// <summary>
      ///   Is ARM 64-bit
      /// </summary>
      property IsARM64   : Boolean read FIsARM64   ;
  ...
  TGIS_ColorRampList = {$IFDEF OXYGENE} public {$ENDIF} class( TGIS_ThreadClass )
    ...
    private
      ...
      /// <summary>
      ///   Adds a new dynamic ramp to the list.
      ///   It is used to generate random colors in different color spaces.
      /// </summary>
      /// <param name="_name">
      ///   name of the map
      /// </param>
      ...
      ///   type of color map
      /// </param>
      /// <param name="_limitArray">
      ///   an array containing the lower and upper values of the color component, e.g.:
      ///   [150, 200, 50, 150, 0, 255] for RGB mode
      ///   [0, 360, 20, 50, 0, 100] for HSV mode
      ///   the number of lower/upper pairs in the array must match the
      ///   number of elements in the given color space
      /// </param>
      /// <param name="_interpolationMode">
      ///    interpolation type: RGB, HSV, HCL, etc.
      /// </param>
      procedure addExDynamic( const _name              : String ;
                              const _mapType           : TGIS_ColorSchema ;
                              const _limitArray        : TGIS_IntegerArray ;
                              const _interpolationMode : TGIS_ColorInterpolationMode
                            ) ;
...
//=============================================================================
  ...
  var
    ...
    i : Integer;
  begin
    SetLength( ar, length( _delims ) ) ;
    for i:=StringFirst to length( _delims ) - 1 do
      ar[i-StringFirst] := _delims[i] ;
  end;
    ...
    {$IFDEF MANAGED}
      ...
      {$ELSE}
        {$IFDEF DOTNET_STANDARD}
          FIs32 := RuntimeInformation.ProcessArchitecture = Architecture.X86 ;
          FIs64 := RuntimeInformation.ProcessArchitecture = Architecture.X64 ;
          FIsARM32 := RuntimeInformation.ProcessArchitecture = Architecture.Arm ;
          FIsARM64 := RuntimeInformation.ProcessArchitecture = Architecture.Arm64 ;
        {$ELSE}
...
//  procedure TGIS_ColorRampList.addExDynamic(
//    const _name    : String ;
//    const _mapType : TGIS_ColorSchema ;
//    const _hMin    : Integer ;
//    const _hMax    : Integer ;
...
//  end;
  ...
  var
    limitArrayCount : Integer ;
    aMin, aMax, bMin, bMax, cMin, cMax : Double ;
    gm : TGIS_GradientMap ;
  begin
    limitArrayCount := length(_limitArray) ;
    if limitArrayCount mod 2 <> 0 then
      exit;

    if limitArrayCount <> 6 then
      exit ;
    ...
    case _interpolationMode  of
      TGIS_ColorInterpolationMode.HSL,
      TGIS_ColorInterpolationMode.HSL360,
  ...
  // add dynamic HSL/HCL color ramps
  procedure fill_unique;
  begin
    //?addExDynamic('Unique',       TGIS_ColorSchema.Qualitative, [0, 360, 70, 90, 70, 80],   TGIS_ColorInterpolationMode.HSL ) ; // old Unique
    addExDynamic('Unique',          TGIS_ColorSchema.Qualitative, [0, 360, 32,  62, 54, 69],  TGIS_ColorInterpolationMode.HSL ) ; // seaborn_muted6
    //?addExDynamic('UniqueHCL',    TGIS_ColorSchema.Qualitative, [0, 360, 30, 80, 70, 90],   TGIS_ColorInterpolationMode.HCL ) ;
    addExDynamic('UniqueBright',    TGIS_ColorSchema.Qualitative, [0, 360, 75, 100, 44, 52],  TGIS_ColorInterpolationMode.HSL ) ; // seaborn_bright6
    //?addExDynamic('UniqueDarkHCL',TGIS_ColorSchema.Qualitative, [0, 360, 30, 100, 25, 70],  TGIS_ColorInterpolationMode.HCL ) ;
    addExDynamic('UniqueDark',      TGIS_ColorSchema.Qualitative, [0, 360, 58, 100, 22, 38 ], TGIS_ColorInterpolationMode.HSL ) ; // seaborn_dark6
    addExDynamic('UniqueDeep',      TGIS_ColorSchema.Qualitative, [0, 360, 29,  51, 49, 62],  TGIS_ColorInterpolationMode.HSL ) ; // seaborn_deep6
    addExDynamic('UniqueDeepDark',  TGIS_ColorSchema.Qualitative, [0, 360, 8, 40, 7, 40],     TGIS_ColorInterpolationMode.HCL ) ;
    addExDynamic('UniqueLight',     TGIS_ColorSchema.Qualitative, [0, 360, 50, 100, 85, 90],  TGIS_ColorInterpolationMode.HSL ) ; // old Pastel
    addExDynamic('UniquePastel',    TGIS_ColorSchema.Qualitative, [0, 360, 62, 100, 72, 86],  TGIS_ColorInterpolationMode.HSL ) ; // seaborn_pastel6
  end;
  {$ENDREGION}
